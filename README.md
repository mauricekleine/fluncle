...
Production uses the `fluncle` Turso database through Wrangler secrets. Deploys run through Cloudflare Workers Builds on push to `main`, and migrations apply as part of the deploy step: the Cloudflare **Deploy command** is `bun run --cwd apps/web deploy:cf`, which runs the bounded primary migration phase, the idempotent backfills, the required telemetry migration, and only then `wrangler deploy`. Both production deploy scripts read the checked-out operation-receipt contract before constructing a database client or building the Worker. While `get_operation_receipt_legacy` exists, no caller-floor value is required; once that compatibility route is absent, the persistent public `FLUNCLE_OPERATION_RECEIPT_CALLER_FLOOR` value must exactly equal `a58f9441088728efa03f8745813ac17425229c18` on this and every later deploy.

The primary runner reads the generated Drizzle journal and the target's `__drizzle_migrations` ledger, then applies only the prefix before the first pending protected contraction. Normal deploys can therefore apply expansions through `0168_melted_the_liberteens` while `0169_lonely_mariko_yashida`, `0170_motionless_squadron_supreme`, and `0171_watery_skreet` remain unreachable. H4 and H8 use the separate fixed `db:migrate:production:h4` and `db:migrate:production:h8` commands, enforce predecessor ledgers, stop at their manifest tags, and require `FLUNCLE_PROTECTED_MIGRATION_APPROVAL` to equal exactly the pending tags inside that phase. The telemetry runner is required before publication; missing credentials and migration failures stop the command instead of leaving a new Worker ahead of its run-ledger schema. Local `db:migrate` behavior is unchanged. The complete commands, prerequisites, and rollback contract are in [the local database guide](./docs/local-database.md#production-deploy--migrations).
