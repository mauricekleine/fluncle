...
- **The right disposition is a GLOBAL `unlisted` ARTIST RULE, not a prune.** `fluncle admin artists rule <artist-mbid> --verdict unlisted` takes the pop act's `/artist/<slug>` page off the site — 404, out of the `/artists` hub, out of the sitemap, out of search, and their credit renders as plain text on every track page — while the DnB remix stays exactly where it is, stored, playable and indexed. It is derived at read time, so it applies retroactively to rows already crawled and is undone by deleting the rule. Prefer it to any deletion here: it wants no backup, no dry run, and no per-track judgement. Strip the artist's off-genre ORIGINALS only when you actually want those rows gone. The structurally-correct long-term fix (re-crediting the remix to the remixer parsed from the title) is deliberately NOT automated here; the risk of over-pruning outweighs it at this surface size.
