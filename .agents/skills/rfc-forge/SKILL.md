...
### Phase 4 — Taste pass
Run the **/taste** skill on the draft. Aim it at the real questions: is the core idea the *elegant* simplification or is it forcing unrelated concerns together? What's over-engineered, redundant, or missing a simpler path? Is the scope coherent, or should it split? Is the recommendation the timid choice where a bolder one is right? Capture the refinements; they go into the final.
...
- **Nothing is deferred or optional.** Every unit ships, complete. Decomposition, sequencing, and separate PRs are *ordering a complete delivery* — not a menu to cut from.
