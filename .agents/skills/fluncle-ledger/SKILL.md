...
Some failures here are a VERDICT rather than a thrown error, and the row names it in `reason`. These are the sweeps that judge their own item failures as a rate, because an item failure is invisible at the run level until every item is one:

| `reason`                                                                 | What the sweep is saying                                                                                                                                                                         |
| ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ytdlp_failing` / `proxy_failing` / `bot_challenged` / `capture_failing` | Capture's attempts that reached the fetcher (`captureAttempts`, excluding budget-refused and pending rows) failed at or above 90%, over a floor of four. The word is the dominant failure class. |
| `embed_systemic`                                                         | Three consecutive embed ticks failed EVERY attempt with the same error class (`embedFailureClass`, `embedFailureStreak`). One failed item is an item failure; a run of them is a dead engine.    |
| `deezer_blind`                                                           | The ISRC-recovery pass got an empty Deezer answer for essentially every searched row — a statement about the ask, never about the catalogue.                                                     |

The counts beside them are untouched measurements; only the verdict was added. Treat each as a real finding and read the named class before guessing at a cause.
...
| `gateState: "paused"` with `reason: "due_work_repair_pending"` and `ok: true` | Designed backpressure: the Worker deferred a guarded due-work queue read while its source repair converges, and the next tick reads again. It means "not a failure" only while the row's derived `ok` is true: a multi-leg sweep can mark a tick paused while another leg genuinely failed, and that row (`ok: false`, `errors > 0`) is a real failure, triaged under step 3. A clean pause becomes a finding only when one unit pauses on it across several consecutive cadences — the /status prober's own stall bar is `max(3 ticks, one hour of that unit's cadence)`, so use the same shape here; then read the repair backlog with `fluncle admin projections get --json`. A `database_admission` pause also carries `admissionYieldReason` (`queue`, `public-latency`, `database-health`, …) when the runner named one, and that word is what says whether the wall is load or a sick database. |
...
**`gateState`** — `active` and `null` mean the tick looked. `admission-skipped`, `paused`, `disabled` and `locked` mean it never looked, and the Worker has already nulled its work counters. An `admission-skipped` row means the payload never started; its exact admission outcome, wait, yield reason, and `payloadStarted: false` remain in `summaryRaw`. Under the daily retry wrapper (`daily-retry-runner.sh`, the daily and weekly sweeps) the skip carries `errors: 1`, so its derived `ok` is false: the day's slot is still owed and the retry slot will try again. Outside that wrapper (the frequent sweeps) the skip carries `errors: 0`, derived `ok` true, because the next tick is the retry. Read the day, not the row: a daily sweep is healthy when a later row the same day completed, and failed when the final slot's row is also a skip. `forced` and `dry-run` LOOKED, so their counters are real readings and it is the reader's job to exclude them from the `produced == 0` conjunction. A `paused` row names its cause in `summaryRaw.reason`, and both designed causes also carry `throttled: true`: `database_admission` is a database-admission phase yield, and `due_work_repair_pending` is the Worker deferring a guarded due-work queue read (its typed HTTP 503 `due_work_maintenance_pending`) while due-work source repair converges. Neither is a run failure on its own; a paused row whose derived `ok` is false failed for another reason and is triaged as a failure. `partial: true` on a paused row means work measured before the pause is still readable in `summaryRaw`, while the Worker has nulled the columns.
