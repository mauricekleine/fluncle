...
| `gateState: "paused"` with `reason: "due_work_repair_pending"` | Designed backpressure: the Worker deferred a guarded due-work queue read while its source repair converges, and the next tick reads again. It becomes a finding only when one unit pauses on it across several consecutive cadences; then read the repair backlog with `fluncle admin projections get --json`. |
