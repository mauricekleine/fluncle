...
| `/status` per-cron freshness | [`docs/agents/hermes/scripts/fluncle-healthcheck.ts`](../../../docs/agents/hermes/scripts/fluncle-healthcheck.ts) | mirrors `cronSurfaces()` (the prober) — except `cron.healthcheck` itself, which it emits self-evidently (it IS the prober, run by a rave-02 host timer, and writes no marker of its own to read) |
