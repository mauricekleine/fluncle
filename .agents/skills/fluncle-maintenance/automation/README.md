...
Together they cover the inventory: the workflow owns `fluncle` / `claude-code` / `bun` and the base-image report; Renovate owns the Action digests; boat.dev is pinned but manual-watch, deliberately outside the auto-bump (pin-watch re-verifies it after any rebuild it does).
