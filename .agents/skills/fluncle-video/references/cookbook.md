...
- **Grain ANIMATION speed is part of the motion read — and the beat-pull gate scores it.** `filmGrain` reseeds its speckle fast (~24fps via `floor(time*24)`); over a SLOW-moving field that fast reseed reads as a nervous shimmer, not film grain, and the beat-pull gate's motion check picks it up as snap-back jitter. Match the grain's clock to the field's motion — pass a SLOWED time to the grain so the emulsion crawls rather than boils: `filmGrain(col, uv, u_time * 0.3, amt)`. Heavy grain is right (Light-Years); fast-reseeding grain over calm motion is the tell. But note the boundary: slowing the grain only fixes the GRAIN component of a jitter — a real motion pull lives in the wiring (a per-beat signal on travel/position), and no amount of grain-calming fixes that. The gate is hardened to separate the two (it temporally pre-smooths, so it scores structural motion, not grain); fix the wiring, then calm the grain if it still shimmers.
