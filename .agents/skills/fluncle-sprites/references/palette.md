...
2. **Per-sprite SUBSET, not the whole set.** Each sprite is quantized to ONLY the ramps it should use — never the whole set. A mailbox = `CREAMS + BLACKS + REDS` (no gold in the set, so gold _cannot_ creep into a highlight). This is both the cohesion lever and the anti-creep guarantee. **`CREAMS + BLACKS` is always the spine**, then add only the accents the sprite earns.
