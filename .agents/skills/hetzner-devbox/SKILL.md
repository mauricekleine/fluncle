...
1. **Install the Go toolchain** (the one provisioning pre-req; rave-01 is otherwise toolchain-free). Match the `go` version in `apps/ssh/go.mod` (read it; it runs ahead of the distro `golang-go`), so use the **official tarball**, not apt: `curl -fsSLO "https://go.dev/dl/$(curl -fsSL https://go.dev/VERSION?m=text | head -1).linux-amd64.tar.gz"`, `sudo tar -C /usr/local -xzf go*.linux-amd64.tar.gz`, then symlink it **onto systemd's default PATH** so the unit finds it: `sudo ln -sf /usr/local/go/bin/go /usr/local/bin/go`.
