...
- **Texture family.** Pick the texture pole that fits the tune: nebula (soft `Starfield` + gradient washes), analog (heavy `Grain`, exposure flares on onsets), dither (the `DitherField` glitch pole, the squared Discman side), or paint (layered translucent chroma through `KaleidoMirror`). One family should lead; do not run all four at once.
