...
**Draft mode (`--draft`) — verify-only, never shippable.** Checking motion / reactivity / "does the drop land" needs the whole clip rendered with audio, but the ship encode above is deliberately slow. `bun run social:preview <trackId> --composition <CompId> --draft` writes `out/<trackId>.draft.mp4` at **half resolution** (`scale: 0.5` → 540×960; the GLSL shader + bloom are per-pixel, so ~4× fewer pixels is the big win), `x264Preset: "veryfast"`, jpeg intermediates, `crf: 28`, and no VBV cap — measured **~6× faster end-to-end** (7s vs 44s on a stipple-flock comp; the gap widens on heavier shaders). **`fps` is NOT lowered** — it's exactly what a motion check needs, and the beat-pull gate runs fine on a draft (it's resolution-robust). What a draft CANNOT show is whether the load-bearing grain reads or blocks (half-res + jpeg hide it), so the final grain judgment still needs the one full render — and `ship` refuses to package a draft-only output. The ship encode above is untouched by `--draft`.
