...
- **Palette blend.** Run `paletteMix(track.swatches)` to bend the artwork's own colors toward the brand anchors. The background stays a warm near-black, gold stays reserved for the sun, but the accent and kaleido chroma come from the artwork, so a cool track and a warm track look like different nights under the same sun. The palette roles are locked to canon by the pipeline — the field stays Deep Field, the sun stays Eclipse Gold, the ink stays cream, and the artwork's own hues live only in the secondary swatches — so a loud cover can never repaint the night; `paletteMix` enforces exactly that in code (the Loadstar incident: a cold blue sleeve once extinguished the sun — never again).
- **The One Vehicle Rule.** Every video is a journey, and exactly one travelling medium carries it: an orb riding the beat (the sun/Eclipse), lines (waveform ridges, contour displacement), a fractal (mirror/kaleido recursion), glass (refractive panels and ribs), or a glitch (dither/pixel corruption travelling across the frame). The vehicle transforms continuously across the clip — approaching, passing through, arriving — while every other layer supports it and stays subordinate. Two competing vehicles in one video means one of them is wrong, the same way two gold elements mean one is wrong.
  - **The Always-Visible Vehicle corollary.** The beat-synced vehicle is visible from frame one and holds the center of attention for the entire clip; it never makes a late entrance. It may transform, travel, and intensify — it may not be absent. Supporting elements are free to float, fluctuate, and drift in and out, but the mesmerizing main driver IS the video from the first frame to the last. A vehicle that fades or scales in over the first second is fine (it reads immediately); a vehicle that is absent or near-invisible for the first few seconds — waiting for the drop to arrive, resolving out of an empty void — is a violation, even when that staging feels true to the music. If the concept wants a void to depart from, the vehicle is what fills it from frame one, not what answers it later.
