...
- **`<CloseCard>`** (`journey/close-card.tsx`) — the mandatory close card (tagline + the selector signature as the emphasis line). Pass `palette` so both inks match the composition — no fixed gold; gold is the sun, not the type. Drive it from the `"arrive"` phase's `phaseProgress`. Every video ends here.
