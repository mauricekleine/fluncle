...
- **Audio hooks** (`hooks/`) — `useAudioReactivity`, `useBeat`, `useOnset`, `useEnergy`, `useBass`, `useMid`, `useTreble`, `useFlux`, and the lower-level `sampleCurve`/`smoothCurveAtFrame`. Pure, deterministic, audio-reactive: each computes from `useCurrentFrame()`/`fps` and the `audio.*` arrays only. `useAudioReactivity` is the default bus (exposes `energy`/`bass`/`mid`/`treble` + `*Fast` + `hit`/`swell`/`drop` + `flux`): audio disturbs the material and may move the picture too — motion only through a smoothed signal (Motion law, doctrine 7), never the raw beat. The canonical named audio uniforms (`u_audioHit`/`u_audioSwell`/`u_audioDrop`, `u_bassFast`/`u_energyFast`/`u_midFast`/`u_trebleFast`, and `u_beatPulse`/`u_onsetPulse`) are pushed into the shader BY NAME; `u_audioDisturbance` is the only entry in the returned `uniforms` bag (the silent-zero alias class was closed — every uniform a shader declares by name exists in the header and is pushed by name).
...
- **`bun run judge:metrics <trackId|video> [--json] [--intent <file>] [--allow-flash]`** (`src/pipeline/analyze-motion.ts`) — the deterministic-metrics script: writes `out/<trackId>.metrics.json` and folds in beat-pull, so one report carries both hard gates (flash-safety + beat-pull) plus advisory coupling and intent-vs-actual reads. The flash-safety gate (WCAG 2.3.1) and beat-pull both block ship (exit non-zero); `--allow-flash` overrides flash only after an eyeball, and `--intent <file>` points it at the render-intent file to check actual-vs-intended.
- **`out/<trackId>.intent.json`** — the render-intent spine (schema `fluncle.render-intent/1`, frozen in `src/pipeline/intent.ts` as `type RenderIntent`): authored by the agent at concept time and shipped into the bundle as `intent.json`. It declares the vehicle, drop, motion model, and per-band → element bindings the metrics and the judge check the render against; a missing intent is warn-and-stub in v1.
...
type NostalgicCosmosProps = {
  ...
  audio: CosmosAudio; // file, startMs, durationMs (10000–30000), bpm, beatGrid[], onsets[], energyCurve[], bassCurve[], midCurve[], trebleCurve[], fluxCurve[] (optional), rawDynamicsHint (optional per-band pre-normalization crest hint)
