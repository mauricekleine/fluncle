...
- **The pool** — `bun run bridge --plan all` builds the pool from `buildAllFindingsPlan` (`plan.ts`): every finding's logId enumerated from the public `sitemap.xml` (`/api/tracks` caps its page size, so the sitemap is the enumeration path), each fetched + enriched down the SAME `props.json`/`composition.tsx` path the ordered plans use, at bounded concurrency (8). Expect ~60 findings (~55 replayable; the rest ride the default-vehicle morph). It is served over `/plan` exactly like any other plan, so the glass renders it with no changes. VJ mode has no fallback tracklist, so it **fails fast + loud**: a non-OK / thrown sitemap fetch (`www.fluncle.com` fronts a Cloudflare rule that 403s crawler-ish user-agents) or an empty resolved pool throws, and the bridge exits non-zero rather than boot a visual-less show.
