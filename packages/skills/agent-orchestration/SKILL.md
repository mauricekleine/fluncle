...
This orchestrator-and-reviewer role runs on **Opus 5** (`claude-opus-5`, the current Opus tier) — and so does every sub-agent it delegates to. Opus 5 lands at Opus 4.8's pricing, Fable 5 is still too expensive at execution scale, and Sonnet has proven unreliable, so there's no model-tier tradeoff to manage: hold the bar with Opus 5 everywhere (see `AGENTS.md`, "Picking the right models and effort for workflows and subagents"). Execution is still what you offload — you just offload it to Opus 5 sub-agents.
