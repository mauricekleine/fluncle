...
- **Copy-paste type.** Identical cream/gold lines in identical spots at identical times (doctrine 4 already forbids this). Vary placement musically AND derive the ink from the scene's own palette — a dusty rose, an artwork counter-accent, the scene's own light. `FloatingType` carries its own contrast guarantee, so a non-cream ink still reads. Cream is a safe default; gold is the SUN, never the type — including on the `CloseCard`.
