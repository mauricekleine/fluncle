...
1. **One Driver.** Exactly ONE travelling, beat-synced medium per video. orb / lines / fractal / glass / glitch are EXAMPLES, not an enum — invented vehicles are welcome, provided there is exactly one. The video still carries ONE committed Eclipse Gold moment — the "pop" — expressed THROUGH the vehicle, never as a second celestial body sharing the frame. But that gold moment is NOT required to be a sun, disc, orb, or any circular shape: a sweeping shimmer (as in the 4 Seasons thaw-front), a gold crest line, a resolving front, a wash, a single cell igniting — whatever the vehicle's own grammar implies. One driver, one gold moment, free form. Two drivers means one is wrong.
...
- **Copy-paste ink.** Placement is fixed now (doctrine 4: the `TypePlate` owns it) — what must NOT be copy-pasted is the COLOUR. Derive the ink from the scene's own palette — a dusty rose, an artwork counter-accent, the scene's own light. `FloatingType` carries its own contrast guarantee, so a non-cream ink still reads. Cream is a safe default; gold is the SUN, never the type — including on the `CloseCard`.

## The constants you cannot touch

Grain over the whole frame, always. Exactly one committed Eclipse Gold moment, expressed through the vehicle — its FORM is free (shimmer, front, crest, wash, cell, or disc), never defaulting to a circular sun. Oxanium for brand marks and numerals only. `Artist — Title` with the one sanctioned em dash. `Found Jun 4` dates (UTC, tabular, never released/added). Warm darks only. The `CloseCard` ending. Determinism: remotion seeded `random()` and frame-derived values only — never `Math.random` / `Date.now` / `new Date()`. The Retint Rule: artwork hues are minor counter-accents only; gold stays the sun (the type is never gold — its ink follows the composition, the `CloseCard` included).
