...
The default clip is 20s, and the default is a FALLBACK, not a decision — every recent cut has shipped at exactly 20.0s, which is the tell that the waveform was never consulted. **The cut is a per-track musical decision** (doctrine 8): read THIS track's energy curve, choose `--duration-ms <10000-30000>` for its shape, end on a drop or just before a transition, never mid-build — and name the duration decision in your report (even "20s, because the drop resolves at 19.2s"; a silent 20.0s is a miss). Derive the drop timestamp(s) from the energy curve peaks — those are the moments your vehicle ignites.
...
**The metrics gate (MANDATORY — do not skip, do not ship past a fail).** Your eyes at step 5 are the first line, but two defects are temporal artifacts a scrub can miss — beat-pull and a photosensitivity-unsafe strobe — so both are gated objectively in ONE run:

```
bun run --cwd packages/video judge:metrics <trackId>
```

It writes `out/<trackId>.metrics.json` and carries BOTH hard gates; either exits non-zero and BLOCKS ship:
