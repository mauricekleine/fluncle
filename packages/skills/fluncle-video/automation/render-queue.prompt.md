...
**After the render, run the metrics gate:**

```
bun run --cwd packages/video judge:metrics <trackId>
```

It writes `out/<trackId>.metrics.json`, and `ship` refuses unless that record measured the current `out/<trackId>.mp4` and passed, so re-run it after every re-render. It carries three hard gates:
...
- **Flash safety** (WCAG 2.3.1): a coherent, large-area, >3/sec strobe. This pipeline runs unattended, so this gate is what keeps a photosensitivity-unsafe clip off the feed. A fail means the reactivity is strobing: smooth the offending bind and re-render. Never pass `--allow-flash`.
...
**The palette gate runs inside `ship`.** Ship cuts the poster, compares its colour histogram against the last three published posters, and refuses when the palette is too close to any of them (the axis `judge:metrics` is blind to). A first render passes, with nothing to compare against. A palette refusal is a rework: swing the palette (and, on a plate render, the light/heat) clearly away from the named neighbour, re-render, re-run `judge:metrics`, and ship again.

(`detect-beat-pull` is only the fast directional read on the half-res draft while iterating; the verdict that counts is `judge:metrics` on the full render.)
...
When `ship` refuses, it names the gate and the command to run; fix it and ship again. Upload only after `ship` prints `bundle ready`: a refused ship leaves `out/<log-id>/` as it was, which on a re-render can hold an earlier bundle.
...
You have filmed exactly one finding. Output a tight report (finding `logId`/title, vehicle + grain family + register — plus, for a representational render, the subject kind and the stranger-test read on the poster, and for a plate-lane render the `--plate-subject` kind + treatment family (or the "plate lane unavailable — no GEMINI_API_KEY" fallback note) — texture family, the one-line concept with its depth mechanism and landing point, the duration decision, the metadata-to-pixels trace, the shipped `video_url`) and exit.
...
- **Never install dependencies.** The conductor installs at wake, before you start, and the launcher refuses to hand you a workspace whose modules are missing. A `Cannot find module …` mid-render is a BOX fault, not something to repair from inside: a full-workspace install running beside a live render session is what kills the render's process group outright, and the run then ends with nothing shipped and nothing to read. Stop and report the missing module instead.
...
- **Re-runs must not double-render.** Resolve the finding fresh each run (the assigned `FLUNCLE_RENDER_LOG_ID`, else the queue head) and apply the `videoUrl` guard; never carry a finding id across runs.
- **Ship only through the gates.** `ship` refuses unless `judge:metrics` passed on the current render (beat-pull, WCAG flash safety, arc/deadness) and the poster's palette is distinct from the last three published. A refusal means revise the composition and re-render; never upload a bundle `ship` refused, and never pass `--allow-flash`.
