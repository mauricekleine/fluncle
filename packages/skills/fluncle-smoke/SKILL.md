...
2. **Public web + feeds + discovery.** Each registry web-route / feed / discovery surface → `200`: `/`, `/log`, `/log/<logId>`, `/mixtapes`, `/status`, `/galaxy`, `/earth`, `/factory`, `/about`, `/docs/api`; `/rss.xml`, `/atom.xml`, `/podcast.xml`, `/sitemap.xml`, `/llms.txt`, `/robots.txt`, `/cli/latest.sh`, `/api/v1/openapi.json`, `/api/v1/postman.json`.
