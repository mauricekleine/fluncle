...
description: "Keep Fluncle's pinned/baked supply chain current: the version-drift sweep over the rave-02 box image (the oven/bun base = bun, node, uv, the fluncle CLI, the Claude Code CLI, yt-dlp, gh, boat.dev) and the GitHub Actions digests. Ships clearly-safe bumps END-TO-END (edit the pin, PR, green CI, merge); a baked pin then self-deploys via the on-box pin-watch timer (rebuild, pre-smoke, swap, auto-rollback). Brakes (reports, never ships) on a major bump or anything touching auth/runtime/model. ALSO owns the dependency VULNERABILITY posture: Dependabot + the blocking bun-audit workflow, the severity policy, the justified --ignore allowlist, and reachability-sequenced fixes. Use when checking version drift, bumping a box pin, judging a base/CLI/Actions bump, merging a safe bump, triaging an advisory or bun-audit finding, or running the maintenance routine. NOT for freshness-only workspace catalog bumps, nor for what the agent token may do (fluncle-hermes-operator)."
---

# Fluncle maintenance

Fluncle's runtime supply chain is **pinned and baked**: the rave-02 box runs a Docker image that pins an `oven/bun` base by digest plus a small toolchain (node, uv, the `fluncle` CLI, the Claude Code CLI, yt-dlp, gh, boat.dev), and CI runs GitHub Actions pinned by digest. Pins do not float — that is deliberate (a reproducible box, a layer-cache bust on every bump) — so they go stale silently. This skill is the **drift sweep** that keeps them current: read each pin, check latest, and either ship a clearly-safe bump **end-to-end** or report the drift and pull the brake. The galaxy should self-maintain — repo AND box.
...
- The **`oven/bun` base image** every sweep runs on (`FROM oven/bun:<ver>-debian@sha256:…` in `docs/agents/hermes/Dockerfile`) — its tag IS the box's **bun** pin, mirrored in `package.json` `packageManager` (every CI workflow reads bun from there). Tag, digest, and `packageManager` move together; `hermes-pin-drift` ships a same-major bump, and a bun major is a brake.
- **node + npm** and **uv** — digest-pinned `COPY --from` sources in the Dockerfile. **MANUAL-watch tier**, like gh and boat.dev: reported, bumped by hand.
...
- **What the agent token may do, or how the box runs.** Those levers are the **`fluncle-hermes-operator`** skill (Worker role guards, secrets, host timers). This skill only keeps the _toolchain versions_ current. The `fluncle-hermes-operator` skill is the reference for the box's run/smoke mechanics that the on-box pin-watch encodes; the maintenance routine does not drive those mechanics.

## The mental model: pins are deliberate, drift is silent, Opus is the gate

Three facts shape every decision here:

1. **The pins are intentional.** Floating `latest` would make the box non-reproducible and would mean an upstream push silently changes the runtime. So the box pins the whole toolchain. The cost of pinning is that nobody is told when a newer, safer version ships — hence this sweep.
...
**The feeds, and who reads them.** Two complementary feeds cover the whole surface: **Dependabot** (GitHub-native, watches the manifests it can parse, alerts on the default branch) and the **blocking `dependency-audit.yml` workflow** (`bun audit` over `bun.lock`, so it sees the transitive surface Dependabot cannot parse; runs on PRs, main pushes, and weekly). Neither feed pages anyone: they are READ on every maintenance pass — a pass begins by checking both — and the weekly scheduled run plus Dependabot's alert badge are the between-pass backstop. A pass that finds new advisories triages them then and there.
...
**The audit BLOCKS (and what that costs).** `dependency-audit.yml` reports the JSON severity table, then gates on `bun run audit` — the invocation that carries the policy, because `bun audit --json` silently discards `--audit-level` and `--ignore`. A non-allowlisted high therefore reddens the check rather than merely noting it, and **advisory drift can redden `main` with no code change at all**: a fresh advisory against an already-locked dependency fails the next run. That is the intended cost of the gate; the response is to drain the finding through this skill's severity policy, never to loosen the gate. Whether the check is additionally REQUIRED in branch protection stays the operator's toggle, and remains separate from the workflow's own exit code.
