...
3. **box.ascii — nothing to ship.** Pre-1.0, pinned to a checksum-verified release binary, MANUAL-watch tier. The routine never bumps it: its verbs are the render conductor's contract, and the vendor's renamed CLI is a conductor migration, not a version bump. (The pin-watch post-swap smoke is only `fluncle version` + container-running; re-verifying the render conductor is operator / `fluncle-healthcheck` work, not pin-watch's.) A sweep mentions it only as a one-line "pinned, manual watch, re-verify post-rebuild" note.
