...
> **A missed bump is a non-event. A bad shipped bump that takes the box down — with nobody watching — is a real incident.**
...
The repo's CI deploy-gate is `bun run deploy:gate` = repository `format:check` + Go `go:check` + `lint` + `typecheck` + `test` (plus `test:scripts`, gitleaks, and the Cloudflare build on the PR). A PR that edits a **repo-side** pin (`package.json` `packageManager`, an Actions SHA-pin) is fully validated by this gate — that real, automatic validation is what makes those edits safe to **merge** unattended.

But the gate has a **blind spot**: it does **not** rebuild or run the Hermes image. A Dockerfile `FROM`/`npm -g` edit passes the gate trivially (the gate never touches the Dockerfile) and is unvalidated by CI. That blind spot is exactly what the on-box **`fluncle-pin-watch` pre-smoke** covers: after the merge, pin-watch rebuilds the image and smoke-checks it (exactly four checks: `fluncle version`, `claude --version`, an agent-tier read returning `{ok:true}`, and a publish-class command refused with a 403) BEFORE touching the live container — the validation CI could not do — and **auto-rolls-back on a failed smoke**. (The broader boat.dev-auth / `dig` / cron-roster probes belong to the separate `fluncle-healthcheck` cron, not pin-watch.) So:
...
- **A change the pre-smoke can't meaningfully validate** (a new base image's failure mode is every sweep at once, not one probe) → that is a brake, no matter the version delta.
...
2. **A base-image change beyond a same-major bun bump.** The `oven/bun` base is the bun pin: a same-major bun release ships (tag + digest, pin-watch pre-smoke, rollback), a bun major is a brake. Swapping the base distro or image family, and the manual-watch node/uv digests, are the operator's call — report them.
3. **boat.dev — nothing to ship.** Pinned to a checksum-verified release binary, MANUAL-watch tier. The routine never bumps it: its verbs — and their blocking/non-blocking behaviour — are the render conductor's contract, so a bump ships as its own PR with a `--preflight` run behind it. (The pin-watch post-swap smoke is only `fluncle version` + container-running; re-verifying the render conductor is operator / `fluncle-healthcheck` work, not pin-watch's.) A sweep mentions it only as a one-line "pinned, manual watch, re-verify post-rebuild" note.
