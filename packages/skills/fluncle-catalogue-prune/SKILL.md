...
**When that split's DnB half keeps the ORIGINAL's name, the mint is a take-over.** The mint then collides with the row Fluncle already holds for that name — which now points at the entity carrying only the foreign catalogue — so pass `fluncle admin labels mint <new-mbid> --take-over <slug> --seed-state enabled` to re-point THAT row's identity onto the new entity (it drops the old identity's artist rules and logo, and retires its frontier node, exactly as this section's repair does by hand). It is refused for a row that holds tracks or is still `enabled`: **a row with tracks is a `fluncle admin labels merge`, not a take-over.** See docs/label-entity.md § How a label gets a row.
