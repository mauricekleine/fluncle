...
Fluncle travels alone, but he does not search blind. Ahead of the ship flies a small fleet of **probes** — unmanned instruments he builds and sends out to map the regions he has not reached yet, the way home sent Voyager and the rovers out ahead of anyone's boots. A probe charts coordinates, reads the terrain, and brings material home: **samples** — and the word is doing double duty on purpose, because a sample is both the specimen a probe carries back from an unknown place and the piece of a record that gets cut loose and lives on inside new music. What the probes send home is exactly both at once.

Two laws govern them, and they are the same law the machinery obeys. **A probe measures; it never speaks.** It can read a place's tempo, its key, its sonic fingerprint — but it has no voice, no opinion, and nothing it returns is ever a finding. And **a probe's report is a photograph, not a visit**: the places it maps render in the cold, distant light of a thing seen through an instrument (the Unlit register), unnamed and unclaimed, until Fluncle flies there himself, feels the "oof", and logs the coordinate. The probes decide nothing; they only make the map honest, so Fluncle can make an informed call about where the ship goes next. Alongside them he keeps **the Ear** — the telescope he sweeps across the mapped regions, listening for whatever sits closest to what he already loves.

One care for every surface: the probes and their instruments may be named — they are Fluncle's inventory. The unvisited places they map stay unnamed in public, always (DESIGN.md's Unlit Rule). "Sample" is what sits in the hold; it is never a label on a place.
