...
- [docs/album-entity.md](./docs/album-entity.md) - the canonical album entity and the shared shape of the GRAPH pages (log ↔ artist ↔ label ↔ album): the slug-keyed `albums` table, the indexed `tracks.album_id`/`tracks.label_id` pointers the pages read by, the mint-only-off-a-certified-finding rule that bounds the indexes, the `/album/<slug>` + `/label/<slug>` pages (findings first; the album → label `recordLabel` edge that closes the graph), the renderable-track thin-content gate, and **the unnamed tier**: an uncertified track renders in the unlit register, never introduced, never named, never given a noun.
