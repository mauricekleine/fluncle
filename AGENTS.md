...
- **M2 (mixing):** Rekordbox + the DDJ-FLX4 + `master.db` — _only_ the `fluncle-mixtapes` Rekordbox scripts and the `fluncle-rekordbox-sync` key/BPM sync run here (they read `master.db`). No OBS, no browser.
