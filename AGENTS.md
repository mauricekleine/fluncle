...
- MUST: Treat `docs/*-brief.md` and everything under `docs/planning/` (roadmaps) and `docs/rfcs/` (design proposals) as non-canonical brainstorms, planning, and proposals — never specification, and never listed in the canon list below. Where one deviates from the codebase or from canon (`DESIGN.md`, `PRODUCT.md`, `VOICE.md`), the codebase and canon win; translate the idea into Fluncle's terms when picking it up.
