...
- [docs/error-tracking.md](./docs/error-tracking.md) - Sentry error tracking for `apps/web` (private diagnostics), the counterpart to the public `/status` liveness stack: the posture (errors + sampled DB-query tracing on the Team plan, no replay/profiling, `sendDefaultPii: false`) with per-query `db.query` spans on the `getDb` chokepoint feeding the Queries insight + Slow DB Queries detector and the `tracesSampler` route policy, what is covered (browser + root `errorComponent`, the `withSentry`-wrapped Worker over `handleOrpc` + the router, and the `apiFault` 500 capture) vs pending (mobile, box sweeps), the commit-SHA release + `@sentry/vite-plugin` source-map upload gated on `SENTRY_AUTH_TOKEN`, the committed public DSNs, the one operator build-env requirement, and the read side — the nightly `fluncle-sentry-triage` box cron that opens a fix PR per straightforward error and files the rest to `docs/sentry-backlog.md`.
- [docs/search.md](./docs/search.md) - Fluncle's search, the surface that becomes the primary navigation at catalogue scale: the four-tier resolver (coordinate → exact entity → FTS5 token → a small LLM that emits FILTERS, never rows), the deterministic sonic tier (`sounds like <a real track>` → an anchored `vector_distance_cos` scan), the degradation contract (the model is never on the hot path, and search still works when it is down), the catalogue rule + DESIGN.md's Unlit Rule on the wire, and why the FTS5 index is a derived artifact rather than a migration.
...
- [docs/catalogue-crawler.md](./docs/catalogue-crawler.md) - the catalogue crawler: METADATA acquisition only, writing uncertified catalogue rows into `tracks` (a `tracks` row with no `findings` row) and never a certification. The boundary gate (the operator's seed-label allowlist + graph distance, hop 0→2, with NO genre inference), the widening loop (a discovered label enters `undecided` and is not crawled until he rules on it), why MusicBrainz carries the walk (Discogs has no recording identity; Spotify is demoted to a per-track ISRC anchor), the `crawl_frontier` table that makes it deterministic + resumable, the shared 1 req/s MusicBrainz client, the two-layer idempotence, the certification rail, and the on-box `fluncle-crawl` sweep. The crawler makes the rows exist; [docs/the-ear.md](./docs/the-ear.md) is the other half of the same `catalogue` domain (it ranks them) and the two share a table, a CLI group, an oRPC domain, and a cron loop.
- [docs/label-entity.md](./docs/label-entity.md) - the canonical label entity: the slug-keyed `labels` table related to the raw `tracks.label` string, the operator's crawl-seed control (`enabled`/`disabled`/`undecided`) and its **crawl-scope-never-storage** rule, the auto-minting write paths + the one-time starting ruling, the `/admin/labels` station, and the `label-review` attention source.
- [docs/album-entity.md](./docs/album-entity.md) - the canonical album entity and the shared shape of the GRAPH pages (log ↔ artist ↔ label ↔ album): the slug-keyed `albums` table, the indexed `tracks.album_id`/`tracks.label_id` pointers the pages read by, the findings-bounded editorial indexes (the crawler now mints catalogue-scale rows, but `/albums` + `/labels` stay archive-sized via a findings join), the `/album/<slug>` + `/label/<slug>` pages (findings first; the album → label `recordLabel` edge that closes the graph), the renderable-track thin-content gate, and **the unnamed tier**: an uncertified track renders in the unlit register, never introduced, never named, never given a noun.
- [docs/album-artwork.md](./docs/album-artwork.md) - the owned cover master: one ≤1200²-capped cover derivative per album/artist in Fluncle's own R2 (`albums/<slug>.<ext>` / `artists/<slug>.<ext>`), best-source-wins (album: Apple template → Cover Art Archive → Spotify floor; artist: Spotify), the REF-05 line (own the display derivative, never the 3000² original), the `backfill_cover_masters` sweep + `fluncle-cover-masters` cron, serving via Cloudflare Images `/cdn-cgi/image/…` with the 64/300/640/1200 ladder + the `?v` bust, the DTO preferring the owned master so web/mobile/video upgrade at once, and the operator's prod-verification curl.
- [docs/the-ear.md](./docs/the-ear.md) - The Ear, the ranked catalogue: a catalogue track is a `tracks` row with no `findings` row, ranked by max-similarity to its single NEAREST finding (never a centroid — his taste is multi-modal), with every row carrying the finding it matched as its WHY; the precompute sweep (`rank_catalogue`) that does the vector math in SQL once so the `/admin/catalogue` page does none, the corpus-fingerprint staleness that makes it self-healing, and the pre-audio capture-priority ladder that decides whose audio gets bought next.
- [docs/gpu-batch-embed.md](./docs/gpu-batch-embed.md) - the audio pipeline at catalogue scale: what the tracks/findings split left behind (all three sweeps read their worklists through the FINDING JOIN, so a catalogue track was invisible to capture/analysis/embedding and The Ear had nothing to rank), the catalogue-aware `list_track_work` queues and their drain order (certified first, then the Ear's `capture_priority` ladder — the order IS the metered capture budget; a ruled-out label is tier −1 and excluded outright), the CERTIFICATION RAIL (`updateTrack` lets an uncertified track be MEASURED — bpm/key/features/embedding/capture — and 409s every field that would make Fluncle SPEAK about it), and the operator-fired GPU batch embed on RunPod (one inference script for CPU and GPU, so the two paths share a vector space) + its runbook.
- [docs/agents/newsletter-agent.md](./docs/agents/newsletter-agent.md) - the weekly newsletter authoring doctrine for the on-box `fluncle-newsletter` Hermes cron (Friday 15:00 Amsterdam): the self-healing discovery window off the last sent edition, the voice rails, the persist-draft-then-offer flow (a draft `editions` row + an attention-queue item; the operator sends via `fluncle admin newsletter send <id>` or /admin/newsletter — Resend Broadcast, operator-gated), and its `/api/tracks` contract.
...
- [docs/agents/prompt-registry.md](./docs/agents/prompt-registry.md) - the prompt registry: every prompt Fluncle feeds a model at runtime lives in the DB, versioned and editable from `/admin/prompts` + the `fluncle admin prompts` CLI with **no deploy and no box rebake** — the repo keeps the baked default and a DB row OVERRIDES it (a missing/corrupt row falls back and logs, never throws). The append-only `prompt_versions` table (why a table, not the `settings` KV), the AGENT-tier `get_prompt` read the pinned-CLI box lives on, the two-construct template, the `*_prompt_version` provenance stamped on every artifact, the prompt inventory, and what deliberately stays baked (the audit briefs, the render brief, the MCP prompts).
...
- [docs/agents/bio-agent.md](./docs/agents/bio-agent.md) - the entity bio, the artist/label sibling of the auto-note: auto-authoring the short Fluncle-voiced paragraph on `/artist/<slug>` + `/label/<slug>` from best-effort Firecrawl facts + identity, the ONE shared `entity-bio-sweep.ts` over two kinds (`--kind`), the two `fluncle-artist-bio` / `fluncle-label-bio` box crons, the `describe_artist` / `describe_label` AGENT-tier routes + `admin {artists,labels} describe` CLI, the shared bio voice gate, the fill-empty-only guarantee, the grounding gap (no on-box finding titles), the operator backfill, and the box-activation-operator-gated note.
...
- [docs/agents/smoke-routine.md](./docs/agents/smoke-routine.md) - the nightly admin-smoke diagnosis, a desktop-app local routine (NOT a box cron — it needs the seeded local DB, a real Chrome, and `.dev.vars`, none on rave-02, so it runs in the main checkout while the Mac is awake): the deterministic `apps/web smoke:routine` wrapper (isolated dev stack on dedicated ports, the three admin browser smokes, the `SMOKE ROUTINE: shell=… queue=… touch=…` summary line, the transient `.dev.vars` DB-URL rewrite the Cloudflare vite plugin forces), the routine's form fields, and the canonical fixture-rot-vs-regression instructions block.
...
- MUST: Give an **admin** route the loader-seeded react-query hybrid — the loader seeds a `useQuery`/`useInfiniteQuery` whose `initialData` is the loaded value, so the live board hydrates from SSR yet can refetch, paired with `useMutation` for writes — and set `refetchOnWindowFocus: true` on those admin queries (public queries set it `false`); exemplar `apps/web/src/routes/admin/labels.tsx`. The single `QueryClient` lives in `__root.tsx`; there are no client-wide `defaultOptions`, so each query states its own focus-refetch.
...
- **MUST: NEVER trust the local database for a PERFORMANCE or SCALE claim.** `turso dev` (sqld) and hosted Turso diverge on precisely the behaviours that decide whether a query survives the archive growing — a change can benchmark clean locally and be catastrophic in production, with no error to warn you. Measured 2026-07-11; the numbers, the traps, and the working shapes are in [docs/local-database.md](./docs/local-database.md) ("Local is not production"). The four that bite:
  ...
  - **NEVER `CREATE INDEX … libsql_vector_idx` against a populated table.** Hosted, it errored `database is locked` and **wedged the database's write path for 20+ minutes**; locally it silently builds an EMPTY index (queries return nothing, no error). The ratified vector shape is an **exact `vector_distance_cos` scan with a btree pre-filter** (key/BPM or galaxy) — no ANN index.
  ...
  - **NEVER fan a multi-probe scan out as `union all` branches over a CTE.** The planner flattens the CTE and re-executes the candidate scan once per branch — 12 probes meant 12 full blob-dragging passes over `tracks` (63 s hosted, measured 2026-07-18 on `/recommendations`); locally the table is small enough that the shape looks fine. Fold the probes into ONE pass instead: `min(vector_distance_cos(vec, ?), …)` in the select list (one-arg `min()` is the aggregate — a single probe binds bare).
- MUST: Prove any query that scans a growing table against **hosted** Turso (a scratch DB, destroyed after) — not `turso dev` — before claiming it scales.
