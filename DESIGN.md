...
### The Letter Plate (an edition)

A **letter** — one sent edition of the mothership, the thing Fluncle addresses and sends back down the trail (LORE.md) — renders at its `L`-marked coordinate as a variant of the archival Plate, not a new surface. Same skeleton as a finding's `/log` page: the Oxanium masthead (the nameplate reads `Letter No. N`, then the coordinate, then the `fluncle://` line), the field row, the decode block, the plate footer. What makes it a **letter and not a page about one** is that it renders as it was sent: the salutation, his opening, the finds each linked home to their own `/log` page, the sign-off. It carries **no cover and no artwork** — a letter is text, and the plate's job here is to hold it still.

The letter's own words are reading text, so they sit in Space Grotesk (§3): the salutation and the sign-off are prose, never Oxanium — Oxanium stays on the masthead, the coordinate, and the numerals, exactly as on every other plate. One action at the foot (get the next one), quiet and outline; the letter never shouts for a subscription it already earned.
