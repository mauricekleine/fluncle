...
- **The title is the loudest text:** extrabold Starlight Cream (the Title tier). Beneath it, a Stardust metadata line carries the artists and the imprint as Graph Links with the release **year** beside them, then the chip row (duration first, then BPM, then key; The Readout Rule). There is **no date column**: the year sits on the metadata line, and where the day matters (`/fresh`, the release band) the day heads its group once, in the one release-date form.
- **The light is the register** (The Unlit Rule). A finding shows its cover in full colour, carries its coordinate at the end of the row, and heats on hover: Gold Veil wash, the coordinate to Eclipse Glow, the artwork to 1.06 behind a gold-tinted edge. A catalogue row shows its **real cover, desaturated and dimmed** (its lead artist's portrait, dimmed, when the record has none; a dust-and-grain tile, never the eclipse tint, when it has neither), catches the Dust Veil, and carries no coordinate and no gold at rest or on hover. No word on the row names the register.
- **Everything rarer lives behind a quiet ⋮ menu** (The Quiet Surface Rule) with the same actions the player carries for the playing track: Listen on Spotify and Similar tracks, plus the row's own (a finding with footage offers its story). The Spotify mark no longer trails the row.
