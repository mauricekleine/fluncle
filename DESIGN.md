...
**The Lore-Art Paint Exemption.** The Galaxy game and the `/pipeline` infographic CARRY lore art, and paint there is theirs to spend: `/pipeline`'s multi-hue node palette — one tint per machine, one per surface, so a node is identified by its colour — is a deliberate exemption from The One Sun Rule, ratified 2026-07-27, on the same footing as the Galaxy game carrying lore art at all on a workstation (§5's Three Areas Rule: the area rule governs chrome and copy, never paint — and it still governs theirs). The exemption covers hue DIVERSITY and nothing else. Every named reservation still binds inside it — among them, Nebula Violet stays exclusive to the live callout (a violet node would read as live, so what is reserved is the BAND, not the hex: blue the top channel with red above green, roughly h240–h300), neutrals stay warm (The Warm Dark Rule, below), and elevation stays shadow-free (§4's Through-the-Glass Rule). Each surface may also choose a STRICTER set than this rule allows: the Galaxy game declines the exemption and holds itself to the canon ramp (`docs/galaxy-sprites.md`). A lore-art surface picks its own hues; it does not get to spend a colour that already means something.
