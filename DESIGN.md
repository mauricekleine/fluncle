...
- **One sound at a time:** any audible media starting elsewhere (Stories, the radio, a video someone unmutes) pauses the preview; the log page's muted footage loop does not.
- **Keyboard:** while the bar is docked, Space or K plays and pauses and J and L step back and forward through the playing list, whenever focus is not in a field, a dialog or a menu; Space toggles only from the page itself and leaves every focused control to do its own job. **Lock screen:** Media Session metadata and play, pause, next and previous, so a phone in a pocket keeps its transport.
