...
- **Surface:** the colophon's ground, a warm near-opaque Sleeve Black pane under an 18px blur, with pane-tooth grain under the content and a Dust Line top edge. It is opaque enough that its text holds AA over the brightest sky (The Legible Sky Rule), and it carries no shadow (Through-the-Glass). It sits directly on the cosmos, never on a plate (One Pane), and the page leaves room below its last row so nothing is trapped under it.
...
- **Keyboard:** Space or K plays and pauses, J and L step back and forward through the playing list, whenever focus is not in a field; Space leaves a focused button or link to do its own job. **Lock screen:** Media Session metadata and play, pause, next and previous, so a phone in a pocket keeps its transport.
