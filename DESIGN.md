...
The core unit of the product — a **finding**, not just a row — and the one row every public track list renders: `/tracks`, `/search`, `/fresh`, the front door's release band, the track lists on the artist, label and album pages, and "Close in sound" on a track's destination (`components/discovery-row.tsx`, fed per list by `lib/discovery-tracks.ts`). The archive at `/findings` keeps the lore variant (`components/track-row.tsx`): the same anatomy, with the Oxanium tabular Log ID as its first column and the **Artist — Title** line.

- **The cover is the play button.** The 3.25rem artwork (6px radius, 1px border) plays the track's bounded preview and hands the player the whole list from that row (the Player Bar, below). The glyph shows on hover and focus and holds while the row sounds; on a touch screen it rides the cover's corner, small, always. The target is at least 44px, and it carries `data-discovery-play`, the hook the capture script and the specs count. It shows only when the row has a live preview source (a stored preview URL or an ISRC); a row whose preview comes back empty dims its glyph and stays tappable.
- **The rest of the row opens the track.** The title is a stretched link over the whole row, to the finding's `/log/<id>` page or the archive track's own `/track/<trackId>` destination (a row neither would take opens Spotify). The credits, the cover and the ⋮ menu sit above it as siblings, never inside it (a link inside a link is not a thing; Graph Link's excluded contexts).
- **The title is the loudest text:** extrabold Starlight Cream (the Title tier). Beneath it, a Stardust metadata line carries the artists and the imprint (as Graph Links where the name has a page) with the release **year** beside them; the credits truncate and the year never does, then the chip row (duration first, then BPM, then key; The Readout Rule). There is **no date column**: the year sits on the metadata line, and where the date matters it heads its group once, in the archive's release-date form (`/fresh`'s week buckets; the Release Entry, below).
- **The light is the register** (The Unlit Rule). A finding shows its cover in full colour (a finding with no cover keeps the eclipse-tinted halftone tile), carries its coordinate at the end of the row, and heats on hover: Gold Veil wash, the coordinate to Eclipse Glow, the artwork to 1.06 behind a gold-tinted edge. A catalogue row shows its **real cover, desaturated and dimmed** (its lead artist's portrait, dimmed, when the record has none; a dust-and-grain tile, never the eclipse tint, when it has neither), catches the Dust Veil, and carries no coordinate and no gold at rest or on hover. No word on the row names the register.
- **Everything rarer lives behind a quiet ⋮ menu** (The Quiet Surface Rule) with the same actions the player carries for the playing track: Listen on Spotify and Similar tracks (offered only when the track has a sound to start from, its own or its artist's), plus the row's own (a finding with footage offers its story), then, under a Dust Line hairline, the listener's two: **Save** (a bookmark glyph, filled once saved, the item then reading "Remove from saves"; any track, kept on the device with no account asked and carried into the account on sign-in) and **Share** (the phone's share sheet, or on a desktop the track's canonical link copied, no query string). The menu closes on the tap, so the Toast (below) confirms it. No platform mark trails the row; Spotify is one tap away in the menu.
- Rows separate with Dust Line borders at 72%; the last row drops its border. The lore variant's caret drifts 2px on hover.
...
### Release Entry (the `/fresh` week)

What came out is counted in **releases**, not rows: one record with several tracks in a week is ONE entry, so an EP never floods a list (`components/fresh/release-entry.tsx`, folded by `lib/fresh-releases.ts`). A release of one track IS the Track Row, unchanged. A release of several is built from the Track Row's own parts and reads as the same object: the cover plays (the release from its first playable track, and on through the list), the record's name is the loudest text and opens its `/album/<slug>` page, a Stardust line carries who made it and the year, and the tail holds the count as the fold's toggle (`4 tracks`, Oxanium tabular, a caret that turns). The tracks fold out beneath as Track Rows, set in past the cover column, in tracklist order as the ISRCs give it; a closed fold renders no rows (the page's structured data lists every track). The light is the register: a release carrying a finding shows its cover lit, the rest unlit, and no word names which.

On `/fresh` the entries sit in **week buckets** (rolling seven days back from today, the window's last few days joining the oldest week). **Each track sits in the week its own release date falls in**: a record whose tracks came out in different weeks (a single ahead of its album) appears in each of those weeks holding that week's tracks only, so a week's count, its entries and its play control all mean exactly what came out that week; the front door's band shows the record whole. A record with no album page is keyed by its name and lead artist, never its date. The **Albums & EPs** view keeps records by what the archive stores: more than one track linked to the album entity, however much of it is in the window (no release type is stored, so a two-version single counts as a record; a record with no album entity counts by its tracks in the window). Each bucket carries a quiet H2 ("This week", "Last week", then the dates), the count as an Oxanium tabular numeral beside the date span, and one outline control, **Play this week**, that hands the player the week from its top and pauses it in place while it sounds. An empty week prints nothing. Above the weeks, the **standouts** are the head of the week as cover tiles, one per release, findings first, printed flat on the plate; they print only when they are a selection (at most half the releases they drew from, two at the least; a thin week tops up from last week, its own newest release first). A release this browser has not seen since the last visit carries a small outlined **New** in Starlight Cream at the head of its credit line, never gold (it is news to the listener, not a claim about the music), and one quiet line counts them. The page ends by saying so: a dashed Dust Line, then one centred Stardust line ("That's every release from the last 30 days. You're caught up."); a window a limit narrowed names the day it reaches back to and points at the Tracks page for the rest.

### Style Chip Row

The way in by sound for a listener who knows the sound they want but no artist to type: one line of quiet pills, one per style in the lexicon (`lib/search-styles.ts`), introduced by a short Stardust line (above the pills on the front door and `/search`, inline beside them on `/tracks`) and scrolling sideways rather than wrapping, its trailing edge fading so a clipped chip reads as more row to reach. It sits above the `/tracks` filter pills, in `/search`'s zero state, and under the front door's worked examples; `components/search/style-chips.tsx` is the one component. Each chip is a real anchor to `/tracks?sound=<style>` (on `/tracks` it toggles `sound` beside the other filters), the pill family of the worked examples: Dust Veil at rest, heating to the Gold Veil on hover (The Ignition Rule). The pressed chip reads as pressed through cream ink and a cream edge, never a gold fill, so the page's one sun stays the focus ring and the lit rows. Chrome, not prose: a chip names its style and nothing else, and no style is ever printed on a track row (a style orders the list; it is not a label the archive can vouch for).
...
The persistent preview player of the public chrome: the object the cover's figure is tethered to by its headphone cable. It is **hidden until the first play**, then docks full width to the bottom of every public page for the rest of the visit, playing or paused, until the listener closes it. `/mix` keeps its own bar, and the chromeless surfaces (`/radio`, `/galaxy`, `/pipeline`, admin) never show it: entering one pauses the preview, so no sound ever plays without a control that can stop it. The shell is `components/player/player-bar.tsx`, mounted once by the public chrome; the queue behind it is `lib/preview-player.ts`.

- **Anatomy:** the playing track's cover (2.75rem, the Track Row artwork treatment), the **title** as the loudest text (the Title tier, a link to the track's own page) over a Stardust artist line, the place in the list as an Oxanium tabular numeral (`4/30`, riding the artist line on a phone), then the transport a preview needs and nothing more: play/pause (the one filled control, cream on the sleeve), next, the playing track's ⋮ menu (the same actions as a row's), and close. There is no scrubber, no volume, no shuffle, no repeat: a preview is short, and the bar is a player, not a studio.
- **Surface:** the colophon's ground, a warm near-opaque Sleeve Black pane under an 18px blur, with pane-tooth grain under the content and a Dust Line top edge. It is opaque enough that its text holds AA over the brightest sky (The Legible Sky Rule), and it carries no shadow (Through-the-Glass). It is one of the surfaces sanctioned to float above the plate (The Plate, below), near-opaque so the plate scrolling under it never reads as a second pane, and the page leaves room below its last row so nothing is trapped under it.
- **Light:** gold is spent on one thing, a 2px progress hairline along the top edge while a **finding** plays. A catalogue track's hairline is Stardust (The Unlit Rule). The bar never heats a catalogue track's title to gold.
- **The list is the queue.** Play on any row hands the player the whole list from that row, in the list's own order, and the next track follows when one ends. A track whose preview comes back empty is skipped and its row's glyph dims. At the end of the list the bar stops and offers one way on, **Keep going**: the last track's sonic neighbours, or the next page of a paginated list. Nothing plays that the listener did not start.
- **It is not a transport.** Nothing on the page moves because the queue did: no scroll, no reveal, no step. The page stays a scroll and the reader can leave in any direction (PRODUCT.md, "The front door").
- **One sound at a time:** any audible media starting elsewhere (Stories, the radio, a video someone unmutes) pauses the preview, and resuming the preview stops any other audible media first; the log page's muted footage loop does not count. Only the listener's own press takes the room back: when a clip ends or a "Keep going" lands while another sound is already playing, the preview does not start, and the bar waits, stopped, for a press. A pause always wins: a clip paused while it is still arriving is cancelled, never counted as missing, and the queue never moves on without the listener. The list moves on by itself only while the listener still wants it playing: a clip that ends, or a missing one that is skipped, holds its place if a pause, a close or another sound came first. A "Keep going" still waiting lapses the moment the listener closes the player, starts or pauses anything, or another sound starts: nothing ever begins on an intent they have moved past. The player lives across pages, so moving to another page is not one of those: a sonic "Keep going" still waiting on the archive plays wherever the listener is, like a clip that keeps playing. A next-page "Keep going" plays only on the page it asked for; landing anywhere else lapses it.
- **The tether:** the seeds a listener followed "Similar tracks" from this visit ride the bar's leading edge as small, desaturated covers strung on one Dust Line cable that runs on into the playing cover, oldest first, at most five; a phone keeps only the nearest seed back (the most recent one whose view is not the page open now, or the only seed, marked current, while its own view is open), in a small strip on the bar's top edge so the transport keeps its room. Each opens its seed's sonic view again, so a rabbit hole stays visible and walkable back; stepping back never reorders the cable, and it clears with the player. The seeds are navigation, not findings: no gold but the focus ring.
...
The one confirmation the public chrome speaks after an action that leaves nothing on screen to show it happened: a save from a ⋮ menu, a copied link, a set saved on `/mix`. One line of chrome copy (a confirmation may carry one dry line of voice), on an opaque Sleeve Black card with a Dust Line edge and no shadow (Through-the-Glass), bottom centre and clear of the docked Player Bar, inside a polite live region so a screen reader hears it. It is mounted on demand (`components/public-toaster.tsx`, fed by `lib/announce.ts`), so no page pays for it before the first confirmation; under reduced motion it appears and leaves without moving. No gold, no icon, no action button: a toast reports, it never asks.
...
**The ink is the host's when the host has one.** Cream is the ink of a name in _reading_ text. Two hosts already own their register and keep it: a **chip**, and the **Track Row's Stardust metadata line** (artists and imprint) (a cream imprint there would invert the row's hierarchy and pull the eye off the music). On those, only the resting colour defers — the dotted underline is the whole affordance, and the heat is unchanged.
...
- **Inside a link.** The `Artist — Title` caption on a cover tile (the findings band, a graph page's grid, a log page's "Close in sound"), a feed row's title line — each of those already sits inside a link to the finding, and a link inside a link is not a thing. There, the name is a **caption**, not a mention: the tile's job is to open the finding.
...
**The Three Areas Rule.** Pages use one of three areas, and the area determines the masthead, register, and amount of fiction. **Lore pages** (the front door `/`, the archive at `/findings`, `/log` entries, `/logbook`, `/galaxies`, `/mixtapes`, `/stories`, `/radio`, About, the newsletter): the fiction is the content — the full plate grammar, the stamped nameplate, first-person prose; this is where the archive speaks. **Catalogue pages** (`/artists`, `/albums`, `/labels`, `/fresh`, their entity pages, and the archive track destination `/track/<trackId>`): reference shelves — the title alone heads the plate (no nameplate, no first-person intro, no signature line), the index intro is one factual line carrying the page's real nouns, and Fluncle appears as data (Found dates, counts, the findings) plus the third-person dossier bio; these pages carry the wiki/SEO weight. **Workstations** (`/search`, `/mix`, `/recommendations`, `/chat`, `/account`, `/galaxy`, and the console pages `/status`, `/reach`, `/pipeline`, `/docs`, `/privacy`): the reader does something, and the interface carries the meaning — no nameplate, no narration, helper paragraphs and lore sub-lines are clutter by definition. The recipe-blog test still governs workstations: if the work sits at the bottom of a page of story, the page is wrong — the recipe IS the page. What survives there: the artifact header, uniform rows, one gesture per row, and the ONE line a state genuinely needs (a disabled action's reason, an empty state's how-it-wakes). Numbers contract (`9/12`), and meaning moves from sentences into marks — the register rides the light (The Unlit Rule), it needs no label. The Galaxy game and `/pipeline` keep their lore art; the area rule governs chrome and copy, never paint (The Lore-Art Paint Exemption, §2). (VOICE.md §5 "The Three Areas" is the language half of this law.)

**The Long Scroll.** Some surfaces are LONG on purpose — the front door (`/`) is the exemplar — and length is only legible when it has a beat. Three things carry it, and a long page that skips any of them reads as a stack of landing-page panels, which this system bans by name.

_The scale._ One spacing scale, in the frontmatter above (`spacing.scale`) and emitted as `--space-*` custom properties in `apps/web/src/styles.css`. Each step from `xs` up is roughly 1.5× the last off a 1rem base, the three sub-`xs` steps are hand-set for the small gaps a scale that size cannot express cleanly, and a surface picks a STEP rather than inventing a number. `--space-band` is the one responsive member — `clamp(3.5rem, 7vw, 5.25rem)` — and it is the gap BETWEEN sections, so a scroll breathes on a desktop and stays walkable on a phone. The scale deliberately does NOT live in `@fluncle/tokens`: that package is the cross-surface contract read by Remotion, Satori, the live glass, and the extension, and a web page's vertical rhythm is not a thing those renderers have.

_The section grammar._ Every band is the same shape, and the shape is a constraint rather than a template: a heading (a real `<h2>`, so the outline reads H1 → H2 per band and the body is `aria-labelledby` it), **at most one** line of intro, **at most one** trailing link out, then the content. Nothing else may enter a section header — no kicker, no eyebrow label, no second call to action, no restating sub-line. The rhythm inside is the scale and nothing else: `--space-md` from a header to its body, `--space-2xs` from a heading to its intro. The shell is `components/front-door/section.tsx`; a later long surface reuses it rather than re-deriving the shape.

_One plate, long._ A long page is still ONE document (The Plate, above), with every band printed FLAT on it. Bands do not each get a pane, because glass does not stack on glass (The One Pane Rule) — the front door reuses `.home-plate`'s exact surface recipe so it and the archive page are visibly the same document, one long and one tall.

_Motion supports the scroll; it never gates it._ Nothing advances on an interaction, nothing reveals on scroll, and no reader is ever stepped through one record at a time to reach the next — that transport model is rejected outright (PRODUCT.md, "The front door"). What motion there is stays inside the 150–180ms budget above, and under `prefers-reduced-motion: reduce` the page is genuinely still: every transition neutralized in one gate placed after the rules it neutralizes, with feedback surviving as COLOUR only.
...
- **Do** put the music first on every surface: artist, title, the Found date, note, and a way out to Spotify one tap away, before anything else.
...
- **Do** pick a step off the spacing scale on a long surface, and give every band the same header shape: heading, at most one intro line, at most one link out (The Long Scroll).
...
- **Don't** build a surface the reader has to advance through one record at a time to see the next; breadth is the point of a front door, and a transport hides it (The Long Scroll).
