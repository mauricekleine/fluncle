...
**The Workbench Rule.** Pages come in two kinds, and they are built differently. A **reading surface** — the archive, a `/log/<id>` entry, an artist/label/album page, About — is informational: prose is content there, the lore and the framing do real work (and carry the wiki/SEO weight), and The Plate's full grammar belongs. A **work surface** — `/mix`, `/recommendations`, `/chat` — is where the reader DOES something, and there the interface carries the meaning: the artifact, the rows, and the controls say everything, so helper paragraphs, explainer kickers, lore sub-lines, and section labels that restate the rows are clutter by definition. The test is the recipe blog: if the work sits at the bottom of a page of story, the page is wrong — the recipe IS the page, straight to it. What survives on a work surface: the masthead's nameplate (the tagline speaks only where a gate still has to make a pitch), the artifact header, uniform rows, one gesture per row, and the ONE line a state genuinely needs — a disabled action's reason, an empty state's how-it-wakes, and nothing more. Numbers contract (`9/12`, never "9 of 12 picked"), and meaning moves from sentences into marks: the Fluncle seal is a gold coordinate pill in the chips row, not a "From Fluncle's own log" label — the register already rides the light (The Unlit Rule), it needs no label. Narrative saturation stays uniform (VOICE.md's Depth Gradient): on a work surface the lore is carried BY the interface — the artifact's name, the seals, the light — never by captions about it. (Ratified 2026-07-17 off the /recommendations rebuild, which shed its helper prose, its tier labels, and its explainer kickers in one pass.)
...
- **Do** let the interface carry the meaning on a work surface (`/mix`, `/recommendations`, `/chat`): the artifact, uniform rows, one gesture per row, and at most the one line a state genuinely needs (The Workbench Rule).
...
- **Don't** put helper paragraphs, explainer kickers, lore sub-lines, or restating section labels on a work surface; prose is content on a reading surface and clutter on a workbench (The Workbench Rule).
