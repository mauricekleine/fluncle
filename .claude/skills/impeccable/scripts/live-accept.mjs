...
Output (JSON):
  ...
  if (!found && !svelteComponentManifest) {
    console.log(
      JSON.stringify({ error: "Session markers not found for id: " + id, handled: false }),
    );
    process.exit(0);
  }
  ...
  if (isDiscard) {
    ...
    console.log(JSON.stringify({ carbonize: false, file: relFile, handled: true, ...result }));
  } else {
...
function handleAccept(id, variantNum, lines, targetFile, paramValues) {
  ...
  return { acceptedOriginalText: originalContent.join("\n"), carbonize: needsCarbonize };
}
