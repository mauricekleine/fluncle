...
async function pushApplyBatchInChunksAndWait(batch, pageUrl, context = {}) {
  ...
  for (const chunk of chunks) {
    ...
    for (const entryId of chunk.entryIds) {
      ...
      if (!failedByEntry.has(entryId)) {
        failedByEntry.set(entryId, { candidates: [], entryId, reason: "not_reported_applied" });
      }
    }
  }
  ...
  return (req, res) => {
    ...
    if (p === "/manual-edit-stash" && req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ ok: true, pendingCount, perPage, totalCount }));
      });
      return;
    }
    ...
    if (p === "/manual-edit-repair-decision" && req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        if (action !== "rollback") {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ action, error: "unsupported_manual_edit_repair_decision" }));
          return;
        }
...
httpServer = http.createServer(createRequestHandler({ detectScript, livePath, sessionPath }));
