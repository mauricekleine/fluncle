...
Output (JSON):
  ...
  if (args.includes("--check")) {
    if (!fs.existsSync(CONFIG_PATH)) {
      console.log(JSON.stringify({ error: "config_missing", ok: false, path: CONFIG_PATH }));
      process.exit(0);
    }
    let cfg;
    try {
  ...
  // Load config
  if (!fs.existsSync(CONFIG_PATH)) {
    console.error(JSON.stringify({ error: "config_missing", ok: false, path: CONFIG_PATH }));
    process.exit(1);
  }
  ...
  if (args.includes("--remove")) {
    if (svelteKit) {
      const adapterResult = removeSvelteKitLiveAdapter({ config, cwd: process.cwd() });
      console.log(JSON.stringify({ adapter: "sveltekit", ok: true, results: [adapterResult] }));
      return;
    }
  ...
  if (svelteKit) {
    const adapterResult = applySvelteKitLiveAdapter({ config, cwd: process.cwd(), port });
    console.log(
      JSON.stringify({ adapter: "sveltekit", gitIgnore, ok: true, port, results: [adapterResult] }),
    );
    return;
  }
