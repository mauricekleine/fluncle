...
export async function runCopyEditBatchAgent(batch, opts = {}) {
  ...
  const provider = opts.provider || chooseCopyEditAgent({ chatAvailable: opts.chatAvailable, env });
  if (provider === "mock") {
  ...
  if (provider === "codex") {
    await runCodex(prompt, { cwd, env, logPath, resultPath, timeoutMs: opts.timeoutMs });
  } else if (provider === "claude") {
    await runClaude(prompt, { cwd, env, logPath, resultPath, timeoutMs: opts.timeoutMs });
  } else {
...
export function runCopyEditPostApplyChecks({ cwd = process.cwd(), files = [] } = {}) {
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    try {
      ...
      failures.push({ file: relativeFile, message: err.message, reason: "read_failed" });
      continue;
    }
