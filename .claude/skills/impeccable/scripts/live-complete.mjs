...
export async function completeCli() {
  ...
  if (serverResult?.ok) {
    ...
    console.log(
      JSON.stringify(
        { id: args.id, ok: true, phase: snapshot?.phase || args.status, snapshot },
        null,
        2,
      ),
    );
  ...
  const event =
    args.status === "discarded"
      ...
      : args.status === "agent_error"
        ? { id: args.id, message: args.message || "unknown error", type: "agent_error" }
        : { id: args.id, type: "complete" };
  const snapshot = store.appendEvent(event);
  console.log(JSON.stringify({ id: args.id, ok: true, phase: snapshot.phase, snapshot }, null, 2));
}
...
async function completeThroughServer(info, args) {
  ...
  try {
    const res = await fetch(`http://localhost:${info.port}/poll`, {
      body: JSON.stringify({ id: args.id, message: args.message, token: info.token, type }),
