...
(function () {
    ...
    // visual failure mode.
    {
      ...
      if (
        ...
        if (anyVisible) {
          ...
          const childrenInsulate = { bottom: false, left: false, right: false, top: false };
  ...
  function checkPageQualityDOM() {
    return checkPageQualityFromDoc(document).map((f) => ({ detail: f.snippet, type: f.id }));
  }
  ...
  `;
    ...
    function sampleDrawablePixel(drawable, sourcePoint) {
      if (visualContrastRasterCache.has(drawable)) {
        ...
        if (!cached || !cached.ctx) {
          return { reason: cached?.reason || "image sample failed", status: "unresolved" };
        }
        try {
          const x = Math.max(
            0,
    ...
    async function sampleCssBackground(el, style, point, textColor) {
      ...
      if (bgImage && bgImage !== "none") {
        ...
        if (/url\s*\(/i.test(bgImage)) {
          ...
          if (!sourcePoint) {
            return { reason: "point outside background image", status: "unresolved" };
          }
    ...
    async function analyzeVisualContrastCandidate(candidate) {
      ...
      try {
        ...
        return { ...candidate, confidence: "none", reason: "stale selector", status: "unresolved" };
      }
      if (!el) {
        return {
          ...candidate,
    ...
    function collectBrowserFindings() {
      ...
      for (const el of document.querySelectorAll("*")) {
        ...
        const findings = [
          ...checkElementBordersDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementColorsDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementMotionDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementGlowDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementAIPaletteDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementIconTileDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementItalicSerifDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementQualityDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementOversizedH1DOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementClippedOverflowDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementGptBorderShadowDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
          ...checkElementTextOverflowDOM(el).map((f) => ({ detail: f.snippet, type: f.id })),
      ...
      for (const f of layoutFindings) {
        ...
        addBrowserFindings(groupMap, el, [{ detail: f.detail || f.snippet, type: f.type }]);
      }
