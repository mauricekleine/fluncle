...
function locatorTargetsInFile(cwd, relativeFile, op) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    out.push({ file: relativeFile, kind: "reported_locator_match", line: index + 1 });
    if (out.length >= 20) {
      break;
    }
  }
  ...
  for (const rawOp of entry.ops || []) {
    ...
    if (hintedOldText) {
      failures.push({
        candidates: [
          hintedOldText,
          ...targets.map((target) => ({ file: target.file, kind: target.kind, line: target.line })),
          ...candidatesForEntry(batch, entry.id),
        ].slice(0, 12),
    ...
    failures.push({
      candidates: targets
        .map((target) => ({ file: target.file, kind: target.kind, line: target.line }))
