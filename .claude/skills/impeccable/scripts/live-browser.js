...
(function () {
  ...
  const LIVE_UI_SURFACES = [
    ...
    { ids: [PREFIX + "-bar", PREFIX + "-params-panel"], key: "variant-cycling-row" },
  ...
  function showHighlight(el) {
    ...
    if (hiWasHidden) {
      ...
      Object.assign(tooltipEl.style, { display: "block", left: tipX, opacity: "1", top: tipY });
    }
  }
    ...
    try {
      const res = await fetch(
        "http://localhost:" +
          ...
          encodeURIComponent(location.pathname),
        {
          body: JSON.stringify({ action: "rollback", pageUrl: location.pathname, token: TOKEN }),
          headers: { "Content-Type": "application/json" },
          method: "POST",
        },
      );
  ...
  function startScrollLock(sessionId, initialTargetY) {
    ...
    const correct = (why) => {
      ...
      window.scrollTo({ behavior: "instant", left: window.scrollX, top: scrollLockTargetY });
    };
    const schedule = (why) => {
  ...
  function handleInsertCreate() {
    ...
    if (!canCreateInsert({ comments: snapshot.comments, prompt, strokes: snapshot.strokes })) {
      return;
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    shaderState = { canvas, gl, program, rafId: 0, reduced, startTime: performance.now(), texture };
    function frame() {
      if (!shaderState) {
        return;
      }
  ...
  function saveInteractionPrefs() {
    try {
      localStorage.setItem(INTERACTION_PREFS_KEY, JSON.stringify({ insertActive, pickActive }));
    } catch {
      /* ignore */
    }
  }
  ...
  function initGlobalBar() {
    ...
    globalBarEl = el("div", {
      ...
      lineHeight: "1",
      opacity: "0",
      overflow: "hidden", // clip the full-bleed brand mark to the bar radius position: 'fixed', transform: 'translateX(-50%) translateY(20px)', transition: 'opacity 0.3s ' + EASE + ', transform 0.3s ' + EASE, zIndex: Z.bar + 5,
    });
