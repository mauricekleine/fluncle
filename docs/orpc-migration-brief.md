...
**Phase 1 (rails + proof route) landed.** The production rails are in: `@orpc/*` + `zod` added (apps/web + packages/contracts), the contract registry on the `@fluncle/contracts/orpc` subpath (pure-types `index.ts` kept zod-free for the CLI/Raycast/extension), the `adminAuth` / `adminProcedure` / `operatorProcedure` auth spine ported from `adminRole` (apps/web/src/lib/server/orpc-auth.ts), `handleOrpc` mounted in `apps/web/src/server.ts` ahead of TanStack (dual-mounted `/api/v1` + `/api`, `matched:false` fall-through), OpenAPI generated from the router (exposed at the temp `/api/v1/orpc-openapi.json` for diffing; the static `public/openapi.json` flip is a later phase), and one public read converted as the proof + pattern: `get_track` → `GET /tracks/{idOrLogId}` (operationId `getTrack`). A coverage test (apps/web/src/lib/server/orpc-coverage.test.ts) enumerates the public routes and fails the build for any without a contract or an explicit pending-list entry — `get_track` converted, the rest pending. The **fan-out phase** (convert the remaining public reads, then the admin wave, then flip the spec) is what remains; everything below is its plan.
