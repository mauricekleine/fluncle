...
The daemon binds `127.0.0.1:4190` by default — reachable only from the machine it runs on. Set `FLUNCLE_HELM_LAN=1` to bind the LAN/tailnet instead (the `packages/live` precedent), which is the **phone companion path**: with the daemon on the LAN, a phone on the same network can open the same window and drive the same stations — the run drawer and its Stand-down button in your pocket while your hands are on the decks. LAN mode is **authenticated**: at boot the daemon prints the phone URL with the helm key embedded — open that, and the window stashes the key and carries it on every call. `FLUNCLE_HELM_PORT` overrides the port. Ports `4173` and `4180` are never touched — they belong to the live glass and bridge, which the Helm spawns but never serves.

## Security posture

The Helm is **localhost by default** — the safe binding — and only opens to the LAN when you explicitly ask with `FLUNCLE_HELM_LAN=1`, and then only on a trusted show network. Opening the binding does not open the door:

- **The helm key.** A request arriving over the loopback (verified by remote address, never by a header) needs no key. Any other peer — only reachable in LAN mode — must present the helm key on every request, `Authorization: Bearer <key>` or `?key=` (EventSource can't set headers), or it gets a 401. The key is `FLUNCLE_HELM_KEY` if set, otherwise minted once and persisted `0600` at `~/.config/fluncle/helm.key`; the LAN boot line prints the phone URL with it embedded, and that is the only place it is ever shown.
- **Host and Origin are allowlisted on every request.** The Host header must name the daemon itself (loopback on the daemon/dev ports, or a LAN address in LAN mode), and when a browser sends an Origin it must sit on the same list — anything else answers 403. That closes DNS-rebinding and cross-origin POSTs from whatever else the operator has open.
...
- **Children run least-privilege.** A spawned action gets a minimal environment — PATH, HOME, TMPDIR, LANG plus what its route explicitly sets — never the daemon's own `process.env`, so the admin token is not ambient in every child. The legs that genuinely need the CLI credentials (the upload and distribute CLI runs, and the Rekordbox scripts that shell out to the `fluncle` CLI themselves) opt in deliberately with the registry's `adminToken: true`.
- **Operator paths are fenced.** Any file path a request supplies to a spawn — the upload's capture, distribute's video/audio masters — must realpath-resolve inside the scan roots (`~/Movies`, extended by `FLUNCLE_HELM_MEDIA_DIRS`) or the request is refused before an argv exists; `..` traversal and symlink escapes resolve outside and die at the fence.
- **Stations act only on their machine.** A wrong-machine action POST (a station whose manifest excludes this Mac) answers 403 server-side — the same gate as panel visibility, enforced where it counts.

Feature runs are scoped to their station: one station can never stream or kill another's child process. Every run leads its own process group, so a kill reaps its grandchildren too; when the daemon exits it stands every group down escalatingly and waits — SIGINT, a bounded grace, SIGKILL, a bounded grace — before it goes.
...
- **A run is stuck.** Every streamed action has a Stand-down button (SIGINT to the run's process group, then SIGKILL after a grace); `GET /api/runs` lists what the registry knows. Restarting the daemon stands every group down the same escalating way, and waits for it.
- **The phone gets 401s.** A LAN peer must carry the helm key — reopen the phone URL the daemon printed at boot (it embeds the key), or check `~/.config/fluncle/helm.key` / `FLUNCLE_HELM_KEY`. A 403 instead means the Host/Origin allowlist refused the request — dial the daemon by its LAN address, not a name.
