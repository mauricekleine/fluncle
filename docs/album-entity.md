...
A graph page does not require a finding. A label the crawler discovered and Fluncle has certified nothing on **still has a `/label/<slug>` page** when it has visible tracks, and a label with 700 crawled releases and zero findings is a genuinely useful one — an honest record of what that label put out.

A graph page exists when its entity has visible tracks. Render each section only when it has content, and use the thin-content gate over total renderable tracks to keep stubs out of the index.

The public register excludes catalogue tracks whose duration is at least 15 minutes; they remain stored for album and identity completeness, while certified findings remain public at any length. Album, artist, and label tracklists, Upcoming bands, page existence, hub tiles and counts, and sitemap floors all use that visible set. An entity with no visible tracks has no public page. `renderable_track_count` mirrors the visible set; the one-time, resumable deploy repair removes existing hidden memberships, while the bounded nightly hub-count reconciliation sweep repairs later drift and updates the latest release date from visible tracks.
...
Artist and label pages place future-dated tracks in a separate, independently paged **Upcoming** section, keeping certified findings in the existing finding grid and catalogue tracks in the existing track rows; those tracks leave the released findings and groups until their release day. The artist and label indexability gates and their sitemap reads both read the maintained `renderable_track_count`, which counts visible linked tracks, Upcoming ones included, so a future-only entity with enough tracks is indexable and in the sitemap. Artist membership starts at the `track_artists.artist_id` index and also checks fixed-size findings and future-release index windows for exact `artists_json` credits on tracks with no artist edge. The boundary is the current UTC day: a stored `YYYY` or `YYYY-MM` date becomes released at the start of that year or month, and an undated track remains in its existing released list. The `/tracks` hub, `/fresh` page and feeds, and the front door's release band omit future-dated tracks until that boundary.
...
**And the crawled TRACK still earns no COORDINATE.** The rail that did not move: a `tracks` row with no `findings` row is not a finding, has no coordinate, and never gets a `/log` URL. The catalogue can grow without bound and the number of findings in the sitemap does not move.

What a crawl earns is a page for the **entity** its tracks hang off — and, since [the archive track destination](./track-destination.md), a page for the RECORDING too, at `/track/<trackId>`, keyed on the row's permanent primary key. That page is not a `/log` page and cannot be mistaken for one: no coordinate, no note, no video, no Found date, and nothing on it that says Fluncle stands behind the recording. It is the catalogue register applied to one track. The tier it belongs to is still never introduced, named, given a noun, or counted aloud — a destination is not a name.
...
Visually they are held apart by the **unlit register** (DESIGN.md): no cover and no coordinate (they have none), Stardust ink, a hairline rule as the only separator, and **no gold at rest or on hover** — so a hovered unlit row can never be mistaken for a focused one, and the One Sun budget survives a list that could run to dozens of rows. Focus, and only focus, is loud: the canonical Eclipse-Glow ring. A row's LINE leads to the recording's own [`/track/<trackId>` destination](./track-destination.md), with the Spotify mark kept beside it as an explicitly-labelled second control — the way in and the way out, two destinations and two controls. A row the destination would refuse (no title, no artist credit) keeps the old behaviour exactly: out to Spotify, or plain unlinked text when it has no streaming presence either.
...
The gate that admits a row is a disjunction: a **certified** entity is always in (`sum(certified) > 0`); an **uncertified** catalogue entity is in only when its page clears the thin-content floor (`renderable >= floor`) — the same floor the sitemap and the `/entity/<slug>` indexability gate use, so the three agree by construction. Each entity table carries a hub listing index (`<entity>_hub_listing_idx`), keyed on `slug` and partial on exactly this gate with the floor spelled as a literal: the total and the A–Z lane count it without reading a table row, and the page slice walks it in slug order and reads the table only for the rows it returns. The `certified` flag and the counts ride the PAGED rows, never a whole-table read into the isolate. A name-filtered page reads the table once through a materialized gated CTE, because its `like '%…%'` has to read every gated row's name.
