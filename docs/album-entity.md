...
The hub gate is a disjunction: a **certified** entity is always in (`sum(certified) > 0`); an **uncertified** catalogue entity is in when its maintained renderable count clears the floor. Each entity table carries a hub listing index (`<entity>_hub_listing_idx`), keyed on `slug` and partial on that gate. The total and the A–Z lane count it without reading a table row, and the page slice walks it in slug order and reads the table only for the rows it returns. The `certified` flag and the counts ride the PAGED rows, never a whole-table read into the isolate. A name-filtered page reads the table once through a materialized gated CTE, because its `like '%…%'` has to read every gated row's name. Artist and label page indexability and sitemap membership use their live indexed rendered-membership count; a drifted maintained hub counter cannot make their page and sitemap disagree.
