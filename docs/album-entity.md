...
- **The internal GRAPH is populated; PUBLIC reachability is deliberately held back.** Minting the row + `album_id` edge inline is a graph change, not a publicness change, and the two are decoupled on purpose. A crawl-minted album with **no certified finding** is fully in the graph but **invisible on every public surface** — its `/album/<slug>` 404s, it is out of the sitemap, and a catalogue-group heading does not link to it — exactly as before the inline mint. The gate is one greppable predicate, `albumHasCertifiedFindingSql` (`lib/server/albums.ts`), applied at the page resolver, `listAlbumSitemapRows`, and the two `catalogue-groups` album joins. It is **TEMPORARY**: the catalogue-publicness slice (004) is the reviewed flip that removes it, at which point a findings-free record reaches its page through the thin-content gate exactly as a discovered label already does. A **certified-finding** album is unchanged throughout.
