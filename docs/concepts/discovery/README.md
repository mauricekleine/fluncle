...
The durable claim is the invariant, not one run's decimals: **26 samples across the three concepts, and not one of them under the 4.5:1 floor on either score.** That is what the spec asserts and what a failing job would catch. The retained readings are in [`evidence/contrast.json`](./evidence/contrast.json), whose own run bottomed out at 5.22:1; the hosted runner bottoms out around 5.5:1 on the same surfaces, because a runner and a laptop antialias text differently. Both clear the floor with room, and the spec compares the count and the floor exactly while leaving the decimals loose — quoting a single ratio as if it were a constant is how the number goes stale.

The run is hermetic (third-party covers are stubbed) and the ambient motion is grounded, so the ground under every sampled line is the local backdrop and the panes over it, held still. That is deliberate on both counts: the sun-bloom breath cycles the backdrop's luminance over 48 seconds, which moved the figure between runs, and a stopped bloom sits at its brightest — reproducible and the harder case at once.
