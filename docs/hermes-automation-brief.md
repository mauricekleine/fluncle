...
| **Newsletter** (draft/send)                      | ✅ time window via `list_tracks`      | draft agent / send operator (cron drafted) | owned by [newsletter-own-the-stack.md](./rfcs/newsletter-own-the-stack.md); the `fluncle-newsletter` Friday cron persists a draft and offers a `clarify` Send button (operator-gated send). See [§ Newsletter](#newsletter--owned-by-its-own-rfc).     |
