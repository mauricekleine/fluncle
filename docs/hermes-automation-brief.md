...
- **Hermes is the cloud home for the agent brain; every step now runs through it, heavy render included.** Running `claude-sonnet-4.6`, it drains queues via the `fluncle` CLI and does the work itself wherever the compute is light or server-side: the deterministic vendor steps (Last.fm, Discogs, context-note Firecrawl — Worker-side), the **observation** (Sonnet authors the recovered-audio script, the Worker renders it with Cartesia), and **audio analysis** (`ffmpeg` + JS DSP), which is now **live on the box** as the `fluncle-enrich` `--no-agent` cron — it reads the enrich-queue, analyzes on-box (the image carries `ffmpeg` + `bun`), and writes back via `fluncle admin tracks update` (the cron model lives under [hermes/cron/](./agents/hermes/cron/)). Even **video render** (headless Chromium + WebGL) is now Hermes-driven: too heavy for the Hermes box itself (2-vCPU / 4-GB), it is the one step Hermes does not run in-process — instead the `fluncle-render` `--no-agent` cron **conducts** it (live 2026-06-24), waking a separate scale-to-zero box.ascii render box that renders + ships one queued finding via a remote `claude -p`, then parks it (see [hermes/cron/](./agents/hermes/cron/) § the render conductor). This replaced the old Mac-bound desktop routine — renders no longer depend on the operator's laptop being open. Hermes still holds no vendor key and no operator power.
...
All gates are now cleared — the crons are wired and live on the box ([cron README](./agents/hermes/cron/README.md)):
...
- ☑ **CLI admin naming rename landed** (#88, refined by the Convention-B cleanup — the crons invoke `fluncle admin tracks <verb>` against stable names).
...
The reliability columns + resolver refactor shipped with the `backfill` crons — but **under different names than this brief proposed**: they landed as the timestamp pair `backfill_lastfm_done_at` / `backfill_discogs_done_at` (plus `*_attempted_at` / `*_attempts` / `*_failures` siblings; see `apps/web/src/db/schema.ts` and `apps/web/src/components/admin/pipeline/board-model.ts`), not the `lastfm_loved_at` / `discogs_status` this brief sketched. The endpoints were built as oRPC contracts on the existing tiers and the crons wired up in the order above.
