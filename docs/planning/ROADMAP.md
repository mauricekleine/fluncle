...
Two things to know before reading. **Go-to-market is the headline** — the overhaul shipped and the feature freeze that protected it is discharged with it, so what `Now` and `Next` carry is operation, reliability, hygiene, and the go-to-market legs. And **the long tails live in their own ledgers**: the nightly auditor's findings ledger ([docs/audit-backlog.md](../audit-backlog.md), forward-facing — open rows only), the DB query-shape scale backlog ([docs/db-scale-backlog.md](../db-scale-backlog.md)), and the [homogenisation evidence ledger](./homogenisation-evidence.md). A ledger row is promoted into this doc only when it is worth scheduling, and a promoted item cites the row it came from — the ledgers keep the analysis, this doc keeps the schedule.

## The overhaul — shipped; the go-to-market leg remains (operator direction 2026-07-26, landed 2026-09-01)

The headline epic is built. Fluncle is the **drum & bass discovery platform** the direction called for — lore present in the theme and available deeper for whoever opts in; the catalogue is **level 1**, the SSH rave terminal is **level 99**, and depth is a ladder a visitor climbs by choosing to. The design this section once held as future work shipped as the discovery funnel (#1237 the zero-input front door at `/`, #1240 the persistent linkable `/search` beside the ⌘K palette, #1239 the archive track destination `/track/<trackId>` with its certified-row 301, #1241 the archive whole at `/findings`, #1245 the aggregate journey events) and is canon rather than planning now: PRODUCT.md ("The front door", "The discovery funnel") and DESIGN.md (The Three Areas Rule, The Long Scroll) carry the shipped shape, and git history carries the build.

The intake resolved with it. Entry-surface plainness and lore-depth layering shipped as the funnel's registers — every public surface sits in a named area (DESIGN.md's Three Areas Rule) and the Depth Gradient grades technical density down the ladder — and the caller-less `update_mixtape_cue` op stays retired (2026-07-27; git history holds it, should a mixtape-scoped cue rail ever want it back). The rows that outlived the epic moved to their own homes under _Next_: the rave terminal's Logbook screen (_Developer & discovery surfaces_), the delivery-perf remainder (its own heading), and the observation dialogs' per-row "generate" action (_Housekeeping follow-ups_).

**What remains is the go-to-market leg, unchanged in shape:** promoting Fluncle in the DnB communities (Reddit and friends), reaching out to record labels (the groundwork is _Next → Label outreach_), and showing up where the scene is — the **September festival** is the first dated beat, with a batch of printed stickers ready to spread (_Next → Printed collateral_). The mobile app is live on the App Store (approved 2026-07-29), so its announcement rides the go-to-market beats. This section stays planning: it schedules the beats, and the canon docs own what the shipped product is.

## Now — the loop runs itself
...
## Next — reliability, hygiene, and the go-to-market prerequisites
...
- [ ] **The observation dialogs' per-row "generate" action.** The trigger path exists end to end (`observe_track`, admin tier; `fluncle admin tracks observe <id>` and the box timer drive it today), so the gap is placement, not plumbing: wiring a button into the dialogs stays deferred to the admin overhaul, which decides where a per-row action belongs across every station at once (`apps/web/src/components/admin/observation-dialogs.tsx`; [docs/admin-shell.md](../admin-shell.md) § placement contract).

### The delivery-perf remainder — CSS/bundle splitting and the cover-master regrain

The last of the delivery work, now measured against the shipped page shapes rather than provisional ones: further CSS / bundle splitting beyond the two delivery rules and the current weight map in [docs/client-bundle.md](../client-bundle.md), plus the cover-master regrain. Unglamorous, unblocked, and sized by the weight map.
