...
Shape: assemble the corpus (with the operator's edits as the preferred targets), LoRA a small instruct model, run it **behind the existing voice gates** — the gates stay as the safety net, and the win is measured by how rarely they fire. First consumer: the auto-note (highest volume, already fill-empty-only, already has correction pairs). Honest open question: is ~60 notes + ~60 observations + the logbook enough corpus yet, or does this wait for it to thicken? And does a fine-tune actually beat a well-prompted frontier model at this scale? A genuine spike, not a foregone conclusion.
