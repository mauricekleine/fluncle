...
- [ ] **The CLI OAuth `bind:none` handoff-URL follow-up.** Carried from the sweep, and **unverified here** — the literal could not be located in the tree (no `bind: "none"` / `bind_mode` / `deviceBind` spelling anywhere; the only `bind` hits are R2 bindings and socket binds). Get the `file:line` from the flagging session before scheduling it; per AGENTS.md an unverified claim carries no recommendation.
