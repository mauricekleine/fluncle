...
- **Sitemap sharding — SHIPPED (per-entity-type index).** `/sitemap.xml` is a `<sitemapindex>` and the URLs live in children at `/sitemap/<kind>-<n>.xml`, ONE CHILD PER ENTITY TYPE (`pages` / `findings` / `artists` / `labels` / `albums` / `galaxies` / `logbook`), each keyset-paginated at `SITEMAP_MAX_URLS` (45,000, under Google's 50,000 ceiling) and dated `lastmod = max(member lastmod)` so a crawler refetches only the type that changed. GSC now reports submitted/indexed per entity type. IndexNow + the GSC submission still point at `/sitemap.xml` unchanged — the children are discovered from the index. Lives in `apps/web/src/lib/sitemap.ts` (`SITEMAP_KINDS`, `buildSitemapIndexXml`, `buildSitemapShardXml`) + the `sitemap[.]xml.ts` index route and the `sitemap.$shard.ts` child route.
...
- [x] **The set builder came to the pocket (SHIPPED 2026-07-13, in the submission build after all).** The Decks tab: a stepped flow (artist seed → opener via suggestions or archive search → the numbered chain + re-ranking rail), the Scales/Camelot key toggle, device-local persistence, and web-compatible share links (`/mix?set=…`). It rode the submission build instead of waiting for 1.1 — it is the listing's 4.2 lead.
