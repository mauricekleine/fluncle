...
- **⏰ 2026-07-09 10:00 — delete the paused gateway crons (soak cleanup).** The activation migrated all 13 jobs to host timers but **paused** (not deleted) the old gateway crons, a reversible overnight soak. Once the timers have run clean, delete the 13 paused crons (`hermes cron rm <id>` inside the container — the IDs are in the durable-deploy activation notes / the **fluncle-hermes-operator** skill) and prune the now-vestigial `/opt/data/{scripts,skills}` (host timers read the baked `/opt/hermes-scripts`). Low-risk cleanup; the paused crons don't fire, so this is cosmetic.
