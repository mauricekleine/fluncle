...
- **Spine-native edition page — SHIPPED.** A sent edition is now **the letter**: an `L`-marked Log ID (`023.L.1A`, derived from the frozen `number` + `sent_at`, so every back issue already sent got one with no migration and no backfill), its own `/log/<id>` page rendered from the persisted payload (`/newsletter/<number>` 301s there — one identity, one canonical URL), quiet tagged inclusion in the three feeds, `PublicationIssue` schema, a sitemap `<loc>`, and a Letters section in `llms.txt`. The fiction is in [LORE.md](../../LORE.md) (_the letter is what he sends back_); the mechanics in [docs/agents/newsletter-agent.md](../agents/newsletter-agent.md) → _The letter's coordinate_.
