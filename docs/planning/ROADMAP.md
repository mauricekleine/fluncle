...
Three platforms answer `unsupported` to machine callers, each for a stated reason rather than a hedge: **Apple Music** on a clause read (ADPLA §3.3.6(D) confines MusicKit results to facilitating a user's own Apple Music subscription — first-party rendering on Fluncle's own pages is untouched), **Deezer** because the integration is an internal ISRC-recovery rung that yields no link to serve, and **Tidal** because there is no integration at all. Re-ruling Apple is one constant in the envelope module.
