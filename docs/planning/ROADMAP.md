...
- **Tier 3, the uncertified catalogue tail, is deliberately unbuilt.** The gate is economics, not capability: the worklist rides the published-findings feed, so ~85 certified rows cost ~85 rendered scrapes, while the catalogue is five figures and every row is a slow third-party page render. The `tracks.beatport_url` column is already catalogue-grained and would serve an uncertified row unchanged — only the worklist would change. Trigger: demand for store links on catalogue rows, plus a scrape budget that makes it cheap.
