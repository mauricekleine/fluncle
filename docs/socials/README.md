...
| Last.fm            | `fluncle`                  | https://www.last.fm/user/fluncle                                                           | Fluncle alias                  | Music-graph profile; About set to the tagline. A sync hook (love/scrobble) is scoped in [the RFC](../rfcs/lastfm-discogs-sync.md) — needs an API key/secret + a session key             |
| Discogs            | `fluncle`                  | https://www.discogs.com/user/fluncle                                                       | Fluncle alias                  | Music-graph profile (user `31223733`); profile set to the bio. Release-ID enrichment + a findings List are scoped in [the RFC](../rfcs/lastfm-discogs-sync.md) — needs a personal token |
