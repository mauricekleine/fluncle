...
**A restamp is not a change.** The tick's write batch owes repair debt only for rows whose _projected inputs_ moved — `capture_priority`, `duplicate_of_track_id`, `nearest_finding_score`, and on the wrong-audio quarantine `capture_status` and `has_embedding`. The comparison is made in SQL, in the same batch and immediately before the updates, where the old and the new value are both authoritative. The fingerprint itself (`catalogue_rank_corpus`) is read by only one queue's evaluator — the rank queue's own — and the tick settles that one directly, because after its write the row is not due in any branch. So re-stamping the catalogue against a moved corpus, which is most of what the sweep does, mints no repair markers at all; a row that genuinely re-ranks still does.

The settled-row delete carries a fence, because the rank queue's predicate also reads `is_catalogue` and `dismissed_at` and the tick writes neither: another producer can re-project a legitimately-due row between the candidate read and the batch commit, and an unconditional delete would drop it silently. The delete therefore only removes a row older than the instant the tick finished selecting, and both columns ride the changed-row comparison as the values the tick observed, so a concurrent move is a change like any other. See docs/database-performance.md for the full projection-column derivation and the one bounded residue it leaves.
...
**Inside one tier, the metered capture queue takes the anchored rows first** (`spotify_uri is not null`, `track-work.ts` § `workOrder` and the same term in the due-work projection's shared key). Audio is what the proxy bills for, and audio on a row with no Spotify identity cannot become recommendable: `REC_ELIGIBLE_WHERE` requires the anchor, so those bytes wait on a separate billed anchor search before they earn anything. The tier still decides which rows drain first — the anchor never lifts a row across a tier, never past the `capture_priority >= 0` veto, and never reorders the findings or the free analyse/embed queues. It sits above the demand reorder, so a demanded un-anchored row waits behind its anchored siblings at the same tier. The term is read off the column at query time, so a row anchored later simply takes its new place on the next read; the projection recomputes the same key from its own source snapshot when the anchor write changes the row's source version. Adding the term itself is a different act: with the cutover open the queue reads a stored `sort_key`, and no source version moves, so the anchored-first order took effect only once `capture-catalogue` was re-keyed. The definition version that makes such a change re-project, and the `fluncle admin projections rekey` lever that forces one queue now, are specified in [docs/database-performance.md](./database-performance.md) § projections.
...
**The gate is precision over recall, and the operator's pin is the one thing that outranks it.** A genuine match scores a BER around 0.02–0.07; for some recordings the only uploads that exist are a different master or edit of the same release — the same length, BER 0.33–0.46 against both store previews — so every walk lands `unmatched` and the finding never gets full audio, never an embedding, and is absent from similarity search. The gate is right to refuse those, and the operator's ear is the only authority that may overrule it. `pin_capture_source` (`PUT /admin/tracks/{trackId}/capture-source`; `fluncle admin tracks pin-source <id> --upload <url|id>`; the Embeddings cell's Capture-source dialog on `/admin/findings`) writes `tracks.capture_source_pin` — one YouTube video id, reduced server-side from a bare id or any `youtube.com` / `youtu.be` / `music.youtube.com` URL — re-queues the row (`capture_status = 'pending'`, failures reset), clears the rejection memory (it described the ladder's rejections, and the ladder is being bypassed), and stamps the id as the row's YouTube provenance with `youtube_verified_by = 'operator'` (so `/identity` says a human ruled, never "matched by audio fingerprint") and `source_verification = 'operator'`. The sweep then downloads that one id through the same sticky proxy session and skips the search ladder entirely. **The pin does not waive the duration guard**: a pasted id that fails it (a live set for a five-minute finding) is refused, lands `failed`, and retries on later ticks rather than landing `unmatched` — a pinned row can never land `unmatched`. The fingerprint still runs, for the record: a mismatch is logged with its BER (`pinned upload mismatches the store reference (ber=…) — capturing on operator authority`), never added to the rejection memory, and the capture is stamped `capture_verification = 'operator-verified'`, which the historic backfill below and the `verify_capture` route leave alone (the row is excluded from the unverified worklist by its non-null verification, and a stale verdict re-posted for it is an `operator-verified` no-op). A pin is a source hint, never a certification: it moves no `findings` column and is legal on a catalogue row. `clear_capture_source` (`DELETE` on the same path; `--clear`) withdraws the pin and its two `operator` stamps and touches neither the capture status nor the audio already captured; `flag_wrong_audio` retires the pin in the same write that rewinds the capture, so a capture flagged wrong is never re-bought from the upload that produced it — flag first, then pin the right one. The pin rides the `prepare_track_captures` snapshot as a key, and the box validates that snapshot against an exact key allow-list; a field the Worker adds after a box was baked is a fact that bake cannot honour, so the sweep leaves THAT row unreached (logged, not frozen, not re-asked in the tick) and freezes the rest of the batch — a stale bake degrades per row, never per tick. Until the box is rebaked with this sweep, a pinned row is simply not captured; the pin-watch rebake on the next CLI bump closes that.

**The pin can waive the duration guard, deliberately.** `pin_capture_source` takes `allowDurationMismatch` (default false; `fluncle admin tracks pin-source … --allow-duration-mismatch`; the dialog's "Accept a different length" checkbox), stored on `tracks.capture_source_pin_allow_duration` and frozen into the prepare snapshot beside the pin (a flip mid-flight stales the in-flight commit like a pin does). It exists for one case: the only upload that exists is a different EDIT of the same recording — a radio cut against the store's extended mix, or the reverse — and the operator, with both lengths in front of him, wants that edit captured anyway. With it set, `findPinnedUpload` skips the guard for the pinned id and logs `pinned upload duration <x>s vs <y>s — accepted on operator authority`; the fingerprint still runs for the record and the capture is still stamped `operator-verified`. The row's `duration_ms` is never rewritten from the captured file — the finding keeps its store length, because that length describes the release Fluncle points at, not the bytes in the archive. It is written on every pin (a re-pin without it withdraws a standing waiver), `clear_capture_source` resets it, and `flag_wrong_audio` retires it with the pin. Without it a pin behaves exactly as above: a wrong length is refused and lands `failed`.

**Consensus: when the preview is the one that is wrong.** The gate has one blind spot. The store preview is a 30 s excerpt, and when that excerpt falls on a low-information section of the recording (a beatless intro), it scores a BER around 0.33 against EVERY genuine upload — the distributor's own `- Topic` art-track included — while the genuine uploads agree with each other at 0.02–0.04 and a wrong-song candidate agrees with nothing (unrelated recordings sit at 0.42–0.45; measured with the sweep's own matcher, `mutualWindowMatch` in `fingerprint-match.ts`, a 30 s window from the middle of the shorter fingerprint slid over the longer one). So the ladder walk does not remember a preview mismatch on the spot: a duration-verified candidate the preview refuses is HELD — its downloaded file moved off the downloader's slot, its fingerprint kept — until the walk ends. This adds no downloads: the held set is the walk's own `DOWNLOAD_ATTEMPTS` files (`FLUNCLE_CAPTURE_DOWNLOAD_ATTEMPTS`, 3 by default), already on disk, and every one of them is a witness — not a top-N. When the walk ends with no preview match, `findConsensus` asks the held uploads about each other: walking them in rank order, the first candidate that agrees (BER ≤ the gate's own `maxBer()`, 0.20) with at least one candidate from a DIFFERENT channel is accepted as `consensus` (its file moved back onto the `audio.<ext>` slot, the one completed file name the provider journal admits, so the work directory looks exactly like a preview-match capture's), and every held candidate that agrees with it — any channel — is its agreeing set. The one log line: `preview gate rejected N duration-verified candidate(s); M of them agree with each other (ber=…) — accepting <id> on consensus`. **What counts as independent:** the upload's channel (`channel_id`, else the channel name). Two uploads from one channel are one uploader's word said twice and never form a consensus between them (they still join an agreeing set led by someone else); a candidate whose channel yt-dlp did not report cannot be shown independent of anyone and never leads or counts toward independence; a candidate that failed the duration guard was never held and has no say. **Where it sits:** a preview match anywhere in the walk wins over any consensus (it returns before the check runs, and the whole held set is then remembered as wrong audio); the operator's pin never reaches this walk at all and outranks both. **What it is:** machine evidence, stamped `capture_verification = 'consensus-verified'` — the server treats it like any other verified capture, so `verify_capture` re-checks it on a posted verdict (a mismatch on a finding flags it, on a catalogue row quarantines it) rather than stepping aside as it does for `operator-verified`. A consensus capture ships NO YouTube id: the uploads proved they carry one recording, not that it is the recording the ISRC names, and `authorize_track_capture` 422s an id beside anything but `preview-match`. **The memory:** the accepted upload and its agreeing set are not remembered as wrong audio; every held candidate that disagreed with the accepted one is (`fingerprint-mismatch`). With fewer than two independent agreeing uploads the walk lands the plain preview-gate verdict — every held candidate remembered, terminal `unmatched` — and a thrown error mid-walk still remembers what was held. The `no-reference` abstain path is untouched: with no preview to refuse anything, the first duration-verified upload is accepted `unverified` and nothing is ever held.
...
**The cover is rendered in the Worker.** `frontier-cover-html.ts` builds Satori markup; `frontier-cover.ts` rasterizes the SVG with the Worker-compatible resvg module, encodes JPEG bytes under Spotify's size ceiling, and uploads through `putFrontierCover`. Same-zone Cloudflare image transformations cannot serve an in-Worker subrequest, so the cover is encoded locally and the raster modules load lazily outside the ordinary playlist and oRPC import graph. Render or upload failure leaves `cover_uploaded_at` null for the bounded backfill to retry without failing a playlist mint. Upload requires the `ugc-image-upload` grant; a missing scope returns `missing_scope` and leaves the row queued.
...
- `apps/web/src/lib/server/frontier-cover-html.ts` + `apps/web/src/lib/server/frontier-cover.ts` — the Worker-compatible cover markup, render, upload, and bounded backfill.
