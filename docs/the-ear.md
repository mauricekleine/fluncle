...
This set is the precise replacement for the earlier `v4` fingerprint, which folded the **raw** `track_artists` edge count plus the two ruling counts. That was correct but catastrophically over-broad: the crawler writes edges continuously, so the edge count moved every day and re-ranked the whole ~55k-row catalogue daily even when no authorization had actually changed. The qualified set moves _only_ when qualification genuinely changes — precisely when a global re-rank is warranted. The two **first-order** changes it cannot see are re-staled at their write paths instead (`catalogue-rank-restale.ts`): a row's _own_ edge set changing (an edge-less catalogue row connecting to an already-qualified artist — nulled per-track at every `track_artists` edge write) and a label ruling's direct effect on its _own_ tracks (`enabled` authorizes them, `disabled` vetoes them — nulled per-label, a bounded indexed UPDATE on `tracks_label_id_idx`, at the ruling write). Together — qualified-set fingerprint (second-order) + per-track edge nulling (first-order) + per-label ruling nulling (direct) — reproduce every re-stale the raw counts produced, without the daily churn.

**Why a digest of the set, not just its size.** A size-only signal has a real, reachable hole: a membership _swap_ that leaves the count unchanged. The operator rules labels in batches on `/admin/labels`; between two ticks, disabling one label (an artist drops below the weighted threshold, de-qualifies) while enabling another (a different artist crosses it) leaves the size identical — and the de-qualified artist's tracks on _other_ labels keep their now-wrong authorization, the exact money direction (a wrong capture gets bought). So the fingerprint also folds an **order-independent digest** of the qualified id set: the tick reads the ids sorted (`order by artist_id`, ~3.5k rows today, a bounded read that also retires `v4`'s full `count(*) from track_artists`) and hashes them (`sha256`, truncated) in JS — order-stable by the SQL sort, collision-safe in practice, and never computed with SQL length/`unicode` arithmetic (where the collision reasoning goes hand-wavy exactly where the money bug lives).
...
`capture_priority` is the pre-audio answer — the cheap metadata signals that _can_ be read before a single byte is downloaded. It carries **two decisions, cleanly separated** (the artist-primary-capture RFC, shipped and pruned — git history holds it): **AUTHORIZATION** — may we spend a metered per-GB byte on this row at all? — and **PRIORITY** — among the rows we may buy, who first? Discovery is label-driven (the crawler seeds from `enabled` labels); **capture is artist-driven**. An artist who moved from a scene label to a major arrives by himself, and his other tunes deserve the spend even though the major is not a seed.

**BPM IS NOT A GATE — ratified 2026-07-28, standing and final.** The pre-audio ladder gates on IDENTITY and AUTHORIZATION, never on an audio guess. A preview-BPM screen before capture spend has been proposed repeatedly (it was the pruned RFC's Slice 2, and the operator has had to strike it down across multiple agent sessions — hence this paragraph): it does not work, structurally. A 30-second preview often carries only the intro or the buildup; a liquid intro is routinely beatless piano — no beat, no bass, no pattern — so preview-derived BPM is unreliable or missing precisely where the archive's taste lives. A confident-looking out-of-band reading on such a preview is not evidence of an off-genre track; it is evidence of an intro. Do not re-propose BPM — from previews or any other cheap pre-capture source — as a capture gate, a crawl gate, or any other spend/keep decision. (Post-capture full-song BPM analysis for METADATA is unaffected; deck identity likewise treats bpm/key as coarse guards, never identity — `docs/live-deck-identity.md`.)
