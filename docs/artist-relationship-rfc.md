...
**Canon/authority:** the codebase (`apps/web/src/db/schema.ts`, `lib/server/{spotify,discogs,lastfm,observation,youtube}.ts`, `lib/log-schema.ts`, `components/admin/pipeline/*`, `docs/admin-tagging.md`, `packages/registry`, `packages/contracts/src/orpc`), `AGENTS.md`, `DESIGN.md`, `VOICE.md` + `copywriting-fluncle`, the surfaces doctrine, the naming-conventions doc. Planning, not spec.
...
- **The `/admin/tag` queue** (`docs/admin-tagging.md`) is the proven per-item operator loop (queue + keyboard) — the pattern the follow queue reuses.
...
- **★ The missing high-leverage link (taste #7):** make every **Artist — Title** on the track row + the `/log` page a _link_ to `/artist/<slug>`. Humans reach the pages from the archive, and the `@id` graph gets its human-visible internal-link twin (where the SEO equity actually flows). Add to the acceptance criteria.
