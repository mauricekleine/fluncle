...
> Planned: the single-`footage.mp4` bundle moves to a **two-master** model (a clean square source + a portrait `footage.social.mp4`), with orientation/audio variants derived on the fly via Cloudflare Media Transformations rather than stored. The full model, surface map, and one-time migration are specced in [video-variants.md](./video-variants.md). The descriptions below reflect the current single-file bundle until that slice lands.
