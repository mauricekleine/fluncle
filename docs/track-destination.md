...
The destination does not change that. A page is not a name, and a link is not a claim. The page speaks in the catalogue register — it says what the recording is, plainly, and never says what Fluncle thinks of it. Across the surfaces that now link into it (the entity pages, the `/tracks` hub, `/fresh`, the front door's release band, search), the unlit row is still coverless, coordinate-free, dust-inked, and carries no gold at rest or on hover. The only thing that changed is where it goes.
