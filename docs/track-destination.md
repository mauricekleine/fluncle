...
The shared cache builds keys from the canonical origin and drops query parameters, so detail pages enroll only their slashless, queryless URL. A trailing slash would miss the purge target, and a query variant could collide with the canonical page. Personalized and interactive routes stay outside this public cache.
