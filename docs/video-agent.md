...
Fluncle is Maurice's drum & bass publishing brand; the video agent turns one track from Fluncle's Findings into one 1080×1920 social video, rendered locally, unmistakably from the same archive as every other Fluncle video. The agent does not exist as a runtime yet; this doc is the bootstrap that points it at its real instructions.

## What it needs

- a checkout of this repo;
- `bun` (repo scripts) and `ffmpeg` (encode + `ffprobe` verification);
...
- Every word on screen passes VOICE.md; every on-screen fact comes from the track's props (Spotify metadata — release year, label), which is authoritative and needs no citation. Render only fields the props expose; never invent a fact, never web-search for one.
