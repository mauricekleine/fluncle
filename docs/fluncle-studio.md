...
When a cue has no track id, recording and mixtape backfills resolve its artist and title with the same `matchKey` rules as `packages/skills/fluncle-rekordbox-sync/scripts/rekordbox_sync.py`. Keep the TypeScript descriptor synonyms and the Python matcher in step: a remix or VIP is a distinct recording, alternate spellings of the same descriptor resolve together, and an ambiguous match resolves to nothing.
...
- Cut + pure helpers: `apps/cli/src/commands/clips.ts` (`clipCutFfmpegArgs` / `clipCutFilterComplex` (the crop); the `clipsListCommand` / `clipCutCommand` orchestration) + CLI wiring in `apps/cli/src/cli.ts` (`admin clips list|cut`).
...
- Tests (ffmpeg-free): `apps/cli/src/commands/clips.test.ts` (the ffmpeg arg shape, the overlay-free crop filtergraph, and the still-live `resolveClipTracks` / `trackLabel` coverage — never invoking ffmpeg), `apps/web/src/lib/studio-clips.test.ts` (`sortClipsNewestFirst`, `filterClips`, `clipPurgeUrls`), and the oRPC coverage/auth-tier nets (`orpc-admin-coverage.test.ts` + `orpc-auth-coverage.test.ts`) pin the two ops as agent tier.
