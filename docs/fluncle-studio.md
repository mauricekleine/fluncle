...
## The mixtape-scoped rail — DELETED

**There is no mixtape-scoped cue rail.** `StudioCueRail` (`apps/web/src/components/admin/studio-cue-rail.tsx`) was the mixtape-scoped marking rail; the finding-linked `RecordingCueRail` superseded it as the Studio's cue editor, it sat with zero importers, and it has now been deleted (git history keeps it). Its would-be write op, `update_mixtape_cue`, is still a live admin-API endpoint (above) with no caller.
