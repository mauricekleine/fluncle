...
For one pending clip: pull the mixtape's staged set rendition `<mixtapeLogId>/set.mp4` from R2 (public read), then **one ffmpeg pass** — trim `[inMs,outMs]`, crop 16:9 → 9:16 at the clip's `xOffset`, store the result as the clip's pseudo-finding master `<clipId>/footage.mp4`, and mark the clip `done`. The clip ships **clean — no baked text overlay**: recorded set footage doesn't read well under a drawtext caption, and the operator writes the caption on Instagram / TikTok at post time, so the earlier brand frame (title + coordinate + changing Track-ID + ink-halo + fonts) has been removed. Because the clip is stored at `<clipId>/footage.mp4`, the merged `videoCrop(clipId)` / `videoCropPoster(clipId)` / `videoAudioStripped(...)` Media-Transformation helpers (`apps/web/src/lib/media.ts`) finish it — the resolution ladder, the poster, and the silent TikTok variant the clip library (Unit G) serves.

## The cut (one ffmpeg pass)

`fluncle admin clips cut <clipId>` (`apps/cli/src/commands/clips.ts`) — runs on the box:

1. Resolve the clip (`list_clips`) + its source set — a recording's owned `r2Key`, or (legacy) its mixtape (`mixtapeGetCommand`). For a legacy mixtape clip it asserts the mixtape is minted (a committed Log ID) and its set video is **staged** (`setVideoAt` set, i.e. `distribute --set-video` ran) — else `set_not_staged` (the clip stays `pending`); a recording clip asserts its set is staged (`r2Key` present). The cut is a pure crop, so all it needs from the source is the rendition URL.
