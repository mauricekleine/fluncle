...
- **It rides the Spotify budget.** GET-only, through the client's 429 Retry-After backoff (#675) and the publish path's OAuth (`getSpotifyAccessToken`). A 429 stops the pass cleanly; a gone grant makes the whole pass a **no-op** (`configured: false`) until the operator reconnects Spotify. The `labels.label_releases_*` reliability pair (checked_at + attempted_at/failures) is the label-images convention.
...
The tap runs as its own daily on-box sweep, `fluncle-label-releases` (`docs/agents/hermes/label-releases-timer/`), Worker-paced exactly like the crawl — the box holds no Spotify identity, so the probe happens in the Worker (`backfill_label_releases`, agent tier) and the timer only paces it.
