...
- **The fuzzy search + the copyrights post-filter.** There is no id-resolution step: the label **name** is the query. `GET /search?type=album&q=label:"<name>" tag:new` finds a label's last-two-weeks releases with day-one dates — but its `label:` filter is **fuzzy** for generic names (live: `label:"RAM Records"` returned 93 junk albums; `label:"Hospital Records"` returned exactly its 2 real ones), and our Spotify tier has no `label` field on the album object at all. So the mandatory post-filter is the album's **`copyrights`** array: an album is kept only when the seed label's name FOLD-matches one of its ℗/© strings. No copyright match ⇒ skip (never mint on the fuzzy hit alone). Each search hit is read as a **single** `GET /albums/{id}` (for the copyrights), and each kept album's tracks as **single** `GET /tracks/{id}` reads (for `external_ids.isrc` + duration) — the batch `?ids=` endpoints are **403 at our app tier** (measured live 2026-07-20, the same family of tier-cuts as the missing `label` field and the search `limit` ≤ 10 cap), so the tap fetches one id at a time under a per-pass fetch ceiling.
