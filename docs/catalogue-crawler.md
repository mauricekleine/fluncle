...
**The rail held; the LABEL it mints did not.** The crawler writes no finding, so no crawled TRACK ever reached a feed, a sitemap, or the star field — that half was airtight and stayed airtight when it was tested at volume. But the crawler DOES mint a `labels` row for every imprint it discovers (it must — that `undecided` row is the ruling queue), and a label row carries a **public page**. Measured against a 10,800-row synthetic catalogue: **eight `/label/<slug>` pages were live and indexable for imprints Fluncle has never certified a thing on**, each one a wall of Spotify outlinks under the line _"Nothing logged off this one yet."_ The certified/uncertified firewall is on the TRACK; nothing was defending the ENTITY the crawl also creates. It is now: a label with zero findings 404s, and the rule is stated once, in [album-entity.md](./album-entity.md#the-catalogue-deepens-a-page--it-never-creates-one) — **the catalogue deepens a page, it never creates one.**
