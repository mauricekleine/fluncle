...
- **The note → the description, with a `fluncle://` breadcrumb.** The dream note doubles as the YouTube / Mixcloud description, with the mixtape's `fluncle://<logId>` coordinate appended as a derived suffix (the note, a blank line, then `fluncle://<logId>`). The marker is **never stored in the `note` column** — it's appended when the note is rendered (on `/log`) and when the description is built for the platforms at upload. The stored note stays clean; the coordinate rides along wherever the note travels, pointing every external description back to the spine.
