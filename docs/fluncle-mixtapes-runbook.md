...
- `fluncle admin auth youtube` — opens Google's consent screen; the durable refresh token is stored server-side in `youtube_auth` (the dashboard "Make public" button + the resumable upload both ride on it). **At the consent screen pick the `@fluncle` channel (a Brand Account), not a personal Google account** — authorize the wrong identity and the upload fails with `youtubeSignupRequired` (the personal account has no channel). Precondition: the `@fluncle` channel is phone-verified (done) — without it, uploads over 15 min fail at insert.
- `fluncle admin auth mixcloud` — prints a Mixcloud authorize URL (same shape as `auth youtube`); approve, and Mixcloud returns to the admin callback which exchanges the code and stores the token server-side in `mixcloud_auth`. The durable credential never touches the CLI — at upload time the CLI fetches it just-in-time from the Worker for the CLI-direct POST (only the bytes are CLI-side). Token revocable; re-run if a later upload reports an invalid token. (Secrets live on Cloudflare: `MIXCLOUD_CLIENT_ID/SECRET` — no redirect-URI var, it's derived from the request origin.)

_Per mixtape, from the Mac where the assets live:_

1. **Distribute.** `fluncle admin mixtapes distribute <idOrLogId> --video <mixtape>.mp4 --audio <master>` (omit a flag to target one platform). YouTube always lands **unlisted** (the `publish-youtube` gate makes it public); Mixcloud publishes **listed/public immediately** — add `--unlisted` to keep Mixcloud private too (a test run, or a cautious first upload to flip live by hand afterward). The CLI mints the coordinate if it's still a draft, then: streams the video to YouTube (**unlisted**, title + description ending in `fluncle://<logId>` + the cued chapter block, the wide cover set best-effort as the thumbnail; resumes on a mid-upload token expiry or dropped session), and POSTs the full-quality master to Mixcloud (name + description + square cover + a per-track `sections[]` tracklist from cued members, published **listed** directly). Each leg records into `mixtape_social_posts`, dual-writes `mixtapes.{youtube,mixcloud}_url`, and flips the mixtape public on the first link.
...
- ~~**External publishing chain:** Mixcloud upload, YouTube mirror…~~ **Shipped** (the [autopublish RFC](./rfcs/mixtape-autopublish-rfc.md), validated end-to-end 2026-06-20): one `distribute` command does Mixcloud audio + YouTube video on our own server-side OAuth, mint-first lifecycle — see Phase C. Still manual by design: the optional **SoundCloud** mirror (API registration externally gated), the **MusicBrainz** DJ-mix release + **Wikidata** loop, and the **announce** posts (Phase D).
