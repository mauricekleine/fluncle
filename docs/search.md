...
Every search tier applies the shared public track duration predicate: catalogue recordings under fifteen minutes are eligible, while a finding remains eligible at any length. This includes the direct coordinate and Spotify seeks, FTS, filters, sonic anchors, and the one-track sonic view. Sonar has no OR filter, so sonic search asks separately for all findings and short catalogue recordings, merges the ranked answers before its top-k, and rechecks visibility when hydrating their IDs.
