...
This is the offline sibling of Fluncle Live (the live+longform RFC, shipped in #287–#291 and since retired from the tree — see git history; Unit O). The live glass and this hour render share the spine but ship independently. The machinery lives in [`packages/video/src/set-video/`](../packages/video/src/set-video); the creative kit it renders against is unchanged (`packages/video/src/remotion/cosmos.ts`, the fluncle-video skill).
