...
Most canonical ops project onto an HTTP surface, so the oRPC contract registry (`packages/contracts/src/orpc`) is their source of truth. A few operations are **local orchestrations** — they run on the operator's own machine and drive local processes (spawn a browser, hold `caffeinate`, read an audio device) rather than call the Worker. These are still named `verb_noun` and registered here, but they have **no oRPC contract, no `operationId`, and no `fluncle` CLI command** — the CLI is a thin HTTP client (`AGENTS.md`), so a process-launching op does not belong on it. Their surface is a package script; this table is their registry.
...
`run` is deliberately outside the closed CRUD/action verb set: that set was drawn for HTTP resource operations, and none of its members names "launch a live show and hold it up until the operator stands it down." The live-longform RFC ratified `run_show` for exactly this local-exec category; new local orchestrations follow the same shape (`verb_noun`, registered in this table, surfaced as a package script, never faked onto the CLI). The glass page and its phone remote are LAN-local only, so — per the RFC — neither is a public `@fluncle/registry` surface; nothing is added to `packages/registry`.
