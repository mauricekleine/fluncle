...
| `admin backfills` (alias `backfill`) | `previews`, `lastfm`, `discogs`                                                                    | **noun → noun**                                                       | plural group; the verb ("backfill") is implied by the group                                                                                                                                                                                                                                                                              |
...
Everything above governs the public vocabulary. A second ratified law draws the internal boundary: **canon lives where the crew looks; tools speak plainly.** The fiction's vocabulary (findings, bangers, the Galaxy, recovered) belongs on crew-facing public surfaces; internal and operator tooling uses functional names — plain **verb + object** labels ("Post to TikTok", "Create clip", "Start visuals"), never an invented noun, because an invented noun in a tool taxes the operator's mental context every time it is read. The ratifying example is the Helm/Cockpit family: the operator app's visible strings say Cockpit, the visuals, the sync server, start/stop — while the internal identifiers (bundle ids, URL schemes, env vars, package paths) keep their existing names, since renaming plumbing buys migration hazard for zero operator relief (rename the face, keep the plumbing). Name new internal/operator surfaces under this law from the start. The voice boundary in §5 keeps the machine noun out of voice copy; this law is its complement — it keeps the fiction out of the operator's tools.
