...
Most canonical ops project onto an HTTP surface, so the oRPC contract registry (`packages/contracts/src/orpc`) is their source of truth. A few operations are **local orchestrations** — they run on the operator's own machine and drive local processes (spawn a browser, hold `caffeinate`, read an audio device) rather than call the Worker. These are still named `verb_noun` and registered here, but they have **no oRPC contract and no `operationId`**. A local-exec op MAY carry a top-level `fluncle` CLI verb when it is the operator's daily driver (`fluncle helm` launches the mission-control daemon; the command spawns local processes and never talks to the Worker) — the `admin` group stays reserved for authenticated Worker authority. Their primary surface is a package script or launcher; this table is their registry.

| Canonical op | Surface (the canonical invocation)                                      | Notes                                                                                                                                                                                                                                 |
| ------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `run_helm`   | `fluncle helm` (launcher; `install`/`uninstall` manage the LaunchAgent) | Fluncle's Helm: starts-or-focuses the local mission-control daemon (`apps/helm`, `:4190`) and opens its app window. Local orchestration with a CLI verb per the amendment above. Runbook: [docs/helm.md](./helm.md).                  |
...
`run` is deliberately outside the closed CRUD/action verb set: that set was drawn for HTTP resource operations, and none of its members names "launch a live show and hold it up until the operator stands it down." The live-longform RFC ratified `run_show` for exactly this local-exec category; new local orchestrations follow the same shape (`verb_noun`, registered in this table, surfaced as a package script or — for a daily-driver launcher like `fluncle helm` — a top-level CLI verb). The glass page and its phone remote are LAN-local only, so — per the RFC — neither is a public `@fluncle/registry` surface; nothing is added to `packages/registry`.
