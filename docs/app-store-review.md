...
The predicted risk landed where this doc said it would: 1.0 was **rejected under Guideline 5.2.3** (music rights) on the first pass, remediated, and resubmitted — see the 5.2.3 section for the fix and the invariant that holds it. That is the shape to expect from Apple: one rejection round on the music question, not a refusal of the app. Music apps that play previews and link out already exist on the store, and Fluncle's native vertical-video feed is genuine native value (not a web wrapper). Google Play review is far more lenient — approval there is near-certain; the friction worth planning around is Apple's.
