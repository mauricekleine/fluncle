...
Every completed `execute` and `batch` span keeps `op=db.query` and carries the same bounded performance vocabulary used by the fleet run ledger: `fluncle.operation_id`, `fluncle.access_class`, `fluncle.release`, `fluncle.attempt_count`, `fluncle.batch_count`, `fluncle.duration_ms`, and the closed `fluncle.outcome` verdict. Numeric `fluncle.queue_wait_ms`, `fluncle.aggregate_in_flight_max`, and `fluncle.request_in_flight_max` measure isolate admission and the real fan-out inside the current request without adding another identifier. A caller may attach a validated stable operation ID and may elevate a proven read to `heavy-read`; otherwise the wrapper derives a deterministic bounded fallback from a literal-redacted statement shape. The SQL text is never a span name or description. `db.statement` contains only the access verb and operation ID, and no SQL argument, interpolated literal, URL, hostname, secret, or row identity enters an attribute.
...
The Sentry issue-list request omits `statsPeriod`; that endpoint accepts only an empty value, `24h`, or `14d`, and rejects other periods with HTTP 400. Cursor pagination follows the `Link` header's `rel="next"` entry only while `results="true"`, within a fixed page bound. Reconciliation considers only recently merged fixes, so a regressed issue that Sentry reopens can return to the worklist. Every run summary derives `ok` from its error count; a failed project fetch cannot emit a healthy result.
