...
## 13a. RATIFIED (operator, 2026-07-11) — do not re-open

- **[D1] Audio source: FULL AUDIO, never previews.** A 30s preview is often all intro, so its vector describes the intro rather than the track — half the catalogue would carry garbage, and a poisoned embedding space destroys the one asset everything rests on. If a track's full audio is genuinely unobtainable it gets **no embedding**, never a preview-grade one. The legal posture is settled (privately gated, analysis-only, never shared; acquisition in the private companion repo) and is not an agent's to re-argue.
- **[D3] Target: ~10k, and let the find-rate decide.** NOT 100k — Discogs holds only ~41k _deduped_ DnB masters, so 100k is larger than the recorded genre. Build the neighbourhood of the labels already in the archive plus their artists' discographies; scale up only if the data says it is paying.
- **[D4] The tier gets NO PUBLIC NAME.** Stronger than the draft's "the catalogue": the second tier is **invisible as a concept** on public surfaces. It is never labelled, never introduced, never given a noun the crew could learn. Uncertified tracks simply appear (in search, in `/mix`, in an artist's "other songs") with a **visual** distinction and no category label. **"Finding" remains the only named object in Fluncle's world.** `catalogue` stays the INTERNAL term (code, docs, `/admin`); it must never surface in public copy.
- **[D5] Both canon amendments ratified.** LORE gains **the unvisited sky**: findings are stars Fluncle has _been to_; catalogue tracks are lights his instruments measured from a distance. The rule that falls out — **"Fluncle may narrate his own absence. He may never characterise the track."** — enforces the no-narrative-voice ruling from _inside_ the fiction rather than bolting a disclaimer onto it. DESIGN gains **the Unlit Rule** (a visual register for an unvisited row that cannot be mistaken for a finding; it must not collide with the Eclipse-Gold focus ring).
- **[D6] Legal posture confirmed.** Acquisition stays private-repo / box-only; this public RFC's silence about it is the intended boundary.
- **[D7] Seed labels are an OPERATOR-MANAGED ENTITY, not a hardcoded list** (operator, 2026-07-11 — a better design than the draft's "tick a list once", which rots the moment a finding lands on an unruled label).

  **The design:** labels become a **first-class entity** — the same `labels` table that powers the public label pages — carrying a **seed state**: `enabled` / `disabled` / `undecided`.
  - Every label appearing on a finding or a catalogue track gets a row automatically. A brand-new one enters as **`undecided`** — never silently crawled, never silently dropped.
  - An `undecided` label is an **attention-queue source**: adding a banger on an unseen label surfaces a cockpit row ("a new label to rule on"), decided with one keystroke. The existing queue pattern; no new operator surface to learn.
  - `/admin/labels` is the management view (enable / disable / review), alongside the artist queue.

  **The semantics are strictly crawl-SCOPE, never storage** (the operator's words): disabling a label removes it from the **next** crawl's seeds and **touches nothing already stored** — no deletion, no hiding, no retroactive effect. "What we crawl from" and "what we keep" are separate concepts and must stay that way in the code.

  **The starting ruling:** ~8 of the 39 labels in the archive arrived via crossover remixes and are not DnB — Anjunabeats, Armada Music, Axtone Records, Positiva, Tomorrowland Music / Experts Only, Atlantic Records UK, Counter Records, Zerothree. These seed as `disabled`. Three need the operator's call at build time (single-track imprints and one channel-brand): **Chelou**, **spiration music**, and **UKF** (a channel/brand rather than a label proper — seeding from it would cast a very wide net). Everything else seeds as `enabled`.

**Also corrected against production (2026-07-11):** the draft's headline ("the engine is dead, 3/56 embedded") was read off a stale code comment and is FALSE — all 60 findings are embedded, the sonic term is live, and MuQ was validated by the k=4 galaxy fit. And the draft's product premise ("Maurice can only find ~60 bangers a year") was wrong by 13× — the 60 are from four weeks.
