...
- **The retired `/admin/tag` queue** was the proven per-item operator loop — a worklist narrowed to the not-yet-done backlog, one keyboard-driven dialog per item, optimistic save (the tool itself is retired per `docs/rfcs/audio-embedding-rfc.md`; git history has `docs/admin-tagging.md`) — the pattern the follow queue reuses.
