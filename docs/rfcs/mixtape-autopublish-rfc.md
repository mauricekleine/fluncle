...
1. **Initiate (Worker).** `POST /api/admin/mixtapes/:id/youtube/initiate`: the Worker refreshes its stored token, then `POST https://www.googleapis.com/upload/youtube/v3/videos?uploadType=resumable&part=snippet,status` with the metadata (`snippet`: title ≤100 chars, description = note + chapters §2.3, `tags`, `categoryId:"10"` Music; `status`: `privacyStatus:"unlisted"`, `selfDeclaredMadeForKids:false`) and `X-Upload-Content-Type/-Length`. Returns to the CLI **both** the `Location:` session URI **and a short-lived (~1 h) access token**.
...
**Token:** OAuth2 authorization-code, browser-initiated once (`/oauth/authorize` → `/oauth/access_token?…&code=…` → `{access_token}`). Sent as **`?access_token=` query param, not Bearer** (Mixcloud diverges; note it). The docs only guarantee a _revocation_ path — **do not claim "never expires"**; provision once, and handle the `"An invalid access token was provided"` error by re-authing. Provision: `mixcloud auth` CLI helper prints the authorize URL; the resulting token goes in the CLI's local env (and `apps/web/.dev.vars` only if a Worker-side read is ever needed — it isn't for upload).
...
| `description`                            | dream note (≤1000 chars)                                                                                                                                                                    |
...
1. **YouTube channel verification — do it now (hard precondition).** Unverified → a >15 min mix fails at insert, not just the thumbnail. Phone-verify `@fluncle` before Phase 1.
2. **YouTube access-token handoff to the CLI is accepted.** The data PUT needs a Bearer token; the Worker mints a short-lived (~1 h) one and hands it over (refresh token stays server-side). Confirm this bounded exposure is acceptable (it's unavoidable for a client-side upload of a Worker-un-proxyable file).
3. **Mixcloud token lives in the CLI env** (operator's own credential), upload is CLI-direct at full quality. Confirm (the draft's Worker-proxy + 192 kbps down-encode is rejected by two reviewers).
4. **Thumbnail/picture sizes:** confirm `size=wide` ≤2 MB for the YouTube thumbnail and `size=square` ≤10 MB for the Mixcloud picture; add a `size=thumb` if either PNG is over.
5. **Audio bucket/key:** the audio master does **not** need R2 at all under CLI-direct Mixcloud (the CLI uploads from local). Decide if you still want an R2 archive copy at `<logId>/master.m4a` (nice-to-have, not required by the flow).

## Acceptance criteria

- **Phase 0:** `mixtape_social_posts` migration applies; `publishMixtape` mints to `distributing` (unit test: the cover endpoint renders a `distributing` mixtape; public `/api/mixtapes` and `/log` exclude it); a simulated total-upload-failure leaves the mixtape `distributing` with its Log ID held and **no public exposure**; first successful finalize flips to `published`; the chapter helper passes its unit tests; the cap pre-check rejects at 55.
- **Phase 1:** `fluncle admin auth youtube` stores a refresh token; `distribute --video` resumably uploads a multi-GB file (with the Bearer token, surviving a mid-upload `401` via re-mint) and the video appears **unlisted** with the wide thumbnail, the title, and ≥3 description chapters; `publish-youtube` flips it public; `mixtape_social_posts(youtube)` + `mixtapes.youtube_url` written; `/log` shows the link.
- **Phase 2:** `MIXCLOUD_ACCESS_TOKEN` provisioned CLI-side; `distribute --mixcloud` uploads the full-quality master with `sections[]` + the square cover, reads back the key, records + dual-writes `mixtapes.mixcloud_url`; published listed.
