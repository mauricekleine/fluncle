...
**PR-2 — add the internal read tools to the MCP + WebMCP:** `search_archive`, `get_artist`, `get_label`, `build_set`. This finally gives the MCP a real **archive** search (its `search_tracks` searches Spotify, not Fluncle). WebMCP's `search_archive` calls the existing **public** `GET /api/v1/search/archive` HTTP endpoint (confirmed public + live — Appendix); `get_artist`/`get_label` call their public `/api/v1/artists|labels/:slug` routes.
