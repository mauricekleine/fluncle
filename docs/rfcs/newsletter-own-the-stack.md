...
**Sequencing.** The oRPC migration this builds on has **shipped** (contracts in `packages/contracts/src/orpc`). The newsletter API is built **on the oRPC contracts** with Convention B `verb_noun` names — so the editions reads, the admin create/send, and the subscribe-move are all new oRPC contracts in `packages/contracts/orpc`, generating their own OpenAPI operations. The newsletter is the first _new feature_ to exercise Convention B end to end (§3).
...
| API       | `/api/v1/mixtapes`, mixtape-typed in `/api/tracks/:id` | **oRPC contracts (Convention B):** `list_editions` (`GET /newsletter/editions`), `get_edition` (`GET /newsletter/editions/{number}`) — §3                  |
...
This is the first new feature built on the ratified Convention B (`docs/naming-conventions.md`) and the oRPC contract rails (`packages/contracts/src/orpc`). Every endpoint is a `verb_noun` canonical op with a fixed projection to each surface. The naming-conventions checklist (its §6) is applied literally.

### 3.1 One root: everything under `/newsletter`

**Decision (locked): the newsletter is a singleton, and everything it owns nests under `/newsletter`.** A bare `/editions` collection was **rejected** — too generic, and it collides conceptually with "mixtape edition." So the editions collection is a **plural sub-resource nested under the `/newsletter` singleton**: reads are `GET /newsletter/editions` and `GET /newsletter/editions/{number}`, admin writes are under `/admin/newsletter/editions`. This is exactly the convention's rule — a singleton stays singular (`/newsletter`) with its collection a plural sub-collection nested under it (`docs/naming-conventions.md` §3, the Convention B API-path rule). One feature, one root: the human page (`/newsletter`, `/newsletter/<id>`) and the API objects (`/newsletter/editions/*`) share it.

- The subscribe POST moves too, for the same one-root reason. Today `subscribeToNewsletter()` is served at `POST /api/newsletter` + `POST /api/v1/newsletter` (operationId `subscribeNewsletter`); subscribe is a non-CRUD action on the newsletter, so per Convention B it becomes a named action sub-resource — **`POST /newsletter/subscribe`** (`POST /{resource}/{action}`, single-word action). The canonical op stays `subscribe_newsletter`; only the path moves.
...
| ---------------------- | ------------------------------------------------------------ | --------------------- | ---------------------- | --------------------------------------------------- |
| `list_editions`        | `GET /newsletter/editions` (sent-only)                       | `listEditions`        | `list_editions` (opt.) | `fluncle newsletter` (root, voice alias)            |
| `get_edition`          | `GET /newsletter/editions/{number}`                          | `getEdition`          | `get_edition` (opt.)   | (resolved via `newsletter`)                         |
...
| `create_edition`       | `POST /admin/newsletter/editions` (draft)                    | `createEdition`       | — (admin, not public)  | `admin newsletter draft`                            |
| `update_edition`       | `PATCH /admin/newsletter/editions/{id}` (edit draft)         | `updateEdition`       | — (admin)              | `admin newsletter update`                           |
| `send_edition`         | `POST /admin/newsletter/editions/{id}/send` (broadcast+mint) | `sendEdition`         | — (admin)              | `admin newsletter send`                             |
...
- **Plural collection nested under the singleton** (`/newsletter/editions`), **singular machine noun** in the op (`edition`) — the `/newsletter` singleton stays singular while its editions are a plural sub-collection under it, per the convention's plural-collection-under-singleton + pluralization rules (`docs/naming-conventions.md` §3; collections plural, op nouns singular). A bare `/editions` is rejected (too generic; mixtape-edition collision).
- **The send is a named action sub-resource** (`POST /admin/newsletter/editions/{id}/send`), single-word action `send`, no dash-compound — matching the mixtape `…/publish` shape and the convention's "no multi-word path segments" rule.
- **Public reads are `/api/v1/newsletter/editions`** (canonical) with the `/api/newsletter/editions` alias via `-alias.ts`. Admin reads/writes (`/admin/newsletter/editions*`) stay **off the public OpenAPI spec**, cookie-or-bearer gated (`requireOperator`/`requireAdmin`), exactly like the mixtape admin routes — admin is intentionally not oRPC-contract-public per the oRPC brief, but the public `list_editions` / `get_edition` / `subscribe_newsletter` ops _are_ contracts.
...
- `POST /admin/newsletter/editions` → `createEdition(payload)` (`requireOperator`/`requireAdmin`) — mirrors `POST /admin/mixtapes`. Creates a **draft** (no number yet).
- `PATCH /admin/newsletter/editions/{id}` → `updateEdition` — edit the payload/subject before send.
- `POST /admin/newsletter/editions/{id}/send` → `sendEdition`: render the email HTML from `contentJson`, create the Resend broadcast, send it (per the §4.2 guardrail), and on success **mint the sequential `number`** (`max(number)+1`, atomic), set `status: "sent"`, `sentAt`, `sendProvider: "resend"`, `sendExternalId: <broadcast id>`, `addedAt`. The mint-on-send keeps numbering honest (a drafted-but-never-sent edition never gets a number).
- Public reads: `GET /newsletter/editions` (`list_editions`, sent-only) + `GET /newsletter/editions/{number}` (`get_edition`) — oRPC contracts under `/api/v1`, `-alias.ts` mirror at `/api`.

**Secret posture (unchanged):** the agent box holds only its admin token; `RESEND_API_KEY` (+ the audience id) is a **Worker secret**, declared in `env.ts` alongside the existing `LOOPS_*` secrets (which are removed once the send is cut over). The broadcast create/send happens **Worker-side** (the agent calls `POST /admin/newsletter/editions/{id}/send`; the Worker holds the Resend key) — same model the rest of the pipeline uses, so no vendor key lands on the agent box.
...
**Gate: the oRPC migration this builds on has shipped.** The newsletter API is built on the oRPC contracts (`packages/contracts/src/orpc`), so the rails it needs are in place.
...
- [ ] `/newsletter` index lists sent editions newest-first (modeled on `mixtapes.index.tsx`); `GET /api/v1/newsletter/editions` (+ `/api/newsletter/editions` alias) returns sent editions as JSON via an **oRPC contract** (`list_editions` / `listEditions`); `GET /newsletter/editions/{number}` (`get_edition`) returns one. **A coverage test confirms each public route has a contract/operationId** (the oRPC-brief rule).
...
- **Convention B + oRPC rails (the API basis):** `docs/naming-conventions.md` Convention B (verb_noun registry; MCP = `verb_noun`, operationId = `verbNoun`, REST resource path, named-action sub-resource for non-CRUD, plural collections) — **ratified** (referenced as such in `AGENTS.md`); the shipped oRPC contract pattern (`packages/contracts/src/orpc`, `OpenAPIGenerator`, the coverage tests that fail on an un-contracted public route — `apps/web/src/lib/server/orpc-coverage.test.ts`). The newsletter contracts (`list_editions`, `get_edition`, `subscribe_newsletter`, + the admin ops) follow these rules and are the convention's first new-feature test.
