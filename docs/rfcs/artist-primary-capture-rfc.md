...
- **Slice 3 (later) — catalogue cleanup pass:** retire tracks/artists that should never have entered (operator-scoped). The first concrete definition, from the 2026-07-21 Alice Ivy audit: rows whose ONLY archive link runs through a MIS-IDENTIFIED or disabled label and who carry no qualified affinity (the type specimen: 11 Alice Ivy tracks via a `helix-records` row whose MBID resolved to the wrong same-named label — enabled-label trust bypasses the artist gate by design, so label identity is the load-bearing check).
- **Slice 4 (new, small) — ruling-time identity:** `/admin/labels` shows each row's MusicBrainz entity (name + disambiguation + founding) beside the seed-ruling buttons, so a name-collision label ("which Helix?") is answered at ruling time rather than at audit time. Companion: a periodic enabled-seed identity audit (the 2026-07-21 sweep's shape) until it ships.
