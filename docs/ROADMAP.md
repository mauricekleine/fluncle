...
### Fluncle Studio — clipping core LIVE, distribution deferred
...
- **Distribution layer (the deferred next slice).** Push clips to socials from the library's disabled "Distribute" seam (Postiz / direct), adding the deferred sibling `mixtape_clip_social_posts` table (designed in the RFC, not yet built — only a deferred comment in `schema.ts` today). Inherits the same constraints as the per-finding tail — no legitimate API audio path, the in-app post is the irreducible manual beat (the **Instagram music-licensing** exposure under _TikTok auto-pipeline → More platforms_ applies; a DJ set carries the tracks' copyright). Download-and-hand-post is v1.
- **Per-clip captions.** The Fluncle-voice caption per clip (`copywriting-fluncle`) — auto-drafted or operator-set; "No caption yet" is the current state.
