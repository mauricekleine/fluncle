...
### Secrets in 1Password — populate the local-dev item

The repo side is wired: `apps/web/.dev.vars.tpl` is a committed `op://$FLUNCLE_1PASSWORD_ENV_ITEM/<field>` template, `bun run --cwd apps/web db:secrets` renders the gitignored `apps/web/.dev.vars` with `op inject --account "$FLUNCLE_1PASSWORD_ACCOUNT"`, and Superset setup renders a fresh file per worktree before `db:refresh-dev` rewrites the local `TURSO_DATABASE_URL` pair. Production Turso credentials already live separately in the `Turso Production Credentials` item and are read at run time by `db:pull-prod`. Remaining operator step: keep `FLUNCLE_1PASSWORD_ACCOUNT` and `FLUNCLE_1PASSWORD_ENV_ITEM` in the local shell so the public repo does not expose vault/item names. The mechanics are in [docs/local-database.md](./local-database.md).
