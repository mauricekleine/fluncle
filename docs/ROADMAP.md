...
- **TikTok draft** — `fluncle admin track draft` pushes the silent cut + caption to the TikTok inbox via Postiz (`SELF_ONLY`/`UPLOAD`); status tracked in `social_posts` via `track social`. _Built_ — see the capstone section + the `fluncle-publish` skill.
- **Still open:** the render-quality regressions above, and **surfacing the video on the web** (it degrades gracefully today). The Spinup agent stays off for now; the local `fluncle-video` → `track video` → `track draft` loop runs by hand.
