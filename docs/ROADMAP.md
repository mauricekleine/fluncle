...
- **`fluncle-healthcheck`** (`--no-agent`, `every 10m`) — probes each service, Discord-pings only on a status flip, and POSTs the snapshot to `record_health`, feeding the public `/status` board. Zero LLM tokens. Source: `scripts/fluncle-healthcheck.{sh,ts}`.
...
- **Brand-frame polish.** v1 bakes the title + the `fluncle://<logId>` coordinate over a scrim via ffmpeg `drawtext` (the cut filter emits each line once; mixtape #1 only appears to repeat the coordinate because its title field happens to contain `| 019.F.1A` — a one-off data quirk, not a frame bug). The richer Remotion-cosmos brand frame is the documented Phase-2 ("start simple, fancy later").
- **Per-track cue labels.** The frame stamps the mixtape level today; labeling each clip with the track playing at its window is the cue-marking synergy the RFC scoped (the editor doubles as the tracklist cue tool), gated on marking cues.
