...
### Tone of voice — narrator lane decided (resolved 2026-06-19)

> **Resolved this round.** Picked the **split-by-surface** lane — machine-facing entity/SEO strings stay honestly-plain active third-person; human surfaces are first-person Fluncle, turned to the crew. Codified as the **said-not-written stack** (Reality → active voice → Oof Test → Selector's Rule → the Sauce) + the Narrator mechanic in `packages/skills/copywriting-fluncle/references/voice.md`, threaded into the skill's checks, and swept across every surface (`digs → discovers`, the About rewrites, prose em-dash fixes). The analysis below is the reasoning record; nothing here is open.
