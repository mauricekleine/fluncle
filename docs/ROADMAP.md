...
### Media-loading stall — root-caused, fix pending device verification

A live defect on **two surfaces**: a video sometimes hangs on "loading" and never starts — seen on both `radio.fluncle.com` and the Stories player. **Root-caused 2026-06-24** (`docs/scratch/media-stall-diagnosis.md`): the shared recovery watchdog (`apps/web/src/lib/use-video-recovery.ts`) latches `recovered` after one wedge and re-arms ONLY on `loadstart`/`emptied`. Recoveries that call `video.load()` self-rearm, but radio's **second** wedge calls `resolveSlot()`, which commonly resolves to the **same** `videoUrl` (schedule unchanged) — so React never reassigns `<video src>`, no `emptied`/`loadstart` fires, the latch stays dead, and a later stall on that clip has no recovery path → frozen poster while the observation audio runs on. Clock-independent, surface-shared, rare. **Fix shape** (pending a real-device repro to confirm — that's why it's not yet merged): re-arm the latch on a healthy-verdict transition or a bounded window (`now − recoveredAt ≥ STALL_TIMEOUT_MS`) rather than `loadstart`, capped by a `MAX_RECOVERY_ATTEMPTS` so a dead source can't tight-loop; the `mediaStallVerdict` core stays unchanged (its tests pass). Device test: a phone on flaky cellular (or DevTools Slow-4G + offline toggling) forcing two cold-misses on one finding.
...
**Not yet surfaced — and the prompt to declutter the homepage (idea, 2026-06-24).** Radio is live but reachable only by knowing the subdomain; it should be **surfaced on the homepage + the other public surfaces** as a real "tune in" affordance, not buried. But the homepage link menu has grown convoluted — the Galaxy hero CTA, Playlist + Newsletter, Submit a track, the eight-icon Follow row, About · Logs · Mixtapes · Docs, Join the Crew, and the CLI · DIG · GIT · MCP · SSH developer row — many competing CTAs with no hierarchy. So adding radio is really a **homepage-navigation rationalization pass**: establish a hierarchy (primary listen/play actions vs. follow vs. read vs. developer surfaces), give radio a first-class home in it, and thin the noise — not bolt another button onto the pile. Do the menu rethink first, then radio lands cleanly; the rationalized nav likely fans out to `/log`, `/about`, and the footer too.
