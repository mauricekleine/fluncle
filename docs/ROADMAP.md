...
### Fluncle's own mixtapes — shipped; a few follow-ups

The mixtape spine is **live** (PRs #18 plumbing, #20 editable-after-publish + cover scaffold, #21 row/CLI/icon polish). A mixtape is a spine-native object — Fluncle dreaming, a checkpoint — with its own `F`-marked Log ID, a `/log/<id>` compilation page, a `/mixtapes` front door, quiet inclusion in the feed / `recent` / MCP (without inflating `Found · N`), `DJMixAlbum` JSON-LD, an RSS `<category>`, sitemap entries, and an llms.txt Mixtapes section. The design + per-mixtape runbook is **[docs/fluncle-mixtapes-runbook.md](./fluncle-mixtapes-runbook.md)**. The first mixtape, **`019.F.1A`** ("Fluncle Drum & Bass Mixtape #1", 17 findings, 72 min liquid), is published and fanned out across the spine; Mixcloud / YouTube / SoundCloud are live with branded covers (rendered from `packages/media`, hosted on R2 at `found.fluncle.com/019.F.1A/`).

Open follow-ups:

- **B-increment regression tests (highest value).** The feed-union behaviour is correct live but unguarded — no test pages past a mixtape without skip/dup, and none asserts `Found · N` stays findings-only. A future touch to `listTracks`/the union could silently reintroduce the skip/dup or the count inflation the review fought; add both guards.
- **SSH rave-terminal mixtapes view.** `019.F.1A` reaches every surface except the deepest room — `apps/ssh` (Go) has no mixtape awareness yet. The runbook lists a mixtapes view in the terminal; a fast-follow.
- **Dedicated OG card.** The mixtape `/log` `og:image` uses the **square** `coverImageUrl` (1:1); the proper 1200×630 `og.png` is already rendered and on R2 (`found.fluncle.com/019.F.1A/og.png`) but unused. A one-line `og:image` swap gives a `summary_large_image`-shaped card.
- **Voice + small polish.** Route the remaining new strings (`/mixtapes` empty state, the `log-decode` F-marker line, admin labels) through `copywriting-fluncle`; surface `publishedAt` on the `MixtapeDTO` (cosmetic — the column is set).
- **MusicBrainz DJ-mix release — done** ([`fc818504`](https://musicbrainz.org/release/fc818504-6c01-4565-be1e-d1b3657f8a7c), 2026-06-19): Album + DJ-mix under Fluncle, all 17 tracks credited (a new **System & Wise** artist created for track 14, with a sourced edit note), and the three hosting links as stream-for-free relationships. Makes the MusicBrainz artist substantial — the "is this a real artist" closer, no AI original needed. Remaining off-site, low-priority: enrich Wikidata `Q140169844` as facts accumulate (the new release + the SoundCloud anchors; tracked in the off-site thread + the SoundCloud item) — marginal for a single release.
