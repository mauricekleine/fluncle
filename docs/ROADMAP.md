...
- **Distribution layer (the deferred next slice).** Push clips to socials from the library's disabled "Distribute" seam (Postiz / direct), reusing the `mixtape_clip_social_posts` table built for it. Inherits the same constraints as the per-finding tail — no legitimate API audio path, the in-app post is the irreducible manual beat (the **Instagram music-licensing** exposure under _TikTok auto-pipeline → More platforms_ applies; a DJ set carries the tracks' copyright). Download-and-hand-post is v1.
...
- **Brand-frame polish.** v1 bakes the title + the `fluncle://<logId>` coordinate over a scrim via ffmpeg `drawtext`; the coordinate currently reads twice (the title line + the `fluncle://` line) — tidy to one. The richer Remotion-cosmos brand frame is the documented Phase-2 ("start simple, fancy later").
