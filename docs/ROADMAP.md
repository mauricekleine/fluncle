...
- **Expose the empty-context retry path.** The `context_status` marker (`pending` / `resolved` / `empty` / `failed`) shipped (#129, `schema.ts contextStatus`, migration 0032): a confirmed-empty Firecrawl fetch is now distinct from never-attempted, the status-aware queue skips `empty`/`resolved`, and the server-side `retryEmptyContext` flag (`tracks.ts`, threaded through the `admin-tracks` oRPC contract + tests) widens the net. What's left is making the widen pass **operationally reachable**: expose `--retry-empty` on the `fluncle admin tracks context` CLI command and thread it through the on-box context sweep, so a rare "facts may have appeared upstream" pass can actually be triggered (today the server flag exists but no CLI/cron path fires it).
...
Radio is now surfaced on the homepage too — the nav reorder (`65628c3`) gives it a first-class "listen" home (the Playlist + Radio pair), a "For the nerds" dev-surface group, a masthead Join-the-Crew, and a live status pill, so the menu no longer buries it.
