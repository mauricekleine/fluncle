...
- **Deterministic per-finding enrichment (off the agent)** — the per-finding pipeline is `--no-agent` Hermes-box sweeps, not a Sonnet agent: `enrich` (BPM/key/spectral vector), `context-note` (Firecrawl facts → distilled prose + a `Texture:` line, now uniform across the archive), `note` (the auto-authored editorial "why", voice-gated, fill-empty-only — #141), `observation` (the recovered-audio script + the bespoke-voice render), and `backfill` (the Discogs/Last.fm catalogue sweeps). Moving the whole pipeline off the agent saves ~$20/day, and **no agent crons remain** — every per-finding and weekly cron runs `--no-agent`. All of them run live on the box today — `enrich` + `context-note` every 5 min, `note` every 10 min, `observation` hourly, `backfill` every 30 min (see _Hermes automation_).
- **Hands-off rendering** — the `fluncle-render` box cron (hourly) conducts a render of the oldest queued finding on the scale-to-zero rave-03 box.ascii box, ships it to R2, then parks the box (one render at a time — the conductor chains immediately to the next queued finding on render-done, then parks when the queue empties). Renders are now music-reactive — the signal-chain dials + the global-vs-internal motion law + the author-time composition lint shipped (see _Brand & canon → Video aliveness_).
...
- [x] **Render (Hermes box cron)** — the hourly `fluncle-render` conductor wakes the scale-to-zero rave-03 box.ascii box, renders one queued finding at a time (single-flight), chains straight to the next on render-done, then parks the box when the queue is empty. See _Now → Hands-off rendering_.
...
- [ ] **Newsletter (Friday)** — the Hermes Friday `--no-agent` sweep (one bounded `claude -p` authoring call) drafts the weekly edition and persists it (one flat finds block, scene tidbits via Firecrawl), but the **send is an operator tap**: the sweep persists the draft + posts a one-line Discord summary and the literal `fluncle admin newsletter send <id>` command (operator-tier `send_edition`); the operator runs it. The one weekly-cadence step; everything above it is per-finding. See _Later → Newsletter — open follow-ups_.

**4 · Deferred (on purpose)** — a surface we could reach but are choosing to leave dark for now:

- **Instagram** (`@fluncle`) — **per-finding masters stay deferred; set clips re-opened (2026-07-06).** For a single finding's master the music-licensing exposure still isn't worth it (no legitimate API audio path — the master gets muted on a business/creator account, IG's licensed audio is app-only). But a live-mixed DJ-**set** clip fingerprints differently and survives (the spike clip held 3 days), so the Fluncle Studio **clip drip-feed** now posts set clips to IG via Postiz on a jittered daily cadence with a kill switch (see _Now → Fluncle Studio_). The per-finding-master path remains parked, not closed. See _Later → TikTok auto-pipeline → More platforms_.
...
The Hermes box is the queue-driven runner for the per-finding pipeline, and the cutover is **complete** — every cron runs `--no-agent` on the box; **no agent jobs remain** (the Friday newsletter was the last, converted to a hybrid sweep on 2026-06-27). The source of truth is the cron records under `docs/agents/hermes/cron/` (`jobs.json` is now `"jobs": []` — kept only as the mechanism record) plus the sweep sources in the sibling `docs/agents/hermes/scripts/`; the operator manages them on the devbox via the **fluncle-hermes-operator** skill (`hermes cron create/edit …`). Operating doc + roles: [docs/agents/hermes-agent.md](./agents/hermes-agent.md).

The crons running on the box:

- **`fluncle-enrich`** (`--no-agent`, `every 5m`) — drains the enrich worklist (`admin tracks enrich --queue`), analyzes each finding on-box (`ffmpeg` + `bun` DSP), and writes BPM/key/spectral vector back via `admin tracks update`. Zero LLM tokens. Source: `scripts/enrich-sweep.{sh,ts}`.
...
- **`fluncle-healthcheck`** (**host systemd timer**, `every 10m` — NOT a Hermes gateway cron; moved off the gateway so the busy runner it monitors can't starve the prober) — probes each service, Discord-pings only on a status flip, and POSTs the snapshot to `record_health`, feeding the public `/status` board. Zero LLM tokens. Source: `scripts/fluncle-healthcheck.{sh,ts}`.
- **`fluncle-studio-clip`** (`--no-agent`, `every 15m`, #215) — drains the pending Fluncle Studio clip queue, cuts a mixtape set into framed 9:16 clips via ffmpeg, and ships each `<clipId>/footage.mp4` to R2 (a clip whose set video isn't staged yet is skipped, stays `pending`). Zero LLM tokens. Source: `scripts/clip-sweep.{sh,ts}`.
- **`fluncle-newsletter`** (hybrid `--no-agent`, `0 15 * * 5`, Europe/Amsterdam box clock) — authors (one bounded `claude -p` call) + persists the Friday edition, then posts a one-line Discord summary + the `fluncle admin newsletter send <id>` command (persist-then-offer; never auto-sends — the operator runs the send).
...
- **Expose the empty-context retry path. (Resolved.)** The `context_status` marker (`pending` / `resolved` / `empty` / `failed`) shipped (#129, `schema.ts contextStatus`, migration 0032): a confirmed-empty Firecrawl fetch is distinct from never-attempted, the status-aware queue skips `empty`/`resolved`, and the server-side `retryEmptyContext` flag (`tracks.ts`, threaded through the `admin-tracks` oRPC contract + tests) widens the net. The widen pass is now **operationally reachable**: `--retry-empty` on `fluncle admin tracks context --queue` (threaded `contextQueueCommand` → `retryEmptyContext` query param, focused test in `admin-tracks.test.ts`) and the on-box context sweep (`scripts/context-sweep.{sh,ts}` — `RETRY_EMPTY=1` / a `--retry-empty` arg widens step 1; the routine 5m cron unchanged, a separate rarely-fired cron carries the flag). A rare "facts may have appeared upstream" pass can now actually be triggered.
...
Re-ship purge is **solved** (#152): the `?v=N` versioned `footage.mp4` source in `media.ts` evicts stale renditions instantly (Media Transformations caches in its own internal layer the purge API can't reach, so the vintage token — not the zone purge — is what forces the flip), and a re-render also purges the master — so replacing a clip at the same key no longer strands listeners on the old rendition.
...
The surface fan-out has **shipped** (PR #206): the `extension.lens` `@fluncle/registry` entry is live with no `pending` and the real store URL, so /status, the homepage dev-row, llms.txt, the sitemap, and the surfaces-doctrine doc all light up, and the "Get the Chrome extension" link is on `/about`. What's still open:
...
- **More platforms — YouTube done, the autopilot-ready channel; Instagram closed.** YouTube Shorts ships now (PR #13): a direct public upload (title + caption) via the per-platform push, recorded in `social_posts`. Because it needs **no manual finish** (unlike TikTok's in-app official sound), YouTube is the one channel that can run **fully on autopilot** — once the auto-advance above chains render → publish, YouTube publishes hands-off with nothing left for the human. (Flagged, not urgent.) Instagram splits by content: a **per-finding master stays deferred** (no legitimate API audio path — the master gets muted on a business/creator account, IG's licensed audio is app-only), but **set clips are a separate, now-built path** — the Fluncle Studio clip drip-feed posts them via Postiz (see _Now → Fluncle Studio_). Building the set-clip drip does not re-open the per-finding-master path; that stays parked (see the autonomy ladder's _Deferred_ group). Per-platform doctrine lives in `docs/track-lifecycle.md` (Phase 3) + the `fluncle-publish` skill.

### Fluncle Studio — clipping LIVE, IG drip-feed BUILT (live validation + cron deploy pending)

Turning one long set into many short posts is **built and live**, shipped end to end as Units A–G: `distribute --set-video` stages one 1080p set rendition to R2 (A), an `analyze-set.ts` drop-detection DSP suggests the windows (B), the `mixtape_clips` data layer + oRPC ops (D), the `/admin/studio/<logId>` editor — waveform + drop suggestions + draggable 9:16 framing, keyed by the Log ID coordinate (E), the `/log` set-video player (F, #208), and the cross-mixtape clip library `/admin/clips` (G). The cut is a deterministic ffmpeg job on the Hermes box: the `fluncle-studio-clip` `--no-agent` cron drains pending clips, trims + crops 9:16 + bakes the brand frame, ships `<clipId>/footage.mp4` to R2, and the library hands off **IG (with audio) / TikTok (audio-stripped, for the in-app sound)** downloads. First clip cut + verified live 2026-06-29. Doctrine: [docs/fluncle-studio.md](./fluncle-studio.md). (Build note: the box runs the standalone bun **binary** for this — the npm thin client can't; see _Hermes automation_-adjacent deploy.)

What's open:

- **Distribution layer — BUILT + DEPLOYED: the clip → Instagram drip-feed.** The once-disabled "Distribute" seam is now a shipped, Fluncle-owned scheduler (spec: [docs/clip-drip-feed-rfc.md](./clip-drip-feed-rfc.md)): the `mixtape_clip_social_posts` schedule + status table and the `settings` kill-switch KV (migration `0046`, live on prod), `pushInstagramReel` (a Postiz IG-Reel push), the agent-tier `drip_clips` tick op, auto-queue-on-clip-create with a **random 23–25h jitter** (drifting post times, anti-bot), and the `/admin/clips` **kill-switch toggle + per-clip schedule/override/unschedule** UI (PRs #321/#325, plus the #330 `deploy:gate` flake fix). The audio-survival spike cleared the old IG block **for set clips specifically** — a live-mixed DJ-**set** clip fingerprints differently from a single copyrighted master (the first hand-posted clip survived 3 days), so this is a distinct, validated path from the per-finding per-master IG deferral (which still holds). **Immediate next steps (operator-gated):**
...
- **Per-clip captions.** `buildClipCaption` ships the coordinate credit — the finding(s) a clip's window plays (resolved via `resolveClipTracks`), or the promoted mixtape's `.F.` — served to the card and used as the drip caption. Gap: a clip from an **un-cued** recording resolves to no finding, so it builds an **empty** caption (all 4 current clips, from the un-cued rolling set); the drip wants either finding-linked cues on the source recording or an operator-set caption. A Fluncle-voice auto-draft (`copywriting-fluncle`) on top stays open.
- **Brand frame — closed decision (no baked overlay).** The baked text frame (title + coordinate via ffmpeg `drawtext`) was dropped in #260 — recorded set footage reads poorly under it, and the credit rides the IG caption (`buildClipCaption`) instead. The clip ships as a clean crop. The Remotion-cosmos frame is deferred indefinitely, pending a reason to revisit.
...
The Friday newsletter ([docs/agents/newsletter-agent.md](./agents/newsletter-agent.md)) is authored + persisted by the `fluncle-newsletter` Hermes `--no-agent` sweep (see _Hermes automation_), with the editions model (#102) persisting each edition's content at draft time, the public `/newsletter` archive + `/newsletter/<n>` edition pages (#130, still galaxy-grouped via the `editions.ts orderedGalaxies` helper, #124), and the `/admin/newsletter` operator front-end (#130/#132) all live. (The letter copy itself dropped galaxy grouping in `d29b0c0` — the sweep now emits one flat finds block; only the archive still groups.) What's open:
...
- **Keep findings tagged.** The archive's galaxy grouping (Solar → Nebular → Lunar → Astral, the `editions.ts orderedGalaxies` helper) is only as full as the Tag step — its coverage depends on findings carrying a vibe placement, so it rides the _Vibe-placement model_ item rather than standing alone; an untagged finding falls to "Also found." (The letter copy no longer groups — `d29b0c0` dropped grouping from the sweep — so this is now an archive-only concern.)
...
- **Confirm the Firecrawl tidbits populate.** Confirm the Firecrawl scene-`tidbits` populate on a real tick — they came back empty on an early run (the email renders fine without them, but they're the extra scene color). (The old chain-of-thought-dump / `clarify` step-7 concern is retired with the agent loop — the sweep's Discord summary is deterministic now.)
...
- **Revisit at ~50–100 placements.** Once there's a meaningful set of placements, **start training a small model** `features → (vibe_x, vibe_y)` — k-nearest-neighbours over the feature vector or ridge regression is plenty at this scale (no infra needed) — and measure what accuracy is achievable with what we have. The early read tells us whether the audio features carry enough signal or whether the feature set needs widening.
