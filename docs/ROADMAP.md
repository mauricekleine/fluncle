...
**SSH version (the flex)** — scoped in [docs/galaxy-ssh-rfc.md](./galaxy-ssh-rfc.md). The same _sim_ inside `ssh rave.fluncle.com` via a third adapter: `apps/web/src/game/sim.ts` compiled to WASM (Javy/QuickJS → wazero, pure-Go) so web (V8) and SSH (QuickJS) run one literal source of truth, a top-down scope renderer, the read-the-log orbit card with an OSC-8 Spotify link, audio-less as flavor ("the audio didn't survive the trip out here — it's still playing back on Earth"), telemetry in the deepest Depth-Gradient register taken from the shipped strings ("Pulled under. Flung across the galaxy." · "Home, junglist."). Map knowledge is portable across surfaces — the Log ID spine paying off. Gated on two PASS/KILL spikes Maurice runs first (SSH input latency; the persisted-QuickJS engine model), which also closes the old "confirm realtime input + frame rate" question. The Minimum Lovable galaxy ships whole; QR / Kitty-input / ambient-crew are named fast-follow.
