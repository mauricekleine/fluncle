...
### Hermes chat agent — server-side publish boundary (done) + sweep scheduling

The Hermes chat agent is **live for the internal crew** — a self-hosted Nous Hermes gateway on a private Tailscale-only box, fronting Discord (allow-listed), acting on the archive through the `fluncle` CLI as the `agent` role (publish-/irreversible-class actions refused server-side), in Fluncle's voice (`SOUL.md` + the `copywriting-fluncle` skill). Operating doc + runbook: [docs/agents/hermes-agent.md](./agents/hermes-agent.md); build context in `docs/agents/hermes/`.

- **Publish boundary moved server-side — DONE.** The box used to hold the full admin token, so the command gate was the only thing limiting it (bypassable via `printenv` + raw `curl` or `fluncle-real`). Now there are two admin **roles**: the box holds an `agent`-scoped token (`FLUNCLE_AGENT_TOKEN`) and the Worker refuses every publish-/irreversible-class route for that role (`requireOperator`); the operator's full token + browser session stay `operator`. A fully-compromised root agent can no longer publish — the credential lacks the authority. This also subsumes the earlier "server-side human-confirm" idea (publish is operator-only by construction). See `apps/web/src/lib/server/env.ts` + the [Roles](./agents/hermes-agent.md#roles-operator-vs-agent) table. **Operator step to activate:** generate + `wrangler secret put FLUNCLE_AGENT_TOKEN`, then point the box's `/etc/hermes.env` `FLUNCLE_API_TOKEN` at the agent value and restart.
- **Optional defense-in-depth (no longer a public-readiness blocker).** Running the agent as a **non-root** user with the token out of its readable env now only protects the agent surface + the token itself, not the publish boundary. Nice-to-have before broadening, not required. (PRODUCT.md tension stands: a public, chatty conversational bot fights "quiet / operator-controlled" — the public-Discord question stays deferred behind a demonstrated need, not behind this hardening.)
