...
`apps/web/.dev.vars` is a growing plaintext secrets file (Spotify, Telegram, Postiz, Loops, Spinup, Better Auth, R2, …). The arc is to keep no plaintext secrets in the repo or on disk as the source of truth: commit a `apps/web/.dev.vars.tpl` of `op://Fluncle/<item>/<field>` references (zero secret values, safe to commit), render the real file with `op inject -i apps/web/.dev.vars.tpl -o apps/web/.dev.vars` behind a `db:secrets` script, and wire that into Superset `setup` in place of copying the main checkout's plaintext `.dev.vars` — so each worktree renders its own secrets and the dynamic local `TURSO_DATABASE_URL` pair stays owned by `db:refresh-dev` (layered on after injection; those are a localhost URL + dummy token, not secrets). Use `op inject` (renders a file), not `op run` (env-only) — every consumer here reads `.dev.vars` as a file (wrangler / the Cloudflare vite plugin, `env.ts`'s dotenv, `drizzle.config.ts`). The first step already landed with the local-database work: production Turso credentials moved out of `.dev.vars` into the `Turso Production Credentials` 1Password item, read at run time by `db:pull-prod`. Open question to settle before doing the rest: auth for non-interactive worktree setup — the desktop-app integration prompts biometric (fine interactively), but fully hands-off provisioning needs an `OP_SERVICE_ACCOUNT_TOKEN`, which is itself one secret to manage outside 1Password. Devex/hygiene, not user-facing; do it when the plaintext file becomes a real liability. The mechanics are in [docs/local-database.md](./local-database.md).
