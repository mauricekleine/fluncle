...
### radio.fluncle.com — the station (built; gated on the voice + the backfill)

`radio.fluncle.com` is **Unit B of the audio-observation RFC** (`docs/rfcs/radio-observations.md`): a station that plays each finding's clean footage under its observation audio, a continuous lean-back run of the archive. **The surface is built** — the isomorphic host-rewrite (`radio.` ↔ `/radio` in `router.tsx`, mirroring galaxy), the `get_random_radio_track` oRPC op (`GET /api/v1/radio/random`, eligibility = `video_squared_at AND observation_audio_url`), and the `/radio` player (begin-gate, cycle on the observation's `ended`, preload-next, broken-asset skip). It draws its own chrome over the **MT-cropped square master with audio stripped** (`videoCrop` → landscape on desktop / portrait on mobile, `videoAudioStripped`), so the obsolete "landscape render" + "un-text portrait render" video gaps the two-master model retired never apply — radio derives every shape on the fly from the clean square master.

What's left before it's worth opening: it cycles only **squared + observed** findings, so it's gated on the same thing `/log` audio is — Fluncle's real voice (above) and the observation backfill filling enough eligible findings to cycle.
...
**Decide `observe`'s role when radio is built.** The Hermes role split made `admin track observe` **operator-only** (`requireOperator`), but the radio RFC wants the observation render to run autonomously on a schedule. Reconcile when radio lands: either the radio observation agent runs with **operator** authority (a trusted first-party automation, not the untrusted-input-facing chat bot), or `observe` becomes **agent-allowed** with the guards the RFC specifies (the `observe:${logId}` idempotency key + the Firecrawl untrusted-input boundary) — the latter path is scoped in [docs/hermes-automation-brief.md](./hermes-automation-brief.md). Pick deliberately; don't just loosen the guard to make it run.
...
### Hermes chat agent — open follow-ups

The Hermes chat agent is **live for the internal crew** (private Tailscale-only box, Discord allow-listed, Fluncle voice via `SOUL.md` + the `copywriting-fluncle` skill). It acts as the `agent` role — the box holds an `agent`-scoped token and the Worker refuses publish-/irreversible-class actions server-side, so the publish boundary lives at the Worker, not the box ([Roles](./agents/hermes-agent.md#roles-operator-vs-agent)). Operating doc + runbook: [docs/agents/hermes-agent.md](./agents/hermes-agent.md).

- **Give the agent real work (automation orchestrator).** Turning the agent from chat-only into the queue-driven runner for the enrichment self-heal, audio observations, the newsletter draft, and the Last.fm/Discogs backfills — what's handoffable and why, the secrets fix (wrap Loops + Firecrawl so the agent holds no vendor key), the queue gaps, the two reliability columns (`lastfmLovedAt` / `discogsStatus`), the role flips, and the build order — is fully scoped in **[docs/hermes-automation-brief.md](./hermes-automation-brief.md)**. Parked there until the CLI noun/verb naming pass below lands, then revise + execute. (Subsumes the old standalone "schedule the enrich-sweep" item.)
...
- **Non-root-in-container (defense-in-depth, low priority).** Run the agent as a non-root user with the token out of its readable env. Now that the token is `agent`-scoped this no longer guards the publish boundary — it only protects the agent's own surface and the token value from a fully-compromised agent, plus hardens against a container escape. Worth doing before any wider/public allow-list; not a blocker for the current private/trusted setup. (PRODUCT.md tension on a public chatty bot stands separately.)
...
**Newsletter archive — bring the editions home (not urgent).** Past letters live only in Loops: sent, then gone, with no way to read a previous edition anywhere on the Galaxy. Surface them on `fluncle.com` — a readable `/newsletter` archive, each edition a permanent object, ideally **spine-native like a mixtape** (a marked Log ID, a `/log/<id>` edition page, quiet feed / RSS inclusion) so an edition is a finding-shaped thing in the Galaxy, not a dead email. The agent already authors each edition's content before it stages the Loops campaign, so the clean source is to persist that payload to the DB at draft/send time (pulling sent campaigns back via a Loops read API is the fallback). Marked here so it isn't forgotten.
