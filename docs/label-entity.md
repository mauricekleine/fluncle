...
One consequence worth naming, because the two lists differ: **`/labels`'s editorial section is narrower than the sitemap.** The hub's TOP section is Fluncle's own list (_"every label I've pulled a banger off"_) and stays findings-joined; the sitemap is the machine's complete map and carries the crawler-discovered pages too. They answer different questions. The hub closes the resulting internal-link gap with a SECOND **"More labels"** section below the editorial list — the sitemap read's findings-free complement (`listLabelsCatalogue`), rendered in the unlit register and never posing as a finding — so the crawler-discovered pages are linked, not orphaned. The shape is documented once, in [docs/album-entity.md](./album-entity.md#the-sitemap-carries-every-page-the-hub-carries-two-sections).
