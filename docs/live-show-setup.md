...
Follow the recording doc's **3-track model** unchanged, with two live-rig substitutions:
...
- **Video capture = OBS Display Capture (ScreenCaptureKit) of the show display** — never Window Capture, which macOS documents as non-performant. This is what carries the glass to the stream.
...
6. **OBS captures real pixels** — the **video meter-bounce**: confirm OBS's Display Capture shows the glass actually moving, not a black rectangle. macOS screen-recording permission resets after an update and captures black silently (the video analogue of a dead audio route); re-grant it after any update. The glass must be fullscreen on the show display.
