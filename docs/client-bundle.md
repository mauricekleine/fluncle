...
The search dialog, Browse popup, signed-in crew menu, per-track action menus, and signed-out Follow popover load on first open, with focus and pointer prefetch from their triggers. The Follow popover carries its magic-link form into that lazy chunk. The `/account` form stays server-rendered in its route chunk so a direct visit shows the sign-in path immediately. The public Sonner toaster loads only after an announcement is requested. The preview player bar stays eager so its pause, close, and keyboard controls are continuous from the first play. The `/status` cron data is derived from the surfaces registry inside its server function and serialized through the loader, so the registry does not enter the eager client chunk. React DOM, the router, Sentry, the root chrome triggers, and their Base UI roots remain eager. Measure the entry with a production build and `chunk.modules[id].renderedLength` before further cuts; keep the first interaction and keyboard handoffs in the browser suite.
