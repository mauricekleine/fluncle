...
| `web.tracks`     | `/tracks`     | the whole list — every track (findings + catalogue), newest release first, filterable by year, tempo, key, label, galaxy; bare hub indexable, any filter `noindex`. Its detail page `/track/:trackId` is the archive recording's own destination (permanent primary key; a certified row 301s to `/log`), indexed on an evidence gate that the sitemap reads from the same SQL expression — see [track-destination.md](./track-destination.md) | secondary |
...
| `cron.follow-digest`          | `fluncle-follow-digest`          | Fri 17:00 Amsterdam | send new releases from followed artists and labels by email (`--no-agent`, Worker-owned send cap and kill switch)                                                                                                                                | secondary |
...
| `cron.cluster`                | `fluncle-cluster`                | nightly 03:20 Amst. | assign each finding to its nearest sonic galaxy (assignment-only k-means over the MuQ space; a full fit is an operator act), retire empties, consume a split request (`--no-agent`, zero LLM tokens)                                             | hidden    |
...
| `cron.label-triage`           | `fluncle-label-triage`           | daily               | read the undecided crawl-seed pile and report whether a triage round is due — a PURE trigger, zero model tokens; it cannot rule (box activation operator-gated)                                                                                  | hidden    |
...
| `cron.isrc-recovery`          | `fluncle-isrc-recovery`          | every 10m           | recover missing catalogue ISRCs through tokenless Deezer search → feed the exact-ISRC anchor queue (`resolve_anchor` with `spotifySearch: false`; never `anchor_track`, zero Apify)                                                              | hidden    |
...
| `cron.pipeline-watch`         | `fluncle-pipeline-watch`         | every 15m           | compare pipeline output with measured ready work and alert the operator when a stage stalls; self-report its own last run                                                                                                                        | hidden    |
...
2. **It lights up `/status`.** If the entry carries a `probeConfig`, the `fluncle-healthcheck` cron walks it (`statusProbes()`) and it appears on the health dashboard — an HTTP surface is GET-probed, a `cron` surface is checked by its last-run freshness. A **status-visible surface MUST carry `title` and `statusDescription`** (the human display name + a quiet, public-safe one-line subtitle): the status server read derives each cron's order, title, description, cadence, and schedule from the registry and sends that compact configuration through the route loader, so the registry remains the single source of truth without joining the client entry bundle. The registry test **build-fails** any cron that omits either display field. (The core HTTP probes render under short infra aliases — `web`/`r2`/`dns` — that are not registry names, so their labels live in `status.tsx`'s `INFRA_SERVICE_LABELS`/`INFRA_SERVICE_SUBTITLES`, guarded by the `apps/web` coverage test.)
