...
| `web.identity`   | `/identity`   | the identity lookup door + `/identity/:key` — one recording's identifiers and platform links, each saying whether Fluncle holds it, looked and found nothing, will not look, or serves no such link (the door is indexable + sitemapped; the keyed answers are `noindex, follow`) | tertiary  |
