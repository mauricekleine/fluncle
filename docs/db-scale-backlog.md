...
STATUS (2026-07-26): everything proven has SHIPPED LIVE — the Wave-1 pure hoists (#847/#848), the cheap #858 index/rewrite batch, the four proven indexes (#857), Keystone 1's maintained `is_catalogue` flag (#859, prod-verified: the `is_catalogue = 0` count matches the `findings` count exactly), and Keystone 2's maintained per-entity hub counts — write side + backfill (#880), read side (slice B). Five Wave-1 index items are DEFERRED — proven on hosted 150k NOT to help as-specified. Keystone 2's ONE unbuilt half is the self-healing reconciliation sweep (the counter-drift backstop); see its entry under "Shipped (live)" for the two known drift paths. Shipped items are collapsed into "Shipped (live)" below (their full pre-fix loc/shape analysis is preserved in git history); everything still outstanding keeps its detail. Prune items as they ship.
...
**1. KEYSTONE — per-track `is_catalogue` flag — SHIPPED (#859).** See "Shipped (live)" above. One consumer is deliberately left on the anti-join: `funnel.ts` (its findings join is pinned by the `certified` arm and the shared `kindClause`, so converting buys nothing). `search.ts`'s entity gate was the other, and Keystone 2's read swap retired it outright — its `findings.track_id is null` lived inside the shared hub group-by, which no longer exists.
...
- **slice B (the read swap) — SHIPPED (#886):** every consumer of the hub group-by now reads the stored columns via the ONE shared gate `hubInclusionWhere(alias)` (labels.ts) — the three web hubs (`listHubPage` incl. the A–Z lane + `?q=`), the MCP browse (`listCatalogueBrowsePage`), the API list ops + `get_*` counts, `countIndexableHubEntities`, the three sitemap row readers' floor (their `tracks ⋈ findings` join survives ONLY for `lastmod` + cover), the three `…MissingBio` worklists, and `search.ts`'s entity gate (now two integer comparisons on the matched row). `EXPLAIN` on every converted gate references no `tracks`/`track_artists`/`findings`. Two deliberate residuals: `scripts/backfill-labels.ts` + `scripts/backfill-album-graph.ts` stamp `label_id`/`album_id` in bulk without moving the counters (bounded — they only touch null pointers, and the nightly sweep corrects any drift), and `labelFindingCountsByIds` (the `/admin/labels` per-page count) stays on the raw aggregate ON PURPOSE — the one place a certification mismatch should be visible from TRUTH rather than the mirror.
