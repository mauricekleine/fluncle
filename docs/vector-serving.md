...
`apps/sonar` serves whole-corpus cosine searches from an in-memory MuQ index. The Worker hydrates returned IDs from Turso. Flag-off routes retain their bounded database implementation; latency-sensitive routes do not start that heavy remote scan after an enabled Sonar route becomes unavailable.
...
- **An unknown `filter` field is REJECTED** (`deny_unknown_fields`) rather than ignored, and that is a safety property. serde's default is to drop an unknown key, so a Worker sending a constraint an older binary does not know would get back a **wider** candidate set with no error — dismissed, duplicate, or long-form rows on a listener's page, indistinguishable from a correct answer. Failing the parse instead yields the empty result, which is a valid no-match response and never widens the candidate set.
- **Bad input is an empty result, never a panic.** Bad JSON, empty probes, a wrong-dimension probe, `top_k: 0` — all return `{"matches": []}`.
- **`top_k` and the probe count are CAPPED, and an over-cap body is a `400`** (`MAX_TOP_K = 1000`, `MAX_PROBES = 32`, `search.rs`). They are the two levers one request can pull to cost the whole box: `top_k` sizes a top-K heap **per rayon worker** under the unit's `MemoryMax=2G`, and each probe is another full dot-product pass over the candidate set. Both caps sit far above every real call — the largest `top_k` any surface sends is `TASTE_SHORTLIST` (300, `/mix`) and the widest probe set is `MAX_REC_SEEDS` (12, `/recommendations`) — so a legitimate caller never sees one. An over-cap request is a caller bug or an abuse attempt and should be visible as a 4xx. The Worker mirrors both constants (`SONAR_MAX_TOP_K`/`SONAR_MAX_PROBES`, `sonar.ts`) and returns unavailable **without a fetch** rather than clamping — a silently shrunk `top_k` would hand back a short page that looks correct.
- **`GET /health` is open**, so the box's healthcheck prober can read it without a secret; `POST /search` requires the `x-sonar-secret` header, compared in constant time. Missing or wrong → `401`. Health reports served row counts, the local checkpoint and baseline, producer head and backlog, delta age, replica-sync age and frame progress, exact raw-vector bytes, artifact version, pending acknowledgement, validation, rebuild cause and duration, plus the build `commit`. An authenticated health read also reports the exact artifact `consumer_id`, letting the Worker join engine evidence to the bounded producer status without a new secret or a consumer-table scan. Unauthenticated health never exposes that identity, and both authenticated and anonymous health responses carry `Cache-Control: no-store` so intermediaries cannot reuse one audience's body for the other. **`commit` is the git SHA the running binary was built from**, baked at compile time by the release workflow and `"unknown"` for a local build; commissioning requires an identified 40-hex commit but does not establish release provenance or require equality with the current Worker SHA.
...
If any of those fails, [`searchSonar`](../apps/web/src/lib/server/sonar.ts) returns **`null`**, and `null` is a documented answer rather than an error: it means _the engine is unavailable_. A timeout, a DNS failure, a 401, a garbled body, an unprovisioned local dev machine — every one of them collapses to the same signal, and none of them can take a page down. A well-formed **empty** result comes back as `[]`, a valid available answer that stays distinct from `null` all the way through the call site.

The database implementations have two hard ceilings in [`vector-fallback.ts`](../apps/web/src/lib/server/vector-fallback.ts): at most **50,000 deterministically ordered candidates** enter `vector_distance_cos`, and the Worker waits at most **12,000 ms** for the query. The candidate cap is inside the ranked statement and is marked by one checked SQL fragment. These results are exact only within that bounded relation and must never be presented as complete-corpus recall. Public sonic search therefore uses the existing full-text degradation when its Sonar flag is off or the enabled engine is unavailable; the accepted bounded SQL builder remains available for diagnostics and parity proof, not as a public fallback. The wait ceiling bounds the Worker response but cannot cancel remote SQL after Turso accepts it, which is why `/log` and `/track` neighbours also do not launch the database scan when their enabled Sonar route is unavailable. Both optional neighbour bands return empty and the main resolved page remains available. Other explicitly bounded analytical surfaces retain their documented database fallback.

The executor also assigns every slow scan the existing database telemetry vocabulary: `access_class = heavy-read` and an `operation_id` under `sonar.fallback.*` (`artists`, `log`, `mix`, `recommendations-catalogue`, `recommendations-findings`, `search`, or `track`). This makes fallback frequency and cost visible in the same query telemetry as every other database operation. Tests execute a relation larger than a supplied cap and observe the truncation, force the deadline to fire, assert the telemetry metadata, and inventory all seven call sites.

Two consequences worth stating plainly. **The flags default OFF, so Sonar is inert until an operator writes `"true"`** — public sonic search takes its typed full-text degradation and the optional `/log` and `/track` bands stay empty. **The bounded SQL path is not a second complete-corpus engine**: it remains explicitly opt-in for diagnostics, parity proof, and selected analytical fallbacks, never the default or outage path for latency-sensitive reads.

Because a surface must degrade rather than approximate, each call site also refuses to route when it cannot reproduce its own predicate in sonar's filter. Enabled sonic search declines whenever the query carries a `key`, `artist`, `album`, `label`, `year`, or free-text filter (key in particular: the database path matches `lower(tracks.key)` against a spread of spellings, while sonar compares the raw stored string case-sensitively). That decline takes the explicit full-text degradation, not the capped vector SQL path. `/mix` maps its key set to `key_in` only because `namedMoveKeys` yields the archive's raw stored spellings and sonar stores that same string verbatim. Re-applying a missing predicate during hydration is never the fix: it would prune rows sonar had already counted against `top_k` and silently return a shorter, different page.
...
`sonar_track_enabled` is separate from `sonar_log_enabled` because the two ask sonar for different candidate sets: `/log` sends `certified: true` (a question about findings), while the archive track destination admits both registers (a question about music), so a certified and an uncertified neighbour compete on the same terms. Both `/track` paths apply the same tempo-window-then-widen rule: a measured target first searches ±8% BPM and widens only when that window cannot fill the requested band; a target without BPM searches unfiltered. Dismissed and duplicate-stamped rows are excluded in both passes. The explicitly invoked diagnostic database path binds one raw-blob probe, ranks in SQL, and makes one vector pass per attempt; an empty answer renders no band rather than an error. A real-libSQL parity fixture places a globally closer half-time row outside a full tempo window and requires the Sonar and database paths to return the same ids in the same order.

**Flipping one is a `settings` row.** `setSonarSonicEnabled` and its six siblings write `"true"`/`"false"` through the same one flag store (`settings.ts`) every other kill switch in the app uses — never a second flag mechanism, and never a deploy. There is deliberately no admin UI. The minimum operator control is `fluncle admin vectors get tracks` plus `fluncle admin vectors set tracks --enabled true|false`, backed by operator-only `GET`/`PUT /admin/vectors/tracks/serving` and the canonical operations `get_vector_serving`/`set_vector_serving`.

The track switch separates **first commissioning** from **steady runtime**. A disabled-to-enabled transition requires an authenticated health response, a non-empty `sonar.track@1/1` index from a build carrying an identified 40-hex commit, successful validation, no pending acknowledgement, an active compatible consumer with a completed rebuild, an engine checkpoint equal to the durable applied checkpoint, and that checkpoint at the producer's current head. That build identity proves this is not a local `"unknown"` build; it does not claim release provenance or require equality with the current Worker SHA. The producer status comes from the bounded artifact-consumer primitive; engine-local `head_seq=checkpoint` cannot prove currentness after boot. Replica freshness uses three hourly reconciliation cadences, while delta debt uses three 30-second cadences. Once enabled, a repeated enable is idempotent. The normal publish-to-ack interval and a checkpoint mismatch between independently sampled engine and producer reads remain commissioning blockers but do not make steady runtime unready; failed validation or debt older than the bounded cadence does. Disable writes the existing flag false before any readiness read and therefore remains an unconditional rollback even when Sonar is down.
...
**Fresh embeddings arrive through bounded deltas.** Sonar synchronizes one embedded libSQL replica explicitly over the V2 sync protocol, requires the server's authenticated `/info` probe to accept that protocol, and never falls back to the retired V1 protocol. A clean open performs its first explicit sync before creating a connection; the pinned native client streams the full export into its same-directory atomic temporary file, flushes and renames the database, and then commits its native generation metadata. Within the native sync loop, successful frame responses are capped at 527,360 bytes and JSON or error responses at 64 KiB; the corpus-sized export is the only unbounded-length response and streams to disk. Later snapshot-fence and reconciliation syncs remain explicit. Every corpus SELECT runs against the local file, and no background replica sync interval is configured. A separate local state database keeps exact raw vectors, subject revisions and tombstones, the validated manifest, the durable checkpoint, and any pending remote acknowledgement. The steady loop reads `sonar.track@1/1` changes in global sequence order every `SONAR_DELTA_SECS` (default 30), applies one bounded batch in a local transaction, builds and validates the complete candidate, publishes tracks, centroids, and checkpoint metadata as one `ArcSwap` generation, acknowledges the exact producer batch, then clears the pending receipt. A crash before the local commit redelivers the batch; a crash after it republishes the committed candidate before acknowledgement. No remote full-corpus SELECT or periodic remote fallback remains. `/health.delta_backlog` is producer head minus the served checkpoint; `delta_age_seconds` is the age of the served generation only while that backlog is nonzero, and is exactly zero when the served checkpoint reaches the head. This prevents an idle, fully current engine from reporting ever-growing lag. The recovery test writes five rows to the local replica while a delta read is live, interrupts the consumer after the first two-row batch is durably pending but before its acknowledgement completes, restarts from that receipt, and requires both freshness metrics to reach zero within four seconds.
...
`cargo run --release --locked --offline --features resource-proof --bin resource-proof --manifest-path apps/sonar/Cargo.toml -- --profile 1x,2x --full-scale` runs the explicit full-scale resource proof serially, with one fresh child process per profile. The 1× child owns 43,372 track vectors (177,651,712 raw track-vector bytes) and the 2× child owns 86,744 (355,303,424 bytes), both read from the database-performance manifest; representative filter metadata and deterministic nonzero 1,024-float vectors exercise the real file-backed `StateStore`, index builders, and atomic `AppState` publication. Artist-centroid vectors are reported separately from those audited track-vector totals. Output is one JSON document suitable for redirection to an artifact directory outside the checkout.

The report labels cold fixture/initial construction and exceptional full replacement/corrupt-file recovery as single-sample windows and omits percentile fields for those one-observation windows, while the warm bounded-delta apply/build/validate and publish paths report five measured 100-event samples with start/end times and latency distributions. It records the child OS peak RSS rather than deriving memory from vector cardinality, captures the cumulative phase high-water mark while an old request holds the retired generation across a publish, proves that a third publish is refused until that request drops its `Arc`, and reports observed headroom against the service's 2 GiB limit. It also records the state main/WAL/SHM file sizes before and after a truncating checkpoint, the rebuilt/recovered file sizes, the observed metadata distributions, and digest parity between bounded delta, full rebuild, and corrupt-state recovery.

The release proof runs `--profile ci`, a small ratio-preserving derivative marked `exactProfileCardinality: false`, to validate the machinery without putting full-scale construction on ordinary `cargo test`. A full 1×/2× result is local hardware and OS evidence for the Sonar state/index lifecycle only: the local source contains the Sonar projection subset and stands in for an already-synchronized embedded replica, so it proves neither full-replica disk capacity, hosted/network replica-sync latency, nor deployed-box CPU capacity. It is also not deployment authorization.

**The engine self-deploys.** Full runbook in [`apps/sonar/deploy/README.md`](../apps/sonar/deploy/README.md); the loop:

1. **CI builds the artifact.** [`.github/workflows/cli-release.yml`](../.github/workflows/cli-release.yml) starts after Quality Checks succeeds for the exact pushed `main` SHA and selects Sonar when `apps/sonar/**` changed since `sonar-latest`. The generic quality owner runs `cargo fmt`/`clippy`/release build/`test` once; the release leg reuses that verdict, builds one static musl binary with `-C target-cpu=x86-64-v3` (AVX2 + FMA — for a brute-force dot-product scan, SIMD width _is_ the latency), proves its linkage, and publishes three assets to a rolling pre-release: the binary, its `sha256`, and the commit SHA it was built from. The artifact build lives in CI rather than on the box because a Rust release build is ~7–14 minutes on the box's two shared vCPUs — pinned at 100% on the machine that is at that moment serving sonar itself.
...
**What a swap costs the surfaces, and it is worth knowing before hunting a latency spike that lines up with the timer's hour:** the restart takes the engine away for as long as it needs to re-read the whole corpus (~30s today, growing with it), and again if it rolls back. For that window enabled sonic search explicitly degrades to full text and the optional `/log` and `/track` neighbour bands disappear; their main pages keep resolving. Selected analytical routes may use their bounded SQL fallback. The absence is honest and prompt while the engine returns, without starting remote vector work that outlives the Worker deadline.
