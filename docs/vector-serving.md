...
If any of those fails, [`searchSonar`](../apps/web/src/lib/server/sonar.ts) returns **`null`**, and `null` is a documented answer rather than an error: it means _use the database path_. The caller falls back to a Turso `vector_distance_cos` scan over a deterministic candidate window. A timeout, a DNS failure, a 401, a garbled body, an unprovisioned local dev machine — every one of them collapses to the same signal, and none of them can take a page down. A well-formed **empty** result comes back as `[]` rather than `null`, and surfaces treat that as a fallback too: a reached surface always has a real probe over a populated corpus, so zero matches is a hiccup, not a true empty neighbourhood.

Every such fallback has two hard ceilings in [`vector-fallback.ts`](../apps/web/src/lib/server/vector-fallback.ts): at most **50,000 deterministically ordered candidates** enter `vector_distance_cos`, and the Worker waits at most **12,000 ms** for the query. The candidate cap is inside the ranked statement and is marked by one checked SQL fragment, so declaring a cap without applying it is impossible through the shared executor. The result remains an exact ranking of that bounded candidate relation; beyond 50,000 eligible rows it deliberately gives up whole-corpus recall to keep an unhealthy or unavailable sidecar from turning into an unbounded database request. No fallback creates or consults a `libsql_vector_idx` or any ANN structure.
...
Two consequences worth stating plainly. **The flags default OFF, so the whole stack is inert until an operator writes `"true"`** — a fresh preview, a local checkout, and an empty database all run the database path. And **fallback is a bounded safety path, not a second primary engine**: below its candidate ceiling it preserves the prior exact answer; above it, it preserves availability and a measurable cost ceiling while Sonar is restored.
...
**What a swap costs the surfaces, and it is worth knowing before hunting a latency spike that lines up with the timer's hour:** the restart takes the engine away for as long as it needs to re-read the whole corpus (~30s today, growing with it), and again if it rolls back. **For that window every routed surface sees an unreachable engine, treats it exactly like a disabled flag, and falls back to the bounded Turso scan.** The fallback first materializes only stable candidate IDs, with all membership predicates already applied, then joins vectors to rank them; a candidate-order sort never carries 4,096-byte vectors. Below the candidate ceiling results stay identical; every request remains deadline-bound and observable while the engine returns. That is not an outage — it is the safety contract doing its job.
