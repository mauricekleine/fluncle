...
The observations were the worst-measured generated family Fluncle has — echoing 59/61 with mean pairwise overlap 0.0816, "…enjoy cosmonauts" verbatim closing 32/61, and a body-reaction lifted verbatim across two scripts (the 2026-07-14 full-corpus audit, [docs/planning/homogenisation-evidence.md](../planning/homogenisation-evidence.md)). This rail ports the auto-note's proven mechanism (which cut within-region overlap 0.041 → 0.015) to the spoken family, in two halves that read the SAME neighbourhood:
...
**A rejected script is HELD, never binned** — the `observation_rejections` ledger (one OPEN row per finding, bounded by a partial unique index) keeps the script, the neighbour it echoed, that neighbour's script as it read at the time, the lifted phrase, the score, and the thresholds in force, and raises a row in the `/admin` attention queue (`observation-rejected`). The operator reads it in the finding's observation dialog (the held-observation panel, the held-note panel's spoken sibling) and rules: **Render it** (operator-tier — it spends a Cartesia render, going through the same shared render path the observe endpoint uses, and a finding that got voiced in the meantime is skipped rather than re-rendered) or **Bin it** (the gate was right; the next sweep tick is free to try a colder read). A held row goes moot on its own the moment the finding gets an observation by any path. `force` on `observe_track` (a deliberate operator re-render) skips the gate — it is the same overrule, inline.

The UPSTREAM half of the same fix lives in the `context_distil` prompt: the `Texture:` line (which seeds every downstream voice) now demands track-specific sensory pointers across the full range and names the worn-through house words (`rolling`/`liquid`/`introspective`/`atmospheric`/`breakbeats`) as to-avoid. The `Texture:` line's SHAPE is unchanged — downstream consumers parse it.

Measured by `apps/web/scripts/measure-artifact-diversity.ts`, which now prints a per-family REGISTER cut (top openers/closers, the opening-word histogram, and the crutch words `hope`/`enjoy`/`cosmonaut(s)`/`shoulders`) alongside the overlap/lift measures — the before/after on those lines is the proof this rail works.
