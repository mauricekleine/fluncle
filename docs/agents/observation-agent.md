...
The web radio is one shared run: each listener joins the server's current segment, and segment boundaries advance from its skew-corrected schedule clock rather than the media element's `ended` event. A scheduled handoff anchors the next segment at the previous start plus its observation duration; failures re-resolve the authoritative slot instead of skipping to a random finding. The spoken observation is the only audio, while one silent, cropped video transform loops beneath it. The player opens its entry gate when both media elements can start, with a bounded fallback for browsers that never report readiness. Captions and the breather use the same clock offset as audio resynchronization, including after a hidden tab resumes. Reduced motion holds a poster frame and starts the observation alone.
