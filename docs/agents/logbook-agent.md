...
**The name exemption.** The gate polices what Fluncle WROTE, not the names he was given. The sweep hands the author every finding's artist and title as the day's material, so before the scan runs those names are masked out of the title and the token-stripped prose (`sectorSubjectNames` reads them from the DB — the right to say a name follows from the finding being logged that day, never from whoever is posting; `maskSubjectNames` in `observation.ts` does the masking). Without it the gate is UNSATISFIABLE rather than strict: a day that logged a track by "Future Signal" could not be written up AT ALL, and the gap list is OLDEST FIRST at `BATCH_CAP=1`, so that one day stopped the logbook backfilling anything newer, forever. The masking is the full name, word-bounded, so a partial reference still trips the ban and a short name can never amnesty a longer word it sits inside. A name that is EXACTLY one banned token is REFUSED from the exempt set (`UNSAYABLE_NAMES` in `observation.ts`): masking it would stop the gate policing that word for the whole entry, and it would lift the earthly-geography ban too, so a day that logged a track titled "London" would otherwise license "the London air" three paragraphs later. The entry simply does not name that one track — an entry is optional editorial and there is no final-attempt bypass, so a permanently unclearable day is the worse trade. The operator overwrite gets the same day-scoped exemption — no wider, no narrower.

## The attempt budget (the end of the retry-forever loop)

A gate rejection used to be a plain skip that left the item queued with **nothing counting the tries**, so "retry" meant "forever". The queue is `BATCH_CAP=1` over an oldest-first worklist, so an unwritable day did not merely waste its own tokens, it stopped the logbook backfilling anything newer. The sibling of this bug, in the entity-bio crons, re-authored three slugs ~90 times each over two days.

So the sweep now keeps a per-item **attempt ledger** (`docs/agents/hermes/scripts/attempt-ledger.ts`, a flat TSV under `$HOME/.logbook-sweep/attempts` that survives a tick, a container swap, and a rebake). A sector-day gets **three refused passes, ever**, and then this sweep never authors for it again.
...
- **The echo gate is charged as a deliberate trade, not as proof.** A voice or length rejection is a property of the draft alone. An echo rejection is scored against the item's NEIGHBOURHOOD, so it is a property of the draft _and_ the corpus around it, and it gets harder as the archive fills — three echo refusals are not proof the draft was bad. It is charged anyway, because the alternative is an item that always echoes sitting at the head of a cap-1 queue burning two authorings a tick forever, blocking everything behind it. The cost is visible and reversible: the day simply stays a gap (the logbook rails keep no rejection ledger), and deleting the item's line from the ledger re-arms it once the neighbourhood has moved on.
- **NO final-attempt bypass** (the operator's ruling, 2026-07-30). The bio sweep stores its third draft even when the gate refused it, because an empty bio slot leaves an entity page half-built. An entry is optional editorial and a gap is a perfectly good state, so gate-failed copy is **never** published to close a queue. An exhausted item is simply skipped, counted in the tick's `exhausted` summary field, and reported once in the sweep's stderr.
