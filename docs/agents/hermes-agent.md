...
- **The remaining `--no-agent` sweeps** — `fluncle-note` (hybrid, every 10 min — the auto-note), `fluncle-social-capture` (every 10 min — captures the YouTube/TikTok post URLs Postiz withholds on create), `fluncle-publish-advance` (every 30 min — the render → publish auto-advance; **ships DARK behind a default-deny kill switch**, so the tick posts nothing until the operator resumes it), and `fluncle-studio-clip` (every 15 min, #215 — cuts a mixtape set into framed 9:16 clips → R2). The canonical, always-current roster (cadence, mode, source) is the table in [`hermes/cron/README.md`](./hermes/cron/README.md) § Cron roster; run `systemctl list-timers 'fluncle-*'` on the host for the live timers + next-run times.
