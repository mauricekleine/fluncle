...
| `tracks enrich --all`                                                                                                           | agent    | Idempotent self-heal; no public footprint            |
...
Hermes is also Fluncle's queue-driven automation orchestrator: scheduled, trusted, no-untrusted-input loops over the `fluncle` CLI (`docs/hermes-automation-brief.md`). The three first crons are **drafted in the repo but not wired on the box** — versioned at [`docs/agents/hermes/cron/`](./hermes/cron/) (`jobs.json` + a `README.md` with the operator's wire-on-the-box steps). The operator wires them; nothing is live until then.
...
- **The three (all hourly, all idempotent queue drains):** enrichment self-heal (`tracks enrich --all`), context-note (drain `hasContext=false`, fetch Worker-side Firecrawl facts), observation (drain `hasContext=true AND hasObservation=false`, author the recovered-audio script with `copywriting-fluncle`, then `observe --script`). They stay under the agent ceiling — reversible, internal, no public footprint — so an injection can do nothing the scheduled run can't.
- **Gate:** wire only after the CLI admin naming rename lands (the brief's sole remaining green-light; sibling PR `cli/observe-context-crons`). The context-note job carries a `TODO(cli-rename)` marker for the moving command name. Backfills (Last.fm / Discogs) and the newsletter are **not** in this set.
...
- **Residual surface:** a fully-compromised root agent is bounded to the agent role — reads, the enrich sweep (`tracks enrich --all`), analysis write-back, a TikTok inbox draft. All reversible/internal, none public without the operator. Anyone on the Discord allow-list can trigger those same agent-allowed writes (not just reads); all publish-class is blocked for everyone but the operator.
