...
- **Pin the upstream tag** (Hermes is pre-1.0; `latest` can change message handling and the model-context startup check under you). Current pin: `nousresearch/hermes-agent:v2026.6.19`. The `fluncle` CLI (`fluncle@0.61.0`) and Claude Code (`@anthropic-ai/claude-code@2.1.190`) are pinned the same way — bump deliberately at each `npm install -g` line.
