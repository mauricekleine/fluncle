...
- **The two hourly AGENT crons (drafted in `jobs.json`, idempotent queue drains):** context-note (drain `hasContext=false`, fetch Worker-side Firecrawl facts) and observation (drain `hasContext=true AND hasObservation=false`, author the recovered-audio script with `copywriting-fluncle`, then `observe --script`). They stay under the agent ceiling — reversible, internal, no public footprint — so an injection can do nothing the scheduled run can't.
