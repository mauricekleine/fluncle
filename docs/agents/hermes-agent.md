...
- **The remaining `--no-agent` sweeps** — `fluncle-note` (hybrid, every 10 min — the auto-note), `fluncle-social-capture` (every 10 min — captures the YouTube/TikTok post URLs Postiz withholds on create), `fluncle-healthcheck` (every 10 min — probes each service → the public `/status` board via `record_health`), and `fluncle-studio-clip` (every 15 min, #215 — cuts a mixtape set into framed 9:16 clips → R2). The canonical, always-current roster (cadence, mode, source) is the table in [`hermes/cron/README.md`](./hermes/cron/README.md) § Cron roster; run `hermes cron list` on the box for live IDs.
