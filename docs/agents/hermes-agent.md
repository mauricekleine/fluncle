...
> Model choice is a **voice** decision as well as cost. A flash/generic model is the _precise anti-Fluncle_ — it slips into third-person "fluncle", prose em-dashes, and chirpy register. Now the crons run independently on subscription auth, the gateway only carries the Discord chat voice, so it moved to `z-ai/glm-5.2` for cost (from Sonnet's $3/$15 to ~$0.94/$3); `anthropic/claude-sonnet-4.6` is the premium fallback if the deadpan/first-person voice ever slips. Whatever is pinned must pass the voice gate (below) before the bot goes public.
...
**Deploy model — the schedule is code.** Every automation cron has been migrated OFF the Hermes gateway's `hermes cron create` scheduler (the schedule lived in the volume's `state.db`, invisible to git, lost on a re-provision) ONTO a **repo-checked-in host systemd `.timer`/`.service` pair** — the pattern `fluncle-pin-watch`/`fluncle-healthcheck`/`fluncle-capture`/`fluncle-embed` already used (docs/hermes-durable-deploy-rfc.md § Unit E). The sweep code bakes into the image at `/opt/hermes-scripts` and the enrichment DSP skill at `/opt/hermes-skills` — read straight from the baked path, so `/opt/data` never holds a script (no `docker cp`, no volume projection); they auto-update from `main` via the hourly `fluncle-pin-watch` rebuild. Each `<job>-timer/`'s `ExecStart` is `docker exec -u hermes … bash /opt/hermes-scripts/<sweep>.sh`; the operator installs them all with [`hermes/install-host-timers.sh`](./hermes/install-host-timers.sh). The **gateway now holds NO automation crons** (`hermes cron list` is empty for automation, `jobs.json` is `"jobs": []`); only the Discord CHAT agent (`gateway run`) remains on the gateway.

**The reset boundary.** A bare re-provision restores **code** (baked into the image), **schedule** (the host-timer units, via `install-host-timers.sh`), and **secrets** (re-injected from 1Password by `fluncle-secrets-sync`) automatically. The one layer it does NOT restore is the accumulated `state.db` state — sessions, memories, kanban, cron output/run-history under the agent home — which restores from the daily `fluncle-backup`→private-R2 backup, whose restore tooling is the one sanctioned follow-on.
...
- **Mechanism (host systemd timers).** Each automation cron is a `Type=oneshot` `.service` (whose `ExecStart` `docker exec`s the baked sweep as the unprivileged `hermes` user) triggered by a `.timer` (`OnBootSec` staggered + `OnUnitActiveSec` cadence, `Persistent=true` to catch up a tick missed across a reboot); the weekly newsletter uses `OnCalendar=Fri 15:00 Europe/Amsterdam` (the timezone is in the timer, so the box-clock TZ pin is no longer load-bearing for it). Host timers run in **parallel** — a strict win over the gateway's one serial runner (a ~300s global budget) where a long sweep starved the latency-sensitive ones. Each sweep is a `--no-agent` `.sh` that sources the shared `0600` secrets file and runs its bun/curl payload; the sources under [`hermes/scripts/`](./hermes/scripts/) are the canonical files, baked into the image (`/opt/hermes-scripts`) as the deploy target. **No agent jobs remain**, and no gateway automation crons — `jobs.json` is `"jobs": []`, kept only as the mechanism record. (The retired gateway cron mechanism is documented for reference in [`hermes/cron/README.md`](./hermes/cron/README.md) § The cron mechanism.)
...
- **The remaining `--no-agent` sweeps** — `fluncle-note` (hybrid, every 10 min — the auto-note), `fluncle-social-capture` (every 10 min — captures the YouTube/TikTok post URLs Postiz withholds on create), and `fluncle-studio-clip` (every 15 min, #215 — cuts a mixtape set into framed 9:16 clips → R2). The canonical, always-current roster (cadence, mode, source) is the table in [`hermes/cron/README.md`](./hermes/cron/README.md) § Cron roster; run `systemctl list-timers 'fluncle-*'` on the host for the live timers + next-run times.
- **All automation is on host systemd timers now.** `fluncle-pin-watch` (the image self-deploy, [`hermes/pin-watch/`](./hermes/pin-watch/)) and `fluncle-healthcheck` (probes each service → the public `/status` board via `record_health`, [`hermes/healthcheck-timer/`](./hermes/healthcheck-timer/)) were the pathfinders — a monitoring/infra job must not depend on the busy scheduler it manages — followed by `fluncle-capture` + `fluncle-embed`, and then the whole remaining roster (§ Deploy model above). The `fluncle-healthcheck` prober is the one sweep that writes NO cron-output marker (it IS the prober — a self-read would be circular; its `/status` row is emitted self-evidently).
...
- The agent still runs as root _inside the container_, but that no longer grants publish authority (the credential lacks it). Running the agent non-root with the token out of its readable env is now **defense-in-depth** — it protects the agent surface + the token itself — not the publish boundary. Tracked as optional, in [ROADMAP.md](../planning/ROADMAP.md).
- Back up `~/.hermes` as an encrypted/snapshot copy only (it holds `.env` + memory) — never a plaintext off-box tarball.

## Status

Live for the internal crew: pinned image with the `fluncle` CLI (ungated; the Worker is the boundary); secrets via `op` → the root-owned secret env-file; Discord app online (both privileged intents, a tight allow-list); model pinned (`z-ai/glm-5.2`); Fluncle voice via `SOUL.md` + the `copywriting-fluncle` skill, voice-gated. The publish boundary is **server-side**: the box holds an `agent`-scoped token and the Worker refuses publish-/irreversible-class actions for that role ([Roles](#roles-operator-vs-agent)) — this supersedes the earlier local-command-gate design and the need for a separate publish-confirm flow. Open (tracked in [ROADMAP.md](../planning/ROADMAP.md), now optional defense-in-depth rather than a public-readiness blocker): non-root-in-container hardening. The full cron roster is **live on the box** ([§ Crons](#crons-automation)): the `fluncle-enrich` / `fluncle-context-note` / `fluncle-backfill` / `fluncle-social-capture` / `fluncle-studio-clip` `--no-agent` sweeps + the hybrid `fluncle-note` / `fluncle-observation` / `fluncle-newsletter` sweeps (the 2026-06-23 cutover, with the newsletter converted from its agent cron on 2026-06-27 — **no agent jobs remain**), and — as of **2026-06-24** — the `fluncle-render` video conductor (it wakes a separate scale-to-zero box.ascii render box, the render box, rendering off the Hermes box entirely). The `/status` prober (`fluncle-healthcheck`) and the image self-deploy (`fluncle-pin-watch`) run from rave-02 **host** systemd timers, not the gateway.
