...
- A named galaxy also carries one plain **sound line** (`GALAXY_SOUND_LINES` in `apps/web/src/lib/galaxy-sound.ts`) on `/galaxies`, its page, its hover card, and its search row, so a reader can choose a galaxy by ear. A newly named galaxy gets its line there, written from its own findings (titles, artists, notes, release styles) through the `copywriting-fluncle` skill and gated by `canon-reviewer`; until then it renders no line at all, never a generic stand-in. A re-run that reshapes a galaxy far enough to make its line untrue means re-hearing and rewriting it. Each line leads with the sound and lands on what it does to Fluncle's body, stays distinct from every other galaxy's line so it helps a reader choose, and inherits the mechanics `galaxy-sound.test.ts` pins (one sentence, sentence case, no em dash, no banned words, 22 words at most).
