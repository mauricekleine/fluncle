...
2. per entity (bounded batch, `ENTITY_BIO_BATCH_CAP`, default 1):
   ...
   - **AUTHOR** (the one agentic step): run `claude -p` (`claude-sonnet-5`, subscription auth, read-only tools) on the Worker-supplied `prompt` so it loads `copywriting-fluncle`.
