...
- **A daily retry unit alerts only on its final slot.** For the units run by `daily-retry-runner.sh`, any failure other than exit 75 of an attempt that STARTED before that unit's final calendar slot (a first attempt killed in a container swap, a non-TERM `docker exec` failure) is logged and not posted, because the retry slot still decides the day. The notifier judges the failing attempt's own `ExecMainStartTimestamp`, never the wall clock, so a final attempt that starts at 23:57 and fails after midnight still alerts. Exit 75, the runner's own final verdict, a failure of an attempt that started at or after the final slot, and a failure whose start time systemd cannot report all post as usual. The per-unit final slots mirror `DAILY_RETRY_SCHEDULES` in `scripts/daily-retry-state.ts`, pinned by `sweep-failure-notify.test.ts`.
