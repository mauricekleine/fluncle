...
- **A daily retry unit alerts only on its final slot.** For the units run by `daily-retry-runner.sh`, any failure other than exit 75 before that unit's final calendar slot (a first attempt killed in a container swap, a non-TERM `docker exec` failure) is logged and not posted, because the retry slot still decides the day. Exit 75, the runner's own final verdict, and any failure at or after the final slot post as usual. The per-unit final slots mirror `DAILY_RETRY_SCHEDULES` in `scripts/daily-retry-state.ts`, pinned by `sweep-failure-notify.test.ts`.
