...
2. `fluncle-healthcheck.ts` probes each service in parallel (each with a short 3–5s timeout), diffs every status against `${HOME}/.healthcheck/state.json`, and Discord-pings on a flip to `down`, a recovery, or a down streak crossing an escalation rung.
...
Designed backpressure is a separate, non-alarming axis. A summary carrying `throttled: true` means the sweep stopped cleanly at a vendor limit or a database-admission wall and will resume next tick; the healthy `sweep-errors` row names those sweeps as having yielded cleanly, and the event contributes no failure points.

## The stall bar: a pause that stopped being backpressure

One yielded tick is the design working. A standing run of them is a sweep that is alive, green, and doing nothing — and it is invisible in every failure signal, because a paused tick has its work counters nulled and reports `errors: 0`. So the backpressure axis carries its own bar: **a cron whose yielded ticks inside its rolling window reach `max(3 ticks, one hour of its own cadence)` is STALLED, and the `sweep-errors` row degrades naming it, the reason it gave, and how many ticks it has now spent paused** — `1 sweep paused without working: crawl (database_admission:queue ×14)`. A 5-minute sweep therefore needs twelve yields, an hourly one three, a daily one three days. The Discord line is edge-triggered on entering and on leaving, exactly like strain, and both bars are env-overridable (`HEALTHCHECK_BACKPRESSURE_STALL_MS` = 1h, `HEALTHCHECK_BACKPRESSURE_STALL_TICKS` = 3).

The reason is the paused summary's own `reason`, qualified by the runner's finer word when there is one: a database-admission yield names `queue`, `public-latency` or `database-health` in `admissionYieldReason`, and the row prints `database_admission:<word>`. A phase-scoped yield exits before the runner writes a summary, so that word is carried across the boundary by the phase module rather than being lost; an unrecognised word is dropped rather than mapped onto a neighbour, so "we could not tell" stays distinct from "the queue was long".
