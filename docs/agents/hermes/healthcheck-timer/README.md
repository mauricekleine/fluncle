...
The rule, in plain words: **count the evidence a sweep leaves that work did not get done, sum it per cron over `max(6 hours, 3 × cadence)`, and report a sweep once its failure points reach 25% of the scheduled ticks in that window AND are spread across at least three separate ticks.** Evidence is two explicit lists and nothing else: `STRAIN_PHRASES` (substrings that mean an item did not get done, tuned against all 238 `log()` string literals in `../scripts/`, so `skipping` is deliberately absent and the far tighter `stays queued` is not) and the failure counters plus nullable error string the sweeps already put in their own summaries (`failed`, `gateSkipped`, `errors`, `error`). Counter values may be numbers or arrays. There is no `/error/i` catch-all: a detector that fires on the 471-a-day `embedded + written` line is worse than no detector.
