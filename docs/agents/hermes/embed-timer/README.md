...
Embed's per-finding work is **minutes-scale**: the source is the CAPTURED FULL SONG (Unit 3), not the 30s preview, and a windowed MuQ forward over a ~5-min song is ~10 sequential window forwards. On its own parallel host timer, with a `TimeoutStartSec` sized for that, a full-song embed never delays the latency-sensitive 5-minute sweeps (enrich, context-note, note) — the same shape as [`fluncle-capture`](../capture-timer/README.md) and [`fluncle-healthcheck`](../healthcheck-timer/README.md).
