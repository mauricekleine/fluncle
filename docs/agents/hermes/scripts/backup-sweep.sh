...
# this thin wrapper is the bash entry; all the work lives in the bun orchestrator beside
# it (backup-sweep.ts). Its stdout is the cron's run output.
#
...
#         DISCORD_ALERT_WEBHOOK (the backup-failed ping — read from this file like the
#         rest).
...
# ../install-host-timers.sh), which `docker exec`s it in the container. The job needs no fluncle
...
# A caller may exec this with a minimal PATH that omits /usr/local/bin (the bun symlink)
# and /root/.bun/bin, so a bare `bun` is "not found" → exit 127. Prepend the known
# install dirs so this wrapper's `bun` resolves regardless of the caller's PATH.
...
# the Turso/R2 creds and the webhook this sweep reads arrive via this file).
