...
# EX_TEMPFAIL tells a phase-aware caller that its command never started and may be retried.
PHASE_YIELD_EXIT=75
...
ADMISSION_POLL_SECS="${DATABASE_ADMISSION_POLL_SECS-}"
...
admission_post() {
  ...
  case "$response_code" in
    ...
    *)
      # Test doubles and older curl builds may omit the write-out suffix on success.
      response_code=200
