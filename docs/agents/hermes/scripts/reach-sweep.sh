#!/usr/bin/env bash
# reach-sweep.sh — the `--no-agent` reach cron's job ENTRY (`fluncle-reach`). The daily
# snapshot behind the public /reach page (how far Fluncle's tentacles stretch across the web).
#
...
# and auto-updates from main via pin-watch; a rave-02 HOST systemd timer docker-execs it once a
...
# credential; the WORKER holds every one and does all the fetching. So this driver just fires
# `fluncle admin reach collect` ONCE — the Worker snapshots every Tier-1 platform (each
# best-effort) and upserts one idempotent row per (platform, metric) per day. A same-day re-run
# lands `inserted: 0`. `record_platform_stats` is AGENT tier, so the box's existing agent-scoped
...
emit_cron_output reach -- "${BUN_BIN}" "${SCRIPT_DIR}/reach-sweep.ts" "$@"
