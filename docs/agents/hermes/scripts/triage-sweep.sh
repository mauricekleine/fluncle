...
#     token is read from the SHARED 0600 operator-placed file at
...
# ../install-host-timers.sh), which `docker exec`s it in the container. `triage_submission` is
...
# A caller may exec this with a minimal PATH; prepend the known install dirs so the
...
# The sweep's credentials (CLAUDE_CODE_OAUTH_TOKEN for `claude -p`, DISCORD_*) live in the
# operator-placed file — sourced here: the SHARED 0600
