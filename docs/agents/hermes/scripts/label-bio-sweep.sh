#!/usr/bin/env bash
# label-bio-sweep.sh — the `--no-agent` label-bio cron's job ENTRY (`fluncle-label-bio`).
#
...
# kinds (artist + label), dispatched by `--kind`. This thin bash wrapper is the label
...
# ../install-host-timers.sh), which `docker exec`s it in the container. `describe_label` is AGENT
...
# ~/.hermes/cron/output/fluncle-label-bio/ (read by the /status prober). See ../cron/README.md.
set -euo pipefail
...
emit_cron_output label-bio -- "${BUN_BIN}" "${SCRIPT_DIR}/entity-bio-sweep.ts" --kind label "$@"
