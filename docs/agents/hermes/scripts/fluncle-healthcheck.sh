...
# systemd timer (`docker exec … bash …/fluncle-healthcheck.sh`) every ~10m, on its own timer
# so no long sweep it monitors can delay or starve it. See the units + the one-time deploy in
# ../healthcheck-timer/README.md.
#
...
# live in ../healthcheck-timer/README.md.
set -euo pipefail
...
# All probe targets + the Discord webhook are file-sourced (the targets are
# public-safe but kept out of the repo). A 0600
# ${HOME}/.healthcheck.env populated by the operator (see the ops runbook note in
# 1Password). FLUNCLE_API_TOKEN is NOT here — it rides the container env.
