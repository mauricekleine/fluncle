...
# ../install-host-timers.sh), which `docker exec`s it in the container. `backfill_label_images` is AGENT
...
# dirs so the wrapper resolves regardless of the caller's PATH.
