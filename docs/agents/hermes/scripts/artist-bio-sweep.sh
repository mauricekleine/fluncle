#!/usr/bin/env bash
# artist-bio-sweep.sh — the `--no-agent` artist-bio cron's job ENTRY (`fluncle-artist-bio`).
#
...
#
# Why a .sh that execs a shared .ts: the entity-bio sweep is ONE orchestrator over two
# kinds (artist + label), dispatched by `--kind`. This thin bash wrapper is the artist
# entry; its sibling label-bio-sweep.sh passes `--kind label`. All the JSON work lives in
...
# DRAFT (`fluncle admin artists draft-bio` → the Worker runs Firecrawl + pulls finding titles
...
#     secrets, so the token is read from a 0600 operator-placed file at
...
#
# Scheduled by a repo-checked-in HOST systemd timer (../artist-bio-timer/, installed by
# ../install-host-timers.sh), which `docker exec`s it in the container. `describe_artist` is AGENT
...
# ~/.hermes/cron/output/fluncle-artist-bio/ (read by the /status prober). See ../cron/README.md.
set -euo pipefail
...
# orchestrator's `fluncle`/`bun`/`claude` spawns resolve regardless of the caller's PATH.
...
# file the operator places — sourced here: a 0600 ${HOME}/.fluncle-secrets.env (mounted ~/.hermes)
...
emit_cron_output artist-bio -- "${BUN_BIN}" "${SCRIPT_DIR}/entity-bio-sweep.ts" --kind artist "$@"
