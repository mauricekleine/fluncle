...
Both halves of the switch must agree before a single request is spent: the server's `crawl_box_fetch_enabled` KV flag (default on; the exact string `false` closes it, no deploy and no rebake) is answered on the prepare as `boxFetch`, and this box's own `FLUNCLE_CRAWL_BOX_FETCH` is the other half. Either side saying no puts every read back on Worker egress with nothing else changed. The tick reports `boxFetch`, attempted box requests in `boxFetched` and `requestsByKind`, positive release-detail commits in `releaseDetailsStored`, and a `blockedReason` on zero-write ticks when the cause is known. The box requests a repair sample only on the first prepare; later prepares and older boxes use only the cheap ready-lane seek. `no_storable_work` requires that first sample to find no ready storable release and fewer than 200 repair rows with none storable. A full 200-row sample with no storable release is unknown, so the reason stays null; failed requests and incomplete wall-budget ticks also leave it null and retain their error or stop reason. Each attempted MusicBrainz request, including a retry, logs its kind, node kind, source, and outcome; the per-kind counts sum to `boxFetched`. Worker fallback requests log the same fields on the server and are not included in the box request counters.
...
Every automation cron runs off repo-checked-in host timers so the SCHEDULE is code. Because a `docker exec` sends stdout to journald, the sweep self-writes the `/status` marker (`# Cron Job: fluncle-crawl`) via the shared [`cron-output.sh`](../scripts/cron-output.sh) helper.
