...
The watchdog reads agent-allowed `GET /api/v1/admin/catalogue/pipeline` for frontier, anchor, storable, unstorable, and capture backlogs. Each count is `{count, atLeast}`; `atLeast: true` makes `count` a lower bound. The caps are 1,000 frontier and anchors, 1 storable, 5,000 unstorable, and 2,000 capture. Capture repair debt makes a positive ready-work count a lower bound; the count is null only when repair debt leaves no positive lower bound. If the due-work cutover is disabled, the read falls back to the exact legacy count. Alerts and marker summaries retain the lower-bound flag, and the ordinary `get_crawl_status` and CLI reads remain exact.
