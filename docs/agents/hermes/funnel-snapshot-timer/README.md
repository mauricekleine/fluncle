...
2. **A database-admission yield** — the runner skips the payload before it starts, so no in-process retry can help and only another firing can. The timer carries a **retry `OnCalendar` slot** later the same UTC day. The shared [`daily-retry-runner.sh`](../scripts/daily-retry-runner.sh) reads the day's payload marker before the Worker trigger, so the extra slot does not write a second snapshot after a completed run. A second pre-payload skip fails the service once and invokes its Discord failure notifier.
