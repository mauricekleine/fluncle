...
| `fluncle-capture` ⏳     | every 5m            | `--no-agent`           | none (yt-dlp → R2)        | capture each finding's full song once → the PRIVATE `fluncle-source-audio` bucket (yt-dlp via a residential proxy; a NON-BLOCKING side-channel — see § below) |
...
| Job               | Schedule | What it does                                                                                                                                                                                                                                                                                                  | Server slice                                                                                            |
| ----------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `fluncle-capture` | every 5m | Read `admin tracks capture-audio --queue` (`captureQueue=true`, newest-first) over direct HTTP; per finding (bounded batch, cap 4): `yt-dlp` a sticky-proxy `ytsearch5` → duration-guard → `-f bestaudio` → ffprobe-confirm → S3-direct PUT to `fluncle-source-audio` → `update_track` sets the key + `done`. | The `capture_status`/`source_audio_*` columns + the agent-tier `update_track` write + the queue filter. |

**OPERATOR-GATED wiring.** The private bucket + the proxy/R2 creds are provisioned (2026-07-07). `yt-dlp` + `ffprobe` on PATH are a box deploy prereq:

1. **Secrets** — already in the box's `op inject` template (materialized to `~/.fluncle-secrets.env`): `FLUNCLE_YTDLP_PROXY_HOST` / `_PORT` / `_USERNAME` / `_PASSWORD`, `FLUNCLE_SOURCE_AUDIO_R2_ACCESS_KEY_ID` / `_SECRET_ACCESS_KEY` (scoped Object Read & Write on `fluncle-source-audio` ONLY, never `fluncle-videos`), and the reused public `R2_ACCOUNT_ID`. The box's AGENT-scoped `FLUNCLE_API_TOKEN` drives the write-back (capture fields are agent-writable analysis).

2. **Install `yt-dlp` + `ffmpeg`** on the box (ffprobe ships with ffmpeg) — a deploy prereq the orchestrator handles; keep `yt-dlp` fresh via the `pin-watch`-style freshening (YouTube's bot-walls move).
...
3. **Deploy the pair + create the cron** (`~/.hermes` is `hermes`-owned 700 — copy IN via `docker cp`, per § Operational gotchas):
   ...
   Smoke-test as the cron user first (§ Operational gotchas): `docker exec -u hermes -e HOME=/opt/data/home hermes bash /opt/data/scripts/capture-sweep.sh` → expect an `{ "ok": true, "done": … }` summary and an object under `analysis/source/` in R2. It is already registered in `@fluncle/registry` (`cron.capture`) + the healthcheck `AUTOMATION_CRONS`, so `cron.capture` shows on `/status` (stale/down until the first tick lands).
