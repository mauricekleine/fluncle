...
**Inputs (deterministic).** The factual fuel is the finding's metadata, read via `fluncle track get <id> --json` (artists, title, label, release year, galaxy, vibe) — the same facts the stored `context_note` was distilled from. The `context_note` itself is internal-only (no read endpoint surfaces it), so the **Worker injects the stored note** as the gate's source of truth at delivery; the metadata is what the authoring prompt grounds the prose on. (If the note's shape is later surfaced for authoring, pass it via `--context-note`.)
