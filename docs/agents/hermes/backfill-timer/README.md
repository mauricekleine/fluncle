...
The rave-02 host trigger for the `--no-agent` **backfill** sweep. `fluncle-backfill` repairs the music-graph side-channels over already-published findings — the Discogs release-id resolve, the Last.fm love, and the Apple Music URL — and then, as a fourth leg, drains Apple URLs + album facts over the CATALOGUE (`admin backfills apple-catalogue`, `--limit 100` = one batched server pass). The catalogue leg runs LAST on purpose: both Apple legs draw on one shared call meter in the Worker, so the certified findings drain first and the catalogue takes what survives. The box holds no vendor keys, so it just PACES the Worker (one bounded batch of each source per tick, `--limit 3` for the three finding legs); the Worker carries the per-row reliability state + the Retry-After backoff. Zero box tokens. A host systemd timer `docker exec`s the baked sweep inside the `hermes` container every 30m.
