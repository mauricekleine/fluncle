...
There are two clip crons in the tree. This one — `fluncle-studio-clip` (`clip-sweep.sh`, the Studio set→clip cut) — is the LIVE one and migrates here. The separate `clip-drip-sweep.sh` (the clip→Instagram drip-feed) is DELIBERATELY un-deployed (excluded from the Unit A bake), so it gets **no** host timer until its own go-live. **This sweep is on /status**: `cron.studio-clip` is registered in `@fluncle/registry` and in the [`fluncle-healthcheck`](../scripts/fluncle-healthcheck.ts) prober's `AUTOMATION_CRONS` (match `studio-clip`, 15m), so its `# Cron Job: fluncle-studio-clip` marker is read. The drip-feed is not: there is deliberately **no** `cron.clip-drip` surface and no prober row — a registered cron with no timer sat permanently green for a job that does not exist, so both were removed until it is actually wired (the reconcile is recorded in [`../cron/README.md`](../cron/README.md)).
