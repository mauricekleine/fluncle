...
2. `capture-sweep.ts` reads the CATALOGUE-AWARE capture worklist over direct HTTP (`GET /api/v1/admin/tracks/work?kind=capture&scope=all` — certified findings first, then `capture_priority` DESC, backoff-aware; an operator-disabled label is tier −1 and excluded in SQL), takes up to `BATCH_CAP` (4), and per row: searches `yt-dlp` on a per-track sticky proxy session, picks the best candidate via **channel-trust matching** (a candidate on the finding's label channel, a curated D&B aggregator, or the artist's own channel relaxes the duration guard asymmetrically — allowing the intro/outro padding a label video carries over the streaming master, bounded so an hour-long DJ set is still rejected; wrong-version titles de-rank), downloads `-f bestaudio`, ffprobe-confirms the length, PUTs the bytes S3-direct to `fluncle-source-audio` at `<logId>/<sha256>.<ext>`, and PATCHes `update_track` with the key + `capture_status='done'` (or `unmatched`/`failed`). It prints one JSON summary line.
