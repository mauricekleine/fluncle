...
**A wholly failed tick is a failed tick.** Capture's failures are item-level by construction — a row that cannot be fetched is counted, left queued, and the tick continues — so `errors` stays 0 however many rows failed, which is right for the ordinary partial batch and wrong for a wall. A rotted `yt-dlp` pin, a dry proxy, or an exit that stopped clearing challenges fails EVERY row identically, and `checked: 12, failed: 12, produced: 0` was a green ledger row for thirteen days. So the tick carries a verdict: **over a floor of `CAPTURE_BLIND_MIN_ATTEMPTS` (4) attempts, a failure share at or above `CAPTURE_BLIND_FAILURE_SHARE` (0.9) fails the run** with `errors: 1`, `ok: false`, and a `reason` naming the dominant class — `ytdlp_failing`, `proxy_failing`, `bot_challenged`, or `capture_failing` when nothing recognisable dominates. The non-zero exit is what the ledger derives `ok: false` from and what the unit's `OnFailure` notifier fires on.

The denominator is **attempts that reached the fetcher** (`captureAttempts`, published beside the verdict): `done + failed + unmatched`. An unmatched row proves the fetcher works — bytes arrived and the fingerprint refused them — so it belongs in the denominator and never in the numerator. A row the server's capture budget refused (`captureRejected`), a row whose write yielded (`capturePending`), and a row skipped before any fetch are not attempts and cannot dilute or manufacture the share; a paused or admission-yielded tick never reaches the verdict at all. The bar sits far above the measured healthy distribution — the worst honest tick in a fortnight failed 55% of its attempts, and the bulk sat at or under a third. Counts stay exactly as measured; only the verdict is added.
