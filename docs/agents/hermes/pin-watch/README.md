...
Every attempt past the single-flight lock also appends **one run-ledger row** (`fluncle-pin-watch`, the same emitter every sweep carries), because the two signals above are moments and the ledger is the durable record: an alert can be missed and a `/status` row only shows the latest tick, so a build failing on every hourly tick left nothing to ask about afterwards. The line states facts and no verdict — `checked` is 1 once the tick has actually compared the box's versions against main's (0 when it died before that, which is the one shape a blind run must not be able to hide behind), `produced` is 1 when a swap landed, `errors` is 1 on any FATAL path, and `queue_depth` is 1 while drift is known and undeployed. A run of failed builds therefore writes the ledger's designed alarm — `produced == 0` beside a standing `queue_depth` — every hour, and its non-zero exit makes the derived verdict `ok: false`. Read it with `fluncle admin telemetry read --unit fluncle-pin-watch`; a tick that only loses the lock reports nothing, because the run holding the lock is the attempt.
