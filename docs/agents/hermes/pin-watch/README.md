...
## Why it runs on the host, outside the container

A container can't cleanly rebuild and replace _itself_. The rebuild has to run on the **host** (which owns the Docker daemon) — the same pattern as the rave-01 [`fluncle-rave-watchdog`](../../../../apps/ssh/watchdog). The sweeps (enrich/observe/render) `docker exec` into the container to do _app_ work; this does _infra_ work on the container itself.
...
- **Guarantee the restart.** An `EXIT` trap (composed with the existing tmpfs-env cleanup trap) restarts **exactly** the timers it stopped, best-effort (`|| true` per timer, so one failing start never strands the rest). Because it rides the `EXIT` trap, it fires on success, on `die()`, on a build/pre-smoke failure, **and on the auto-rollback path** — a failed or rolled-back rebuild never leaves the box with its sweeps disabled, which would be worse than the bug it fixes. `INT`/`TERM` are trapped into an ordinary exit as well, so a `TimeoutStartSec` kill or an operator's `^C` still runs that guarantee instead of stranding the roster.
...
- **The monotonic sweeps** (most of the roster) ride `OnUnitActiveSec`, which systemd measures from their **service's** last activation. Nothing about the stop/start moves that reference point, so a period that elapsed during the build is simply overdue the moment the timer is armed again. `Persistent=` has nothing to do with it — `systemd.timer(5)` states that flag "only has an effect on timers configured with `OnCalendar=`".
- **The `OnCalendar=` units** (`audit`, `backup`, `cluster`, `reach`, the two `*:0/15` pollers, …) all carry `Persistent=true`, which is exactly the documented catch-up: the service "is triggered immediately if it would have been triggered at least once during the time when the timer was inactive."

Start forty-odd of them inside one loop and forty-odd sweeps arrive at the box's single database write lane together, where the admission coordinator ([docs/database-performance.md](../../../database-performance.md)) serializes them: measured holds of 174s (`fluncle-label-releases`), 71s (`projection-maintenance`), 60s (`cover-masters`) and 37s (`artist-sweep`), eight more units giving up after the 120s admission wait, and an attended render tick losing its lease twice.

**Per-timer `RandomizedDelaySec` cannot fix this**, and the reason is worth stating because the intuition runs the other way. The catch-up fire is "subject to the delay imposed by `RandomizedDelaySec=`" — but systemd rolls that delay from the restart instant, so a 90-second jitter still lands the whole fleet inside one 90-second window. Jitter dissolves _phase alignment_ over a long run, which is what it was added for; it cannot widen a burst that starts from a single instant. The spread has to come from the release itself.

So a run that reaches a **clean end** releases the roster over a window instead:

- **Spacing the starts IS the stagger.** Both routes above fire at `systemctl start` of the timer, so spacing the starts spaces the firings — and the overdue tick is still honoured rather than dropped. Nothing has to be cleared to get this: no stamp file is touched and `Persistent=` is left exactly as the unit files declare it, which matters because poking at that flag is how a monotonic timer ends up stranded at `NextElapse=infinity` ([../timer-watchdog/README.md](../timer-watchdog/README.md)). The `rearm_stalled_timer` kick rides inside the same loop, so its `systemctl start` of the _service_ is spaced too.
- **The order is deterministic and spreads the long holders.** The units measured holding the write lane longest — `artist-sweep`, `capture`, `cover-masters`, `crawl`, `label-releases`, `projection-maintenance` — are distributed evenly across the sequence rather than left adjacent, with the short units filling the gaps between them. Both groups are sorted first, so the same roster releases in the same order every rebake and a bad release is reproducible. The order and the spacing are logged.
- **The window is a knob.** `PINWATCH_RELEASE_STAGGER_SECS` sets it in seconds; an invalid value refuses **before anything is built**, beside the resource-ceiling validation, so the refusal can never land after the swap with the roster stopped. Left unset it is sized from the roster (9s per timer, floored at 60s and capped at 900s — a ~46-timer box gets a ~7-minute window, ~9s apart). An explicit `0` collapses the release back to one instant.
- **Only a clean end waits.** The staggered release is called from exactly two places: the `--dry-run` exit and the post-deploy exit, the latter _after_ the post-swap smoke passed and the prune finished, so the rollback decision is already behind it. Every failure path — an aborted build, a failed pre-smoke, a rollback, any `die()` — falls through to the `EXIT` trap, which restores the roster **immediately**. Trading a convoy for a roster stopped for minutes after a failed rebuild would be the worse bug.

`pin-watch.service` carries a `TimeoutStartSec` sized to hold the slowest rebuild _plus_ the widest window, and a test pins it above the script's own window ceiling so the two cannot drift apart.
...
Default `--if-stale` (the timer); `--force` runs immediately regardless of drift or the two-hour window; `PINWATCH_FORCE_IMMEDIATE=1` is the equivalent environment override; `--dry-run` builds + pre-smokes without swapping; `--fingerprint` prints the watched paths + the current hash and exits before any of this. Every actual build attempt, including a failed build or dry run, records its start time in `/opt/fluncle-pin-watch/last-build-at` before Docker builds. A normal tick with drift within 7,200 seconds of that time reports `queue_depth: 1`, leaves the fleet running, and defers the build. The next eligible hourly tick fetches `main` again and builds its latest image. The state file survives service runs; `PINWATCH_LAST_BUILD_FILE` changes its path for tests. A malformed state file fails closed and requires operator attention.
...
3. **Compare pins AND baked content:** the `fluncle` binary release-URL version + the `@anthropic-ai/claude-code@` pin in `docs/agents/hermes/Dockerfile` vs the running container's `fluncle version` / `claude --version`, AND the baked-content fingerprint on `main` (`git ls-tree -r HEAD` over the paths derived from the Dockerfile's `COPY` set) vs the running image's stamp at `/opt/.hermes-baked-fp`. Both current → **no-op**; either drifted (or an empty stamp) → rebuild when the two-hour window is open, otherwise defer with the drift queued. The `Dockerfile` is part of the fingerprint, so a merged `FROM` change (a bun base bump, tag + digest) or a node/uv/gh pin edit rebuilds the same way; pin-watch never queries a registry itself — upstream drift is [`hermes-pin-drift`](../../../../.github/scripts/hermes-pin-drift.sh)'s job.
...
7. **Pre-smoke the NEW image in throwaway containers — before the live one is touched:** `fluncle version` == the pin; `claude --version` == the pin; `fluncle admin tracks publish <sentinel> --json` returns the exact `forbidden` code for the captured agent token (proving authentication and the role boundary in middleware, before its handler can open the primary database); and a **runtime container** starts under the production security flags on a scratch `tmpfs` `/opt/data` owned by uid 10000, stays running past a settle, and lets the `hermes` user write its `$HOME`. The pre-smoke makes no primary-database read, so database unavailability cannot block rebuild or swap. Any failure → abort, alert, **the live box is never touched**.
8. **Swap** (the only moment the live container changes): stop+rm, `docker run` the new image with the captured env + the canonical `§ Run` flags, including the **resource ceiling** — `PINWATCH_CPUS` / `PINWATCH_MEMORY_GIB` (defaults `3` / `6`, `--memory-swap` pinned equal to `--memory` because the host has no swap), read once into the globals the swap and the rollback both create from, and raised to the LIVE container's own limits when the operator has already set something higher with `docker update`. An invalid override refuses before anything is built.
9. **Post-swap smoke:** the container is `Running` and `fluncle` answers.
...
11. **Release the sweep timers.** A run that reached a clean end (a landed deploy, or `--dry-run`) starts them again **over a window**, long write-lane holders spread apart — see [§ Releasing the roster over a window](#releasing-the-roster-over-a-window). Every other exit — an aborted build, a failed pre-smoke, a rollback, any `die()` — restores them **immediately** through the EXIT trap from step 5. Either way, the stopped timers are started again before the process exits.

Discord alerts (deploy / rollback / failure) use the `DISCORD_ALERT_WEBHOOK` already in the container's env — no config file.

Every run also reports a **`self-deploy` health check** to the public [`/status`](https://www.fluncle.com/status) board (POST `/api/v1/admin/health`, agent tier) so the self-maintenance loop is visible alongside the other services: `ok` when current or freshly deployed, `degraded` when a bump failed to build / pre-smoke or was rolled back (box healthy on the prior tools, a human should look), `down` if a rollback itself failed. It reuses the **agent token already in the container's env** (the same one the pre-smoke role-boundary probe uses) — no token on disk, none from `op` — and the message is public-safe and deliberately vague (no host, no tool VERSIONS — those are internal — and no raw error; the version detail rides only the operator-only Discord alerts). If the timer ever stops, the row simply goes stale on `/status`, which is itself the signal that the box may be silently drifting.

Every attempt past the single-flight lock also appends **one run-ledger row** (`fluncle-pin-watch`, the same emitter every sweep carries), because the two signals above are moments and the ledger is the durable record. The line states facts and no verdict — `checked` is 1 once the tick has actually compared the box's versions against main's, `produced` is 1 when a swap landed, `errors` is 1 on any FATAL path, and `queue_depth` is 1 while drift is known and undeployed, including the deferred window. A failed build remains queued and its non-zero exit makes the derived verdict `ok: false`; its next attempt waits for the window unless the operator forces it. `quiesce_started_at`, `quiesce_ended_at`, and `quiesce_duration_seconds` report the actual timer-stop interval through the staggered or immediate restore, with null timestamps and zero duration on ticks that never quiesce. Read the rows with `fluncle admin telemetry read --unit fluncle-pin-watch`; a tick that only loses the lock reports nothing, because the run holding the lock is the attempt.
...
The script is a no-op when current. Repeated ticks during a rebuild window report known drift without stopping any sweeps.

The runtime smoke holds no credentials and never mounts the real `/opt/data`; that is what makes it safe for both normal and `--dry-run` rebuilds. It proves image boot, the idle main process (`tini` → `box-entrypoint`) staying up under the production security flags, and that the image's `hermes` user (uid/gid 10000, home `/opt/data`) can write a home owned the way the production mount is. It never touches the real mount, so it does not prove the host `~/.hermes` directory's ownership; the first sweep ticks after a swap are what exercise that.

The swap's `docker run` line ends in `gateway run`. The current image ignores its arguments (the entrypoint only idles); the arguments stay so a rollback to an older image, which needs them, starts through the same line.

## Testing the rollback rail

The rollback rail is the safety net (it restores the previous image if a future bump passes pre-smoke but then fails to come up). Drill it without putting the box at risk: `PINWATCH_TEST_FAIL_POSTSMOKE=1` forces the _first_ (post-swap) health check to fail exactly once — the box swaps to the freshly-built image, "fails", and is restored to the previous image (the second health check, the rollback's own, runs for real). Both images are known-good, so the container stays healthy throughout; it just exercises the recovery path end to end.

```bash
sudo env PINWATCH_TEST_FAIL_POSTSMOKE=1 /opt/fluncle-pin-watch/rebuild-hermes.sh --force
```

Expect: `swapping … -> <new>` → `TEST: forcing this post-swap smoke to fail` → `rolling back to <previous>` → `FATAL: rolled back …` (a rollback exits non-zero by design), a `↩️ ROLLED BACK` Discord alert, and `docker ps` back on the previous image, healthy.
