...
Every other sweep is a thin wrapper that sources [`cron-output.sh`](../scripts/cron-output.sh) and wraps its single payload internally. The render conductor is a full STATE MACHINE with many inline `exit`s (skip-lock, single-flight hold, queue-empty, provision-fail, started), so it can't wrap itself with one `emit_cron_output` call. Instead the marker wrap happens at the UNIT: [`database-admission-runner.sh`](../scripts/database-admission-runner.sh) admits `fluncle-render`, then `bash -c '. cron-output.sh && emit_cron_output render -- bash render-conductor.sh'` runs the conductor as a child, captures its one-line summary, and writes the `# Cron Job: fluncle-render` marker the [`fluncle-healthcheck`](../scripts/fluncle-healthcheck.ts) prober reads for `cron.render`. Admission covers the final due-work queue maintenance write; while that cutover remains default-off, the operation's compatibility profile is the legacy read-only queue lookup. The prober is UNCHANGED.
