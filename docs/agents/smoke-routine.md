...
`smoke:routine` boots a fully isolated dev stack on **dedicated ports** — a `turso dev` libSQL server over this checkout's seeded `.dev/local.db` (`:8899`) plus a Vite dev server (`:3120`) — runs the four smokes (`shell`, `queue` with `SEED=1`, `touch`, `labels` with `SEED=1`) sequentially against it, tears the stack down, and prints one machine-parseable final line:

```
SMOKE ROUTINE: shell=PASS|FAIL queue=PASS|FAIL touch=PASS|FAIL labels=PASS|FAIL
```

It exits `0` iff all four passed. It continues past a failing smoke so all four always report. Preflight fails fast with one clear reason if `.dev.vars` or the seeded `.dev/local.db` is missing, or if either dedicated port is already listening (it refuses, never kills).
...
1. `git pull --ff-only`, then `bun run --cwd apps/web smoke:routine`. It boots an isolated dev stack on dedicated ports, runs the four admin browser smokes (shell, queue with SEED=1, touch, labels with SEED=1), tears down, and ends with one summary line: `SMOKE ROUTINE: shell=… queue=… touch=… labels=…`.
