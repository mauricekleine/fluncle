...
The global background-work admission coordinator first runs as a shadow observer: it resolves the committed operation registry, reads the durable lane and queue state against the database clock, evaluates database health, one harmless direct-read latency, and public web latency, then emits a bounded `database.admission` event without queueing, waiting, or changing the sweep. The shadow response carries the contender, access class, queue age, wait, hold, yield reason, recovery flag, and outcome vocabulary that the enforced path uses, so cutover evidence and enforcement cannot classify the same firing differently.
...
Inspection is available through `fluncle admin receipts get <operation-key> [--json]`. Current inspection and digest-bound read-only reconciliation (`fluncle admin receipts reconcile <operation-id> <operation-key> <request-digest> [--json]`) use bounded POST bodies, never path or query coordinates. The initialization-era keyed GET inspection route remains compatible until Goal H, with its coordinate redacted from Sentry error and span URLs. `fluncle admin receipts repair --stale-before <ISO> [--limit 50] [--json]` retains the initialization-phase repair transport and is the explicit operator-only mutation: it rejects at most 100 accepted rows strictly older than the supplied fence. These commands return bounded state, operation ID, stable result identity, and timestamps, never the request digest or stored result payload. The Worker disables Sentry request-body capture. Reconciliation and repair emit low-cardinality spans and structured evidence containing the operation ID, outcome, state, and aggregate counts only; operation keys, digests, request/result bodies, secrets, and private topology never enter Sentry or logs.
