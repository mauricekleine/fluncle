...
- **Version scheme — SET.** `version` is `1.1.0` in `app.config.js` (the marketing version, bumped from `0.1.0` on the operator's 2026-07-30 ruling: `1.1.0` rather than `0.2.0`, so the store-visible version reads as the arc every doc already calls "the 1.1 arc"). `eas.json` uses `cli.appVersionSource: "remote"` + `autoIncrement` on the production profile, so EAS owns the iOS build number and bumps it per build — you never hand-edit a build number.
...
- [x] Marketing `version` bumped in `apps/mobile/app.config.js` — **DONE (2026-07-30): `1.1.0`**, the operator's ruling over `0.2.0` so the store-visible version matches the arc's name (EAS still owns the build number via `autoIncrement`).
