...
Static policy bans JS/TS comments through the root oxlint config and the web package's nested config, both globally; `scripts/no-comments-scope.ts` requires both switches and gives the JSON, shell, and YAML gates every tracked file outside their vendored skill, generated migration, and symlink carve-outs. Tool directives, shell shebangs and shellcheck directives, and version labels beside digest-pinned GitHub Actions remain valid. The root config disables DOM-specific `jsx-a11y` rules only for React Native primitives, keeps React Compiler backlog rules explicit, and preserves `vitest/no-focused-tests` while tuning out stylistic Vitest rules. Raycast's CLI owns formatting for `apps/raycast`; oxfmt handles the rest.
...
`deploy-watch-paths.json` is the canonical Cloudflare exclusion list. `deploy-watch.mjs verify`, classifier tests, and workflow topology tests prove that every ordinarily excluded path skips the post-deploy job and every deploy-triggering or unknown path selects it. The classifier also mirrors Cloudflare's documented watch-path bypasses: a push with zero changed files, at least 3,000 changed files, or at least 20 commits always expects a build. The workflow deliberately has no top-level path filter, so it always emits an outcome. The downstream live discovery walk inspects the triggering run and starts its browser only when the public-surface sweep actually passed, rather than treating an intentional no-deploy success as a deployment. It has no pull-request trigger: selected hermetic E2E proves candidate code, while the live walk records only a served deployment and checks out the exact triggering `workflow_run.head_sha` for reproducible harness attribution. Repository and manual dispatch payloads are resolved by code checked out from pushed `main`; their exact SHA must be a commit reachable from `origin/main` before Actions checks out or executes payload-selected code.

The repository contains an optional Queue consumer in `apps/ci-events`. It accepts only the documented terminal Workers Builds push-event schema for the configured main branch, GitHub repository, and Worker, requires a full commit SHA and a bounded workflow-safe build UUID, and emits `cloudflare-workers-build` repository dispatches. The GitHub workflow treats failure and cancellation as immediate failures. A success waits briefly for `/api/v1/health` to report the exact SHA or a descendant, covering a coalesced deployment without accepting an unrelated commit. Queue delivery is at least once: a failed or malformed terminal dispatch is retried and eventually reaches the dead-letter queue, while irrelevant nonterminal events are acknowledged. The build UUID gives each retry a stable correlation key, duplicate terminal events converge through workflow concurrency, and a later Cloudflare retry has its own build UUID and run. The relay keeps the GitHub token exclusively in a Worker secret binding, bounds each GitHub request, logs structured non-secret correlation fields, and enables sampled traces plus complete terminal dispatch logs.
...
The event payload and retry semantics follow Cloudflare's [Workers Builds event schema](https://developers.cloudflare.com/workers/ci-cd/builds/event-subscriptions/) and [Queues retry model](https://developers.cloudflare.com/queues/configuration/batching-retries/). The release trigger follows GitHub's [`workflow_run` contract](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#workflow_run), and rotating snapshots use the documented [restore/save cache pattern](https://github.com/actions/cache/blob/main/caching-strategies.md#reusing-cache-across-frequently-used-branches).
