...
Agent edit hooks run `bun run quality:preflight -- start --quiet`. It fingerprints the resulting tracked worktree tree, every untracked path and file body, Node, and classifier configuration; the same bytes therefore keep one identity across unstaged, staged, and committed states. It starts selected leaves in the background and returns immediately. Concurrent edit hooks use an atomic, recoverable launch election so one detached worker owns the shared worktree state. Repeated edits update the desired fingerprint instead of blocking the writer after each edit.
