...
GO
	...
	*)
		rm -rf "$ARCHIVE_WORK"
		die "public source archive identity did not match ${NEW_SHA:0:12}"
		;;
	esac
...
if [ "$MODE" = "--force" ]; then
	...
	should_build=1
	reason="recorded baseline $OLD_SHA unreachable — re-baselining"
elif [ "$OLD_SHA" = "$NEW_SHA" ]; then
	should_build=0
	reason="already at $NEW_SHA"
elif [ "$SYNC_KIND" = "git" ]; then
	changed="$(git -C "$REPO_DIR" diff --name-only "$OLD_SHA" "$NEW_SHA" -- "$APP_SRC" |
		grep -E '\.go$|/go\.(mod|sum)$' || true)"
	if [ -n "$changed" ]; then
		should_build=1
		reason="apps/ssh sources changed: $(printf '%s' "$changed" | tr '\n' ' ')"
	else
		should_build=0
		reason="no compiled-source change in $OLD_SHA..$NEW_SHA"
	fi
elif [ ! -f "$SOURCE_MANIFEST_FILE" ]; then
	should_build=1
	reason="no deployed source manifest — safely re-baselining from the public archive"
elif [ "$(cat "$SOURCE_MANIFEST_FILE")" = "$NEW_MANIFEST" ]; then
	should_build=0
	reason="no compiled-source change in $OLD_SHA..$NEW_SHA"
else
	should_build=1
	reason="apps/ssh compiled sources changed in $OLD_SHA..$NEW_SHA"
fi
