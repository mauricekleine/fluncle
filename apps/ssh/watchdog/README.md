...
| **rave-01→rave-02 cross-ping** | `WATCH_STATUS_URL` in this watchdog — reads the public `/api/v1/status` freshness                                                                                | the rave-02 prober going dark, _on-box and fast_ (a Discord ping within ~10m, no external-service grace-period wait) |
...
- `WATCH_STATUS_URL` — the public `/api/v1/status` URL (the cross-ping freshness source).
...
- `WATCH_ONION_URL` — _optional_ (job 3); the onion health URL (`http://<addr>.onion/api/v1/health`). Unset skips the onion probe.
