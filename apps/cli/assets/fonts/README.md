...
The TTF is still **baked into the Hermes image** — `docs/agents/hermes/Dockerfile` copies it to `/opt/fonts/Oxanium-SemiBold.ttf`, and `apps/cli/assets/fonts` is one of the paths in pin-watch's baked-content fingerprint (`docs/agents/hermes/pin-watch/README.md`). Keep the asset and its license together for as long as the image carries it; retiring it is an image-build change, not a docs change.
