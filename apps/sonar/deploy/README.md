...
**`gateState` speaks the ledger's own closed vocabulary** — `active` / `disabled` / `paused`, the `run_events.gate_state` enum — not words of this script's invention, because the Worker rejects an unknown one and a rejected POST leaves no row at all. Two ticks are gated: a run that finds the flock held (it looked at nothing, so its counters are `null` too) and `--dry-run` (it verifies and pre-smokes and then deliberately leaves the build undeployed, so its `produced:0` beside `queueDepth:1` is an operator's choice rather than a stalled box). `--force` is **not** gated: it is a real deploy that really swaps, the ledger nulls a gated run's work counters, and gating it would erase the `produced:1` that proves the swap happened — nor can it raise a false alarm, since a forced swap ends `produced:1, queueDepth:0`.
