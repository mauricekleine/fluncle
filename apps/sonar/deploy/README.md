...
8. **On any post-swap failure → ROLLBACK:** the process-wide exit trap stops the candidate, restores the exact pre-swap SQLite files before the previous binary, restarts, and requires the previous recorded commit when that identity is known. The same restore is idempotently resumed from disk after a hard interruption. Intent and state backup are cleared only after exact candidate acceptance or a confirmed previous-generation restart. Once the previous generation is healthy and intent removal is durable, leftover backup cleanup is reported as cleanup debt rather than a false outage. If rollback itself fails, fire the loudest alert, post `down`, and stop for a human. **The box is never left broken.**
