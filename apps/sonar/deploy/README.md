...
7. **Post-swap smoke:** the service is `active` and the live port answers `/health` with `"ok":true`, polled for the same timeout. Before binding the HTTP port, every normal restart validates the durable last-good state and confirms that its exact checkpoint is the artifact consumer's active checkpoint. Successful activation and acknowledgement persist an internal proof bound to the exact local digest and checkpoint; if the artifact API is unavailable during a later restart, only that proven generation may serve. A successful remote response always wins: any state/checkpoint divergence revokes the proof before rebuild, while a valid local manifest left by a crash before activation has no proof and cannot be reported healthy. This internal proof is distinct from the freshener's `local-state-ready` marker: the proof governs whether one exact index may serve during a control-plane outage, while the marker records that the runtime-contract bootstrap completed and enables validate-only pre-smokes. Each artifact API call has a 10-second connect timeout and a 30-second total request timeout, safely inside the 180-second post-swap budget, so a peer that accepts but stalls fails closed unless the exact local generation is already proven. The loopback TLS probe ignores hostname validation because the origin certificate names the public host, not `127.0.0.1`.
8. **On any post-swap failure → ROLLBACK:** immediately after snapshotting the prior binary, the process-wide exit trap is armed to restore `sonar.prev`, restart, and confirm healthy on every unaccepted exit—including a failed command, TERM, or shell interruption. It disarms only after live health and the durable bootstrap marker both succeed, so the explicit post-smoke failure and asynchronous exits share one rollback rail and cannot leave a candidate installed or poison `.prev`. If rollback itself fails, fire the loudest alert, post `down`, and stop for a human. **The box is never left broken.**
