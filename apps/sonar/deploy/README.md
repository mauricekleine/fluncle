...
8. **On any post-swap failure → ROLLBACK:** immediately after snapshotting the prior binary, the process-wide exit trap is armed to restore `sonar.prev`, restart, and confirm healthy on every unaccepted exit—including a failed command, TERM, or shell interruption. The on-disk intent survives failures where no trap can run and is cleared only after acceptance or confirmed rollback. If rollback itself fails, fire the loudest alert, post `down`, and stop for a human. **The box is never left broken.**
