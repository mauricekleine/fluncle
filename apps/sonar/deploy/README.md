...
5. **Pre-smoke the new binary in isolation before touching the live service.** When durable state exists, boot the candidate on a free high loopback port with TLS disabled and `SONAR_VALIDATE_ONLY=true`, using the live environment's consumer-state path and search secret. Then poll `/health` until it reports both `"ok":true` and the exact commit from `sonar.commit`. The commit equality closes the mutable-release race where separately fetched assets could otherwise pair a newer commit label with an older self-consistent binary and checksum. This proves the binary runs on the CPU, opens the embedded state, validates the durable manifest and raw vectors, builds both indexes, and serves HTTP. It does not open/sync the replica or contact/mutate the artifact consumer. Any failure after the completed-bootstrap marker exists leaves the live service untouched and fails the deploy. Before that marker exists, an invalid partial state follows the retry bridge into step 6, but a healthy response with the wrong baked commit is always a hard identity failure. The throwaway process is always reaped because it holds a second full copy of the index in RAM.
6. **Swap** (the only moment the live service changes): read the exact commit identity from the healthy live process (falling back to the validated deployed marker only for a predecessor without that health field), snapshot the current binary to `sonar.prev`, persist an initial `swap-in-progress`, stop the service, copy the quiesced SQLite main/WAL/SHM generation with its service ownership and mode to bounded rollback files, durably mark that state snapshot ready, atomically rename the new binary into place, then `systemctl restart sonar`. Every critical removal is checked explicitly because Bash errexit is suppressed inside conditional callers. The intent, binary, state files, and their directory entries cross explicit `sync -f` boundaries in dependency order. The in-process exit trap covers ordinary failures and signals; the durable intent makes the next run restore both binary, state, and deployed identity before release lookup, no-op detection, or a new snapshot after SIGKILL, reboot, or power loss. The systemd unit and `/etc/sonar.env` are **left untouched**. On the first local-replica deployment, this start performs Sonar's bounded registration, local-replica sync, snapshot attestation, durable build, and activation; any failure enters the same rollback rail as an ordinary restart. Only a healthy post-swap service atomically writes the freshener's `local-state-ready` marker, so a crash or rollback before acceptance remains retryable without deleting local files.
7. **Post-swap smoke:** the service is `active` and the live port answers `/health` with `"ok":true` and the exact candidate commit, polled for the same timeout. Before binding the HTTP port, every normal restart validates the durable last-good state and confirms that its exact checkpoint is the artifact consumer's active checkpoint. Successful activation and acknowledgement persist an internal proof bound to the exact local digest and checkpoint; pending finalization clears its receipt and writes that proof in one transaction. If the artifact API is unavailable during a later restart, only that proven generation may serve. A successful remote response always wins: identity, contract, state, or checkpoint divergence revokes the proof before rebuild, while a valid local manifest left by a crash before activation has no proof and cannot be reported healthy. A full rebuild stages the prior proved local bytes through the activation verdict but never serves that stage during a control-plane outage after registration; only authoritative status confirming its checkpoint may restore it. Scheduled reconciliation requires an exact active remote checkpoint and carries proof to the replacement digest in the same local transaction. Network failures, 429, and 5xx responses are availability failures; ordinary 4xx, malformed success bodies, wrong identity, and contract errors are semantic failures. This internal proof is distinct from the freshener's `local-state-ready` marker. Each artifact API call has a 10-second connect timeout and a 30-second total request timeout. The loopback TLS probe ignores hostname validation because the origin certificate names the public host, not `127.0.0.1`.
8. **On any post-swap failure → ROLLBACK:** the process-wide exit trap stops the candidate, restores the exact pre-swap SQLite files before the previous binary, restarts, requires the captured live commit when that identity is known, and atomically restores `deployed-sha`. The same restore is idempotently resumed from disk after a hard interruption. Candidate acceptance durably writes its SHA and an accepted-intent marker before disarming rollback or removing intent, so every crash phase names the generation actually on disk. Intent and state backup are cleared only after exact candidate acceptance or a confirmed previous-generation restart. Once either generation is healthy and its identity is durable, leftover backup cleanup is reported as cleanup debt rather than a false outage; a later no-op retries it. If rollback itself fails, fire the loudest alert, post `down`, and stop for a human. **The box is never left broken.**

**What a deploy costs the surfaces.** The restart in step 6 takes sonar away for as long as it needs to validate its state and rebuild both indexes (about three minutes at a 45k-track corpus; about fifteen when a restored older state generation forces a `checkpoint_divergence` rebuild, which is what a rollback produces), and again if step 8 rolls back. `BOOT_TIMEOUT_SECS` (default 1200) must stay above the slower of the two, or a healthy swap reads as a failed one. That is a bounded degradation, not a universal SQL fallback: sonic search degrades to full-text search, while optional `/log` and `/track` neighbour bands return empty; only the explicitly bounded analytical and diagnostic paths use their Turso fallbacks. Results remain truthful for the length of the reload. Worth knowing before hunting a latency spike that lines up with the timer's hour — and worth remembering when picking that hour.
...
| `SONARFRESHEN_ROLLBACK_INTENT_FILE` | derived from state dir     | Durable unaccepted-swap intent recovered before release lookup; override for tests/provisioning.         |
| `SONARFRESHEN_STATE_ROLLBACK_FILE`  | derived from state dir     | Bounded quiesced SQLite rollback generation retained only until candidate acceptance.                    |
