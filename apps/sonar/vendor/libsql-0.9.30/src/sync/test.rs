...
async fn wait_for_atomic_temp(directory: &Path, expected: &[u8]) -> PathBuf {
    tokio::time::timeout(Duration::from_secs(2), async {
        loop {
            let temp_path = std::fs::read_dir(directory)
                .unwrap()
            ...
            if let Some(temp_path) = temp_path {
                if std::fs::read(&temp_path).unwrap() == expected {
                    return temp_path;
                }
            }
    ...
    .await
    .expect("streamed body did not reach the atomic temp file")
}

fn assert_body_limit(
    error: Error,
    expected_context: &'static str,
    expected_limit: usize,
    expected_status: StatusCode,
) {
    let Error::Sync(error) = error else {
        panic!("body overflow must remain a sync error: {error:?}");
    };
    let Some(SyncError::ResponseBodyTooLarge {
        context,
        limit,
        status,
    }) = error.downcast_ref::<SyncError>()
    else {
        panic!("body overflow must retain its typed cause: {error:?}");
    };
...
#[tokio::test]
async fn test_control_body_limit_preserves_context_and_status() {
    let error = collect_body_bounded(
        Body::from(vec![0; MAX_CONTROL_BODY_BYTES + 1]),
...
#[tokio::test]
async fn test_frame_body_accepts_the_protocol_limit_and_rejects_overflow() {
    let accepted = collect_body_bounded(
        Body::from(vec![0; MAX_PULL_BODY_BYTES]),
        MAX_PULL_BODY_BYTES,
        StatusCode::OK,
        "frame pull",
    )
    .await
    .unwrap();
    assert_eq!(accepted.len(), MAX_PULL_BODY_BYTES);

    let error = collect_body_bounded(
        Body::from(vec![0; MAX_PULL_BODY_BYTES + 1]),
        MAX_PULL_BODY_BYTES,
        StatusCode::OK,
        "frame pull",
    )
    .await
    .unwrap_err();
    assert_body_limit(error, "frame pull", MAX_PULL_BODY_BYTES, StatusCode::OK);
}

#[tokio::test]
async fn test_atomic_write_body_streams_before_eof_and_publishes_at_commit() {
    let temp_dir = tempdir().unwrap();
    let db_path = temp_dir.path().join("streamed.db");
    let task_path = db_path.clone();
    let (mut sender, body) = Body::channel();
    let writer = tokio::spawn(async move { atomic_write_body(task_path, body).await });

    sender
        .send_data(Bytes::from_static(b"first chunk"))
        .await
        .unwrap();
    let temp_path = wait_for_atomic_temp(temp_dir.path(), b"first chunk").await;
    assert!(!db_path.exists());

    sender
        .send_data(Bytes::from_static(b" then second"))
        .await
        .unwrap();
    drop(sender);
    writer.await.unwrap().unwrap();

    assert_eq!(std::fs::read(&db_path).unwrap(), b"first chunk then second");
    assert!(!temp_path.exists());
}

#[tokio::test]
async fn test_atomic_write_body_never_publishes_a_midstream_failure() {
    let temp_dir = tempdir().unwrap();
    ...
    sender
        ...
        .await
        .unwrap();
    wait_for_atomic_temp(temp_dir.path(), b"partial").await;
    sender.abort();
...
#[tokio::test]
async fn test_atomic_write_body_cancellation_removes_its_owned_temp() {
    let temp_dir = tempdir().unwrap();
    let db_path = temp_dir.path().join("cancelled.db");
    let task_path = db_path.clone();
    let (mut sender, body) = Body::channel();
    let writer = tokio::spawn(async move { atomic_write_body(task_path, body).await });
    ...
    sender
        ...
        .await
        .unwrap();
    let temp_path = wait_for_atomic_temp(temp_dir.path(), b"partial").await;
    writer.abort();
...
#[tokio::test]
#[ignore = "explicit 1x/2x streaming resource proof"]
async fn test_atomic_write_body_streams_full_scale_with_bounded_chunks() {
    const ONE_X_BYTES: u64 = 1_500_000_000;
    const TWO_X_BYTES: u64 = 3_000_000_000;
    const CHUNK_BYTES: usize = 64 * 1024;

    let total_bytes = std::env::var("FLUNCLE_LIBSQL_STREAM_PROOF_BYTES")
        .expect("FLUNCLE_LIBSQL_STREAM_PROOF_BYTES must select the 1x or 2x profile")
        .parse::<u64>()
        .expect("stream proof bytes must be an integer");
    assert!(matches!(total_bytes, ONE_X_BYTES | TWO_X_BYTES));

    let temp_dir = tempdir().unwrap();
    let db_path = temp_dir.path().join("stream-proof.db");
    let task_path = db_path.clone();
    let (mut sender, body) = Body::channel();
    let writer = tokio::spawn(async move { atomic_write_body(task_path, body).await });
    ...
    while sent < total_bytes {
        ...
        let chunk_len = usize::try_from(remaining.min(CHUNK_BYTES as u64)).unwrap();
        let marker = u8::try_from(chunk_index % 251).unwrap();
        sender
            .send_data(Bytes::from(vec![marker; chunk_len]))
            .await
            .unwrap();
        sent += u64::try_from(chunk_len).unwrap();
        chunk_index += 1;
    }
    drop(sender);
    writer.await.unwrap().unwrap();

    assert_eq!(std::fs::metadata(&db_path).unwrap().len(), total_bytes);
    let mut file = std::fs::File::open(&db_path).unwrap();
    let mut verified = 0_u64;
    let mut verify_chunk_index = 0_u64;
    let mut buffer = vec![0_u8; CHUNK_BYTES];
    while verified < total_bytes {
        let remaining = total_bytes - verified;
        let chunk_len = usize::try_from(remaining.min(CHUNK_BYTES as u64)).unwrap();
        file.read_exact(&mut buffer[..chunk_len]).unwrap();
        let marker = u8::try_from(verify_chunk_index % 251).unwrap();
        assert!(buffer[..chunk_len].iter().all(|byte| *byte == marker));
        verified += u64::try_from(chunk_len).unwrap();
        verify_chunk_index += 1;
    }
    assert_eq!(file.read(&mut buffer[..1]).unwrap(), 0);
    assert_eq!(verified, total_bytes);
}
