...
use hyper::{body::HttpBody as _, Body};
...
const MAX_PULL_BODY_BYTES: usize = DEFAULT_PULL_BATCH_SIZE as usize * FRAME_SIZE;
...
pub enum SyncError {
    ...
    #[error("{context} response body exceeded {limit} bytes (status={status})")]
...
pub(crate) async fn collect_body_bounded(
    ...
    while let Some(chunk) = body.data().await {
        ...
        if chunk.len() > limit.saturating_sub(bytes.len()) {
            return Err(SyncError::ResponseBodyTooLarge {
                context,
                limit,
                status,
            }
...
impl SyncContext {
    ...
    async fn push_with_retry(
        ...
        loop {
            ...
            if res.status().is_success() {
                ...
                let res_body =
                    collect_body_bounded(res.into_body(), MAX_CONTROL_BODY_BYTES, status, "push")
                        .await?;
            ...
            if nr_retries > max_retries || !res.status().is_server_error() {
                ...
                let res_body = collect_body_bounded(
                    res.into_body(),
    ...
    async fn pull_with_retry(&self, mut uri: String, max_retries: usize) -> Result<PullResult> {
        ...
        loop {
            ...
            if res.status().is_success() {
                let status = res.status();
                let frames = collect_body_bounded(
                    res.into_body(),
                    MAX_PULL_BODY_BYTES,
                    status,
                    "frame pull",
    ...
    async fn sync_db(&mut self, generation: u32) -> Result<()> {
        ...
        atomic_write_body(&self.db_path, res.into_body()).await?;
        self.durable_generation = generation;
        self.durable_frame_num = 0;
...
impl AtomicFile {
    async fn create<P: AsRef<Path>>(path: P) -> Result<Self> {
        let target_path = path.as_ref().to_path_buf();

        // Create a temporary file in the same directory as the target file
        let directory = target_path.parent().ok_or_else(|| {
    ...
    fn file_mut(&mut self) -> &mut tokio::fs::File {
        self.file.as_mut().expect("atomic file remains open")
    }
    ...
    async fn commit(mut self) -> Result<()> {
        ...
        let temp_path = self
            .temp_path
            .as_ref()
            .expect("atomic temp path remains owned");
...
async fn atomic_write<P: AsRef<Path>>(path: P, data: &[u8]) -> Result<()> {
    let mut atomic_file = AtomicFile::create(path).await?;
    atomic_file
        .file_mut()
        .write_all(data)
        .await
        .map_err(SyncError::io("temp file write_all"))?;
    atomic_file.commit().await
}

async fn atomic_write_body<P: AsRef<Path>>(path: P, mut body: Body) -> Result<()> {
    let mut atomic_file = AtomicFile::create(path).await?;
    while let Some(chunk) = body.data().await {
        let chunk = chunk.map_err(SyncError::HttpBody)?;
        atomic_file
