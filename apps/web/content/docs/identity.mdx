...
**Tidal is not covered.** There is no Tidal integration here at all, so it gets no row on the page. The API still answers Tidal explicitly, `unsupported`, so a machine caller reads the scope from the response instead of guessing at it.
