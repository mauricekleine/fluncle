...
**YouTube comes out of the audio, not out of a search.** When Fluncle buys a recording's audio he searches YouTube for it, pulls down what comes back, and fingerprints it against the official preview. That match is what can put a link here, so the row fills only for a recording he has captured since he started keeping the video id. He never goes looking for a YouTube link on its own, and that is the platform's doing: `search.list` sits in its own bucket of [100 calls a day](https://developers.google.com/youtube/v3/determine_quota_cost) that only a compliance audit lifts, and `youtube.com/robots.txt` disallows the search page. He checks the uploader too, and keeps the link only when the channel is the artist's own or one of YouTube's auto-generated artist channels, because a rip carries the right audio just as well and he would rather show you nothing than send you to one. Every other recording reads `Not checked yet`, a rip he turned down included.
