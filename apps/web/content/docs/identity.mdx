...
Seven rows on the answer page: two identifiers, **ISRC** and **MusicBrainz**, and five links out, **Spotify**, **Apple Music**, **Deezer**, **Discogs**, and **Beatport**. Apple Music is page-only, for the licence reason below.
...
**YouTube.** Going looking means searching, and `search.list` sits in its own bucket of [100 calls a day](https://developers.google.com/youtube/v3/determine_quota_cost) that only a compliance audit lifts, while `youtube.com/robots.txt` disallows the search page. Hand him a video and matching it to the recording he holds costs almost nothing, so a YouTube link can appear here when it reached him some other way. He can take a YouTube link; he cannot go find one.
