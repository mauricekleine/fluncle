...
// The vertical-swipe Stories player. One full-bleed story at a time (a 9:16
// pane centered on the cosmos at desktop widths), flicked through with a
// vertical swipe, arrow keys, or space; sound is the official preview, fading
// in through the audio engine once the first gesture unlocks it.
export function StoriesPlayer({
  ...
  useEffect(() => {
    if (track?.logId) {
      window.history.replaceState(null, "", `/stories/${encodeURIComponent(track.logId)}`);
    }
  }, [track?.logId]);
