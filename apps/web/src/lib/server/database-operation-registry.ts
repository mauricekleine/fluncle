...
export const DATABASE_ADMISSION_SHAPES: Readonly<Record<string, DatabaseAdmissionShape>> = {
  ...
  "catalogue.crawl": phased(
    ...
    "One claim is prepared in one bounded phase and its whole batch of nodes is committed in one more, around unadmitted MusicBrainz fetches; each node keeps its own receipt.",
    0,
  ),
  ...
  "track.embed": phased(
    ...
    "The worklist read and the tick's batched vector write with its cost rows are bounded phases; source-audio fetch and MuQ inference run between leases.",
    0,
  ),
  "track.enrich": phased(
...
export const TRIGGER_MUTATION_POLICY_IDS = {
  ...
  "track.embed.write-batch": "track.embed",
...
export const DATABASE_OPERATION_REGISTRY: readonly RecurringDatabaseOperation[] = [
  ...
  defineOperation({
    ...
    triggers: [
      ...
      endpoint(
        "track.capture.prepare-batch",
        "read",
        "POST",
        "/api/v1/admin/tracks/captures/prepare",
        `${SCRIPTS}/capture-sweep.ts`,
        { mutationTarget: null },
      ),
      noDatabase(
      ...
      endpoint(
        "track.capture.write-batch",
        "write",
        "POST",
        "/api/v1/admin/tracks/captures/commit",
        `${SCRIPTS}/capture-sweep.ts`,
        { mutationTarget: "primary" },
      ),
    ],
  ...
  defineOperation({
    ...
    triggers: [
      ...
      cli(
        "catalogue.crawl.commit-batch",
        "write",
        ["admin", "catalogue", "commit-nodes"],
        "fluncle admin catalogue commit-nodes --file <file> --json",
        `${SCRIPTS}/crawl-sweep.ts`,
        { mutationTarget: "primary" },
      ),
    ],
  ...
  defineOperation({
    ...
    triggers: [
      ...
      endpoint(
        ...
        "write",
        "POST",
        "/api/v1/admin/tracks/embeddings",
        `${SCRIPTS}/embed-sweep.ts`,
        { mutationTarget: "primary" },
      ),
    ],
  ...
  defineOperation({
    accessClass: "read",
    cadence: calendar("*:0/15"),
    directory: "pipeline-watch-timer",
    heavy: false,
    mutationTarget: null,
    operationId: "pipeline.watch",
    service: "fluncle-pipeline-watch.service",
    telemetryUnit: "pipeline-watch",
    timer: "fluncle-pipeline-watch.timer",
    triggers: [
      endpoint(
      ...
      endpoint(
        "pipeline.watch.capture-budget",
        "read",
        "GET",
        "/api/v1/admin/catalogue/capture-budget",
        `${SCRIPTS}/pipeline-watch.ts`,
        { mutationTarget: null },
      ),
      endpoint(
        "pipeline.watch.worklist",
        "read",
        "GET",
        "/api/v1/admin/tracks/work",
        `${SCRIPTS}/pipeline-watch.ts`,
    ...
    wrapperSource: `${SCRIPTS}/pipeline-watch.sh`,
  }),
  defineOperation({
    accessClass: "write",
