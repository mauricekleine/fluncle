...
/**
 * Build the `health` domain's handlers. `get_health` is the liveness probe, plus the
 * deployed commit `sha`. A liveness poll must never be cached, so the
 * `Cache-Control: no-store` header is applied at the rails
