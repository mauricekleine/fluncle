...
export async function getProjectionStatusFor(client: ProjectionClient): Promise<ProjectionStatus> {
  ...
  const results = await client.batch([
    ...
    },
    {
      ...
      sql: `select state, scanned_count, projected_entry_count as projected_count,
        source_digest, projected_digest, source_epoch, aggregate_epoch
  ...
  const track = trackFamilyStatus(results, trackAudit);
  const crawl = crawlFamilyStatus(results, crawlAudit);
...
function parseAnchorState(value: unknown): AnchorRebuildState | undefined {
  ...
  try {
    const state = JSON.parse(value) as AnchorRebuildState;
    return state.version === 1 && Number.isSafeInteger(state.shard) ? state : undefined;
  } catch {
    return undefined;
  }
}
...
export async function advancePublicAnchors(
  ...
  const [projection, persisted] = await Promise.all([
  ...
  const row = projection.rows[0] as
    | { default_track_total: number; generation: string; release_hub_order_epoch: number }
    | undefined;
  if (row === undefined) {
    return { complete: false, processed: 0 };
  }
  const generation = row.generation;
  const orderEpoch = Number(row.release_hub_order_epoch);
  const total = Number(row.default_track_total);
  const saved = parseAnchorState(persisted.rows[0]?.value);
  const state: AnchorRebuildState =
    saved?.generation === generation && saved.orderEpoch === orderEpoch
      ? saved
      : {
          cursorId: null,
          cursorKey: null,
  ...
  return {
    complete: (results[validityIndex]?.rowsAffected ?? 0) > 0,
...
function matchingAuditSql(alias: string, target: ProjectionAuditTarget): string {
  return `json_valid(${alias}.value)
    and json_extract(${alias}.value, '$.version') = 1
