...
// (the finding + mixtape + edition patterns, the `fluncle://` scheme scanner, and the
// shared cross-surface test vectors). This module stays as the web-app import site the
// `/log/$logId` route guard and the admin backfill validation already use, so those
// importers don't move; only the definition did.
export { isEditionLogId, isLogId, isMixtapeLogId } from "@fluncle/contracts/log-id";
