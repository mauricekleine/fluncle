...
describe("performance registry", () => {
  ...
  it("retains deterministic mixed-load distributions and bounds as JSON metadata", async () => {
    ...
    expect(metadata).toMatchObject({
      ...
      requestWideAggregateCeiling: 4,
      requestWideAggregateObservedMaximum: 4,
      requestWideFanoutPerPublicRequest: 4,
      requestWidePublicReadLatencyP50Ms: expect.any(Number),
      requestWidePublicReadLatencyP95Ms: expect.any(Number),
      requestWidePublicReadLatencyP99Ms: expect.any(Number),
      requestWidePublicReadQueueP50Ms: expect.any(Number),
      requestWidePublicReadQueueP95Ms: expect.any(Number),
      requestWidePublicReadQueueP99Ms: expect.any(Number),
      requestWidePublicRequestCount: 3,
      requestWideRuntimeSpanMetric: "request_in_flight_max",
      requestWideScope: "worker-isolate-simulator",
      requestWideWriteBatchLatencyP50Ms: expect.any(Number),
      requestWideWriteBatchLatencyP95Ms: expect.any(Number),
      requestWideWriteBatchLatencyP99Ms: expect.any(Number),
