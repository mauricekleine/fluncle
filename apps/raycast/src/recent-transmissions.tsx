...
export default function Command() {
  ...
  return (
    <List isLoading={isLoading} searchBarPlaceholder="Search recent transmissions">
      {tracks.map((track) => (
        <List.Item
          key={track.trackId}
...
function TrackActions({ track }: { track: RecentTransmission }) {
  return (
    <ActionPanel>
      <Action.Push icon={Icon.Eye} title="Show Details" target={<TrackDetail track={track} />} />
...
function TrackDetail({ track }: { track: RecentTransmission }) {
  ...
  return <Detail markdown={markdown} actions={<TrackActions track={track} />} />;
}
